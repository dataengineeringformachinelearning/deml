...
`;var Se=new Set(["accent","secondary","success","warning","danger","info","muted","subtle"]),I=class extends v{static tag="viking-badge";static legacyTag="viking-badge-wc";static get observedAttributes(){return["tone","size","icon","removable"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Gt)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"";return Se.has(t)?t:null}get size(){return this.getAttribute("size")==="sm"?"sm":null}get removable(){return this.hasAttribute("removable")&&this.getAttribute("removable")!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone;t&&this.getAttribute("tone")!==t?this.setAttribute("tone",t):!t&&this.hasAttribute("tone")&&this.removeAttribute("tone");let i=this.size;i&&this.getAttribute("size")!==i?this.setAttribute("size",i):!i&&this.hasAttribute("size")&&this.removeAttribute("size");let n=this.getAttribute("icon")??(t?rt[t]:null),r=n?u(n,16):"";this.shadow.innerHTML=`
      ${r}
      <span part="label"><slot></slot></span>
      ${this.removable?`<button type="button" class="viking-badge-remove" part="remove" aria-label="Remove">${u("x",14)}</button>`:""}
    `,this.shadow.querySelector(".viking-badge-remove")?.addEventListener("click",this.onRemove)}},at=()=>{d(I.tag,I),h(I.legacyTag,I)};var Ee=new Set(["outline","primary","secondary","filled","danger","ghost","subtle"]),Ae=new Set(["sm","xs"]),B=class extends v{static angularTag="viking-button";static tag="viking-button-wc";static get observedAttributes(){return["variant","size","type","disabled","loading","href","target","aria-label","aria-busy","square","full-width","compact"]}shadow;control=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,qt)}connectedCallback(){this.render(),this.addEventListener("click",this.onClick)}disconnectedCallback(){this.removeEventListener("click",this.onClick)}attributeChangedCallback(){this.isConnected&&this.render()}onClick=t=>{if(this.disabled||this.loading){t.preventDefault(),t.stopPropagation();return}this.dispatchEvent(new CustomEvent("viking-press",{bubbles:!0,composed:!0,detail:t}))};get variant(){let t=this.getAttribute("variant")??"outline";return Ee.has(t)?t:"outline"}get size(){let t=this.getAttribute("size");return t&&Ae.has(t)?t:null}get disabled(){return k(this,"disabled")}get loading(){return k(this,"loading")}get square(){return k(this,"square")}render(){let t=this.getAttribute("href"),i=!!t,n=i?"a":"button",r=["viking-btn",`viking-btn-${this.variant}`,this.size?`viking-btn-${this.size}`:"",this.square?"viking-btn-square":""].filter(Boolean).join(" "),o=this.getAttribute("aria-label")??"",s=this.getAttribute("aria-busy")==="true"||this.loading?"true":null,l=a(this.getAttribute("type")??"button"),c=t?a(t):"",g=this.getAttribute("target"),m=g?a(g):"";this.hasAttribute("type")&&!i&&this.removeAttribute("type"),this.shadow.innerHTML=`
      <${n}
        class="${r}"
        part="control"
        ${i?`href="${c}"`:`type="${l}"`}
        ${i&&m?`target="${m}"`:""}
        ${i&&g==="_blank"?'rel="noopener noreferrer"':""}
    ...
    `,this.control=this.shadow.querySelector(n)}},ot=()=>{d(B.tag,B)};var Ie=new Set(["accent","secondary","success","warning","danger","info","muted"]),M=class extends v{static tag="viking-callout";static legacyTag="viking-callout-wc";static get observedAttributes(){return["tone","heading","icon","dismissible","hidden"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Ft)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"info";return Ie.has(t)?t:"info"}get dismissible(){return this.hasAttribute("dismissible")&&this.getAttribute("dismissible")!=="false"}onDismiss=()=>{this.setAttribute("hidden",""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};render(){let t=this.getAttribute("heading")??"",i=this.getAttribute("icon")??rt[this.tone]??"info",n=u(i,22,"viking-callout-icon");this.shadow.innerHTML=`
      <div class="viking-callout viking-callout-${this.tone}" role="note" part="surface">
        ...
        <div class="viking-callout-body" part="body">
          ${t?`<p class="viking-callout-heading" part="heading">${a(t)}</p>`:""}
      ...
      <div class="viking-field" part="field" role="group" aria-labelledby="${this.labelId}">
        ${t?`<div class="viking-field-label-row" part="label-row">
                <span class="viking-field-label" part="label" id="${this.labelId}">
                  <span>${a(t)}</span>
                  ${r?'<span class="viking-field-required" aria-label="required">*</span>':""}
                </span>
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.shadow.querySelector(".viking-modal-close")?.addEventListener("click",()=>this.closeModal()),this.dialogEl?.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissible&&(i.preventDefault(),this.closeModal())})}},gt=()=>{d(V.tag,V),h(V.dialogTag,V)};var Me=e=>{let t=e.getAttribute("items");if(!t)return[];try{let i=JSON.parse(t);return Array.isArray(i)?i:[]}catch{return[]}},Ce=(e,t)=>[e.title,e.snippet??"",e.group??"",e.href,...e.keywords??[]].join(" ").toLowerCase().includes(t),Te=(e,t)=>{let i=t.trim().toLowerCase();return i?e.filter(n=>Ce(n,i)):e},Le=e=>{let t=new Map;return e.forEach(i=>{let n=i.group??null,r=t.get(n)??[];r.push(i),t.set(n,r)}),Array.from(t.entries()).map(([i,n])=>({group:i,items:n}))},_=class extends v{static tag="viking-command-palette";static searchTag="viking-search-palette";static legacyTag="viking-search-palette-wc";static get observedAttributes(){return["open","placeholder","items","global-shortcut"]}shadow;dialogEl=null;inputEl=null;resultsEl=null;globalKeyHandler=null;resultsId=E("viking-search-results");inputId=E("viking-search-input");query="";activeIndex=0;flatResults=[];constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Xt)}connectedCallback(){this.render(),this.removeAttribute("open"),this.syncOpen(),this.bindGlobalShortcut(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick),this.inputEl?.addEventListener("input",this.onInput),this.inputEl?.addEventListener("keydown",this.onInputKeydown)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick),this.inputEl?.removeEventListener("input",this.onInput),this.inputEl?.removeEventListener("keydown",this.onInputKeydown),this.unbindGlobalShortcut()}attributeChangedCallback(t){if(this.isConnected&&(t==="open"&&this.syncOpen(),t==="global-shortcut"&&(this.unbindGlobalShortcut(),this.bindGlobalShortcut()),t==="items"||t==="placeholder"))if(t==="placeholder"&&this.inputEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026";this.inputEl.placeholder=i,this.inputEl.setAttribute("aria-label",i)}else this.renderResults()}openPalette(){this.setAttribute("open",""),this.syncOpen()}closePalette(){this.removeAttribute("open"),R(this.dialogEl)}search(t){this.query=t,this.inputEl&&(this.inputEl.value=t),this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))}onClose=()=>{this.removeAttribute("open"),this.query="",this.activeIndex=0,this.inputEl&&(this.inputEl.value=""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{t.target===this.dialogEl&&this.closePalette()};onInput=t=>{this.query=t.target.value??this.inputEl?.value??"",this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))};onInputKeydown=t=>{if(this.flatResults.length===0){t.key==="Escape"&&(t.preventDefault(),this.closePalette());return}if(t.key==="ArrowDown"){t.preventDefault(),this.activeIndex=Math.min(this.flatResults.length-1,this.activeIndex+1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="ArrowUp"){t.preventDefault(),this.activeIndex=Math.max(0,this.activeIndex-1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="Enter"){t.preventDefault();let i=this.flatResults[this.activeIndex];i&&this.activateItem(i)}};bindGlobalShortcut(){k(this,"global-shortcut")&&(this.globalKeyHandler=t=>{(t.metaKey||t.ctrlKey)&&t.key.toLowerCase()==="k"&&(t.preventDefault(),this.hasAttribute("open")?this.closePalette():this.openPalette())},document.addEventListener("keydown",this.globalKeyHandler))}unbindGlobalShortcut(){this.globalKeyHandler&&(document.removeEventListener("keydown",this.globalKeyHandler),this.globalKeyHandler=null)}syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(this.dialogEl.removeAttribute("aria-hidden"),it(this.dialogEl),this.activeIndex=0,this.renderResults(),queueMicrotask(()=>this.inputEl?.focus())):!t&&this.dialogEl.open?(R(this.dialogEl),this.dialogEl.setAttribute("aria-hidden","true")):t||this.dialogEl.setAttribute("aria-hidden","true")}scrollActiveIntoView(){this.resultsEl?.querySelector(".viking-search-result.is-selected")?.scrollIntoView({block:"nearest"})}activateItem(t){if(this.dispatchEvent(new CustomEvent("viking-select",{bubbles:!0,composed:!0,detail:{item:t}})),this.closePalette(),t.action==="cookie-settings"){globalThis.DemlWidgets?.openCookieSettings?.();return}if(t.action==="bug-report"){let i=globalThis.DemlWidgets;if(i?.openBugReport){i.openBugReport();return}}if(t.href&&t.href!=="#")try{let i=new URL(t.href,window.location.href);i.origin===window.location.origin?window.location.assign(`${i.pathname}${i.search}${i.hash}`):window.location.assign(i.href)}catch{window.location.assign(t.href)}}renderResults(){if(!this.resultsEl)return;let t=Me(this),i=this.query.trim().toLowerCase(),n=Te(t,i);if(this.flatResults=n,n.length===0){let c=this.query.trim();this.resultsEl.innerHTML=`<p class="viking-search-empty" role="status">${c?"No results found":"Start typing to search\u2026"}</p>`,this.inputEl?.removeAttribute("aria-activedescendant");return}this.activeIndex>=n.length&&(this.activeIndex=n.length-1);let r=0,s=Le(n).map(({group:c,items:g})=>{let m=c?`<p class="viking-search-group-label" role="presentation">${a(c)}</p>`:"",f=g.map(b=>{let x=`${this.resultsId}-result-${r}`,A=r===this.activeIndex;return r+=1,`
              ...
              >
                <div>
                  <div class="viking-search-result-title">${a(b.title)}</div>
                  ${b.snippet?`<div class="viking-search-result-snippet">${a(b.snippet)}</div>`:""}
                </div>
              </a>`}).join("");return`${m}${f}`}).join("");this.resultsEl.innerHTML=`<div class="viking-search-results" id="${this.resultsId}" role="listbox" aria-label="Search results">${s}</div>`;let l=`${this.resultsId}-result-${this.activeIndex}`;this.inputEl?.setAttribute("aria-activedescendant",l),this.inputEl?.setAttribute("role","combobox"),this.inputEl?.setAttribute("aria-expanded","true"),this.inputEl?.setAttribute("aria-controls",this.resultsId),this.resultsEl.querySelectorAll(".viking-search-result").forEach(c=>{c.addEventListener("click",g=>{let m=g;if(m.metaKey||m.ctrlKey||m.shiftKey||m.button===1){this.closePalette();return}g.preventDefault();let f=Number(c.dataset.index??0),b=this.flatResults[f];b&&this.activateItem(b)}),c.addEventListener("mouseenter",()=>{let g=Number(c.dataset.index??0);if(g===this.activeIndex)return;this.activeIndex=g,this.resultsEl?.querySelectorAll(".viking-search-result").forEach((f,b)=>{let x=b===g;f.classList.toggle("is-selected",x),f.setAttribute("aria-selected",x?"true":"false")});let m=`${this.resultsId}-result-${g}`;this.inputEl?.setAttribute("aria-activedescendant",m)})})}render(){let t=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026",i=nt();this.shadow.innerHTML=`
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.inputEl=this.shadow.querySelector("input"),this.resultsEl=this.shadow.querySelector(".viking-search-results-host"),this.shadow.querySelector(".viking-search-palette-close")?.addEventListener("click",()=>this.closePalette()),this.dialogEl?.addEventListener("keydown",n=>{n.key==="Escape"&&(n.preventDefault(),this.closePalette())})}},q=()=>{d(_.tag,_),h(_.searchTag,_),h(_.legacyTag,_)};var Ve=new Set(["accent","secondary","success","warning","danger","info","muted"]),$=class extends v{static tag="viking-status-card";static legacyTag="viking-status-card-wc";static get observedAttributes(){return["title","subtitle","status","status-tone","status-dot","href","target","compact","loading","interactive","aria-label"]}shadow=this.attachShadow({mode:"open"});connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get compact(){let t=this.getAttribute("compact");return t!==null&&t!=="false"}get loading(){let t=this.getAttribute("loading");return t!==null&&t!=="false"}get interactive(){let t=this.getAttribute("interactive");return t!==null&&t!=="false"}get href(){return this.getAttribute("href")}get target(){return this.getAttribute("target")}get statusTone(){let t=this.getAttribute("status-tone")??"muted";return Ve.has(t)?t:"muted"}get statusDot(){let t=this.getAttribute("status-dot");return t!==null&&t!=="false"}get cardTitle(){return this.getAttribute("title")??""}get subtitle(){return this.getAttribute("subtitle")??""}get status(){return this.getAttribute("status")??""}render(){let t=this.status.length>0,i=this.cardTitle,n=this.subtitle,r=this.compact?" status-card--compact":"",o=this.loading?" status-card--loading":"",s=this.interactive?" status-card--interactive":"",l=`status-card${r}${o}${s}`,c=this.target==="_blank"?' rel="noopener noreferrer"':"",g=this.statusDot?'<span class="status-card__status-dot" aria-hidden="true"></span>':"",m=t?`<span class="status-card__status status-card__status-${this.statusTone}" part="status">${g}<span>${a(this.status)}</span></span>`:"",f=this.href?a(this.href):"",b=a(this.getAttribute("aria-label")??""),x=f?"a":"div",A=`
      <header class="status-card__header">
        <div class="status-card__title-wrap">
          ${i?`<h3 class="status-card__title" part="title">${a(i)}</h3>`:""}
          ${n?`<p class="status-card__subtitle" part="subtitle">${a(n)}</p>`:""}
        </div>
        ${t||g?`<div class="status-card__status-wrap">${m}</div>`:""}
      </header>
    `;this.shadow.innerHTML=`
      <style>
        ${$e}
      ...
      <${x}
        ...
        part="card"
        ${f?`href="${f}"`:""}
        ${this.target?`target="${a(this.target)}"`:""}
        ${f?c:""}
        ${b?`aria-label="${b}"`:""}
      >
        ${A}
...
`,vt=()=>{d($.tag,$),h($.legacyTag,$)};var Ne=new Set(["accent","secondary","success","warning","danger","info","muted"]),N=class extends v{static tag="viking-status-pill";static legacyTag="viking-status-pill-wc";static get observedAttributes(){return["tone","icon","href","target","compact","dot","removable","aria-label"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,He)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"muted";return Ne.has(t)?t:"muted"}get compact(){let t=this.getAttribute("compact");return t!==null&&t!=="false"}get removable(){let t=this.getAttribute("removable");return t!==null&&t!=="false"}get href(){return this.getAttribute("href")}get icon(){return this.getAttribute("icon")}get showDot(){let t=this.getAttribute("dot");return t!==null&&t!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-pill-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone,i=this.compact?" status-pill--compact":"",n=this.icon?u(this.icon,14,"status-pill__icon"):"",r=this.showDot?'<span class="status-pill__dot" aria-hidden="true"></span>':"",o='<span class="status-pill__label" part="label"><slot></slot></span>',s=a(this.getAttribute("aria-label")??""),l=this.href,c=this.getAttribute("target"),g=l&&c==="_blank"?' rel="noopener noreferrer"':"";if(l){this.shadow.innerHTML=`
        <a
    ...
    `,this.shadow.querySelector(".status-pill__remove")?.addEventListener("click",this.onRemove)}},He=`
:host {
  display: inline-flex;
}
...
`,ht=()=>{d(N.tag,N),h(N.legacyTag,N)};var U=[{id:"explore",label:"Explore",icon:"globe",appHref:"/explore",marketingHref:"/explore",platform:!0},{id:"documentation",label:"Documentation",icon:"file",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{id:"blue-notes",label:"Blue Notes",icon:"file",appHref:"https://dataengineeringformachinelearning.com/blog/",marketingHref:"/blog/"},{id:"dashboard",label:"Dashboard",icon:"home",appHref:"/dashboard",marketingHref:"/dashboard",requireAuth:!0,platform:!0},{id:"sites",label:"Sites",icon:"building",appHref:"/settings",marketingHref:"/settings",requireAuth:!0,platform:!0},{id:"account",label:"Account",icon:"user",appHref:"/account",marketingHref:"/account",requireAuth:!0,platform:!0}],ut=[{title:"Platform",links:[{label:"Explore",appHref:"/explore",marketingHref:"/explore",platform:!0},{label:"Dashboard",appHref:"/dashboard",marketingHref:"/dashboard",platform:!0,requireAuth:!0},{label:"Sites",appHref:"/settings",marketingHref:"/settings",platform:!0,requireAuth:!0},{label:"Account",appHref:"/account",marketingHref:"/account",platform:!0,requireAuth:!0}]},{title:"Resources",links:[{label:"Documentation",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{label:"Whitepaper",appHref:"https://dataengineeringformachinelearning.com/whitepaper/",marketingHref:"/whitepaper"},{label:"Book",appHref:"https://dataengineeringformachinelearning.com/book/",marketingHref:"/book"},{label:"Blue Notes",appHref:"https://dataengineeringformachinelearning.com/blog/",marketingHref:"/blog/"}]},{title:"Support",links:[{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0},{label:"Report a Bug",appHref:"#",marketingHref:"#",action:"bug-report"}]},{title:"Legal & Compliance",links:[{label:"Privacy Policy",appHref:"https://dataengineeringformachinelearning.com/privacy/",marketingHref:"/privacy"},{label:"Terms of Service",appHref:"https://dataengineeringformachinelearning.com/terms/",marketingHref:"/terms"},{label:"SOC2 Compliance",appHref:"https://dataengineeringformachinelearning.com/compliance/",marketingHref:"/compliance"},{label:"GDPR Compliance",appHref:"https://dataengineeringformachinelearning.com/privacy/#gdpr",marketingHref:"/privacy#gdpr"},{label:"Cookie Settings",appHref:"#",marketingHref:"#",action:"cookie-settings"}]}],Oe="reportBug=1",De="cookieSettings=1",Jt=e=>/^https?:\/\//i.test(e),pt=(e,t)=>{if(Jt(t))return t;let i=e.replace(/\/$/,""),n=t.startsWith("/")?t:`/${t}`;return`${i}${n}`},Lt=e=>`${pt(e.app,"/")}?${Oe}`,Vt=e=>`${pt(e.marketing,"/")}?${De}`,Qt=(e,t)=>Jt(e)||e.startsWith("mailto:")||e.startsWith("/")?e:pt(t.marketing,e),Zt=(e,t,i)=>t==="app"?e:pt(i.app,e),W=(e,t,i)=>e.platform?Zt(e.appHref,t,i):t==="app"?e.appHref:Qt(e.marketingHref,i),kt=(e,t,i)=>e.action==="bug-report"?t==="app"?"#":Lt(i):e.action==="cookie-settings"?Vt(i):e.platform?Zt(e.appHref,t,i):t==="app"?e.appHref:Qt(e.marketingHref,i),mt=(e,t)=>e==="marketing"?"/":t.marketing,te=(e,t=!1)=>e.filter(i=>!i.requireAuth||t);var y={app:"https://deml.app",marketing:"https://dataengineeringformachinelearning.com",backend:"https://backend.deml.app"};var ee=e=>{if(e==null)return!1;let t=e.trim().toLowerCase();return t===""||t==="true"||t==="1"?!0:!(t==="false"||t==="0"||t==="off"||t==="no")},bt=(e,t=0)=>{if(e==null||e.trim()==="")return t;let i=Number(e);return Number.isFinite(i)?i:t},ie=e=>{if(!(e==null||e.trim()===""))try{return JSON.parse(e)}catch{return}},ft=(e,t,i)=>{let n=i??t[0];if(n===void 0)throw new Error("parseSelect requires a non-empty options list");return e==null?n:t.includes(e)?e:n};var G=class extends v{_props={};_updateScheduled=!1;connectedCallback(){this.scheduleRender()}attributeChangedCallback(t,i,n){i!==n&&this.isConnected&&this.scheduleRender()}attr(t,i){let n=this.getAttribute(t);return i?.parser?n==null&&i.default!==void 0?i.default:i.parser(n):n??(i?.default!==void 0?String(i.default):null)}setProp(t,i){Object.is(this._props[t],i)||(this._props[t]=i,this.isConnected&&this.scheduleRender())}getProp(t){return this._props[t]}requestUpdate(){this.scheduleRender()}scheduleRender(){this._updateScheduled||(this._updateScheduled=!0,queueMicrotask(()=>{this._updateScheduled=!1,this.isConnected&&this.render()}))}};var re=["app","marketing","backend","docs"],ne=()=>{if(typeof document>"u")return"marketing";let e=document.documentElement.getAttribute("data-deml-context");if(e&&re.includes(e))return e;if(typeof window>"u")return"marketing";let t=window.location.hostname;return t.startsWith("backend.")?"backend":t.startsWith("ui.")||t.includes("ui.dataengineeringformachinelearning")?"docs":t.includes("deml.app")?"app":"marketing"},H=class extends G{static tag="viking-site-footer";static legacyTag="viking-site-footer-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","year","authenticated"]}onAuthState=t=>{let i=t.detail;this.toggleAttribute("authenticated",i?.isAuthenticated===!0)};connectedCallback(){window.addEventListener("deml:auth-state",this.onAuthState),document.documentElement.dataset.authenticated==="true"&&this.setAttribute("authenticated",""),super.connectedCallback()}disconnectedCallback(){window.removeEventListener("deml:auth-state",this.onAuthState)}resolveContext(){return this.attr("context",{parser:t=>ft(t,re,ne()),default:ne()})}resolveUrls(){let t=globalThis.__DEML??{};return{app:this.getAttribute("app-url")??t.app??y.app,marketing:this.getAttribute("marketing-url")??t.marketing??y.marketing,backend:this.getAttribute("backend-url")??t.backend??y.backend}}resolveYear(){return this.attr("year",{parser:t=>bt(t,new Date().getFullYear()),default:new Date().getFullYear()})}render(){let t=this.resolveContext(),i=this.resolveUrls(),n=this.resolveYear(),r=this.hasAttribute("authenticated"),o=ut.map(s=>{let l=s.links.filter(c=>!c.requireAuth||r).map(c=>{let g=kt(c,t,i);return`
          <li>
            <a href="${a(g)}">${a(c.label)}</a>
          </li>
        `}).join("");return`
      <div class="footer-column">
        <h3 class="footer-column-title">${a(s.title)}</h3>
            ...
            <div class="footer-compliance-row">
              <div class="copyright-info">
                <span class="copyright-text">
                  Copyright \xA9 ${n} Data Engineering for AI Engineering and Cybersecurity by
    ...
    `,this.bindUsaBadge()}emitUsaBadgeHover=t=>{if(t.type==="keydown"){let i=t.key;if(i!=="Enter"&&i!==" ")return;t.preventDefault()}this.dispatchEvent(new CustomEvent("usaBadgeHover",{bubbles:!0,composed:!0,detail:t}))};bindUsaBadge(){let t=this.querySelector("#usa-badge");t&&(t.addEventListener("mouseenter",this.emitUsaBadgeHover),t.addEventListener("focus",this.emitUsaBadgeHover),t.addEventListener("keydown",this.emitUsaBadgeHover))}},Q=()=>{d(H.tag,H),h(H.legacyTag,H)};Q();var ae=new Set(["app","marketing","backend","docs"]),ze=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&ae.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},Pe=e=>{let t=e.getAttribute("context");return t&&ae.has(t)?t:ze()},oe=(e,t)=>{let i=e.getAttribute(t);return i!==null&&i!=="false"},Re=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??y.app,marketing:e.getAttribute("marketing-url")??t.marketing??y.marketing,backend:e.getAttribute("backend-url")??t.backend??y.backend}},Ke=e=>oe(e,"authenticated"),Be=e=>e.getAttribute("show-search")===null?!0:oe(e,"show-search"),qe=(e,t)=>{let i=e.getAttribute("sign-in-href");if(i)return i;let n=e.getAttribute("return-url")??window.location.href;return`${t.app}/login?returnUrl=${encodeURIComponent(n)}`},P=(e,t,i={})=>{let n=i.id?` id="${a(i.id)}"`:"",r=i.color?` data-viking-icon-color="${a(i.color)}"`:"",o=i.className?` class="${a(i.className)}"`:"";return`<span${n}${o} data-viking-icon="${a(e)}" data-viking-icon-size="${t}"${r} aria-hidden="true">${u(e,t,"viking-navbar-inline-icon")}</span>`},O=class extends HTMLElement{static tag="viking-site-navbar";static legacyTag="viking-site-navbar-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","show-search","sign-in-href","dashboard-href","return-url"]}mobileMenuOpen=!1;connectedCallback(){this.render(),this.addEventListener("click",this.onHostClick),document.addEventListener("click",this.onDocumentClick),document.addEventListener("keydown",this.onDocumentKeydown),window.addEventListener("resize",this.onWindowResize)}disconnectedCallback(){this.removeEventListener("click",this.onHostClick),document.removeEventListener("click",this.onDocumentClick),document.removeEventListener("keydown",this.onDocumentKeydown),window.removeEventListener("resize",this.onWindowResize)}attributeChangedCallback(){this.isConnected&&this.render()}renderNavLinks(t,i,n="desktop"){let r=n==="mobile"?"mobile-nav-btn":"nav-btn";return U.map(o=>{let s=W(o,t,i),l=o.requireAuth,c=l?" hidden":"",g=l?' data-require-auth="true"':"";return`
        <a
          href="${a(s)}"
          class="${r}"
          data-nav-id="${a(o.id)}"
    ...
    `:""}render(){let t=Pe(this),i=Re(this),n=Ke(this),r=Be(this),o=mt(t,i),s=qe(this,i),l=this.getAttribute("dashboard-href")??`${i.app}/dashboard`,c=this.renderNavLinks(t,i,"desktop"),g=this.renderNavLinks(t,i,"mobile"),m=this.renderAuthDesktop(n,s,l),f=this.renderAuthMobile(n,l,s),b=this.renderSearchButton(r);this.innerHTML=`
      <header class="navbar">
        <div class="navbar-content">
          <div class="navbar-left">
            <a href="${a(o)}" class="navbar-brand" aria-label="Go to homepage" id="navbar-brand-link">
    ...
    `,this.syncMobileMenu()}onHostClick=t=>{let i=typeof t.composedPath=="function"?t.composedPath():[];if(i.some(o=>o instanceof HTMLElement&&o.id==="mobile-menu-btn")){t.preventDefault(),this.toggleMobileMenu();return}i.some(o=>o instanceof HTMLElement&&o.closest?.("#mobile-menu")&&(o.matches("a")||o.matches("viking-button-wc")))&&this.closeMobileMenu()};onDocumentClick=t=>{if(!this.mobileMenuOpen)return;let i=t.target;i instanceof Node&&!this.contains(i)&&this.closeMobileMenu()};onDocumentKeydown=t=>{t.key==="Escape"&&this.closeMobileMenu()};onWindowResize=()=>{window.matchMedia("(min-width: 768px)").matches&&this.closeMobileMenu()};toggleMobileMenu(){this.mobileMenuOpen=!this.mobileMenuOpen,this.syncMobileMenu()}closeMobileMenu(){this.mobileMenuOpen&&(this.mobileMenuOpen=!1,this.syncMobileMenu())}syncMobileMenu(){let t=this.querySelector("#mobile-menu"),i=this.querySelector("#mobile-menu-btn");!t||!i||(t.classList.toggle("open",this.mobileMenuOpen),t.toggleAttribute("hidden",!this.mobileMenuOpen),i.setAttribute("aria-expanded",this.mobileMenuOpen?"true":"false"),i.setAttribute("aria-label",this.mobileMenuOpen?"Close navigation menu":"Toggle navigation menu"),i.innerHTML=P(this.mobileMenuOpen?"x":"menu",24))}},yt=()=>{d(O.tag,O),h(O.legacyTag,O)};var Ue=e=>{let t=new Set;return e.filter(i=>{let n=`${i.title}:${i.href}:${i.action??""}`;return t.has(n)?!1:(t.add(n),!0)})},We=(e,t,i)=>({title:e.label,href:W(e,t,i),snippet:`Open ${e.label}`,group:"Platform",keywords:[e.id,e.label.toLowerCase(),"navigate","go"]}),Ge=(e,t,i,n)=>e.action==="cookie-settings"?{title:e.label,href:Vt(n),snippet:"Manage analytics and cookie preferences",group:t,keywords:["cookies","consent","privacy","gdpr"],action:"cookie-settings"}:e.action==="bug-report"?{title:e.label,href:i==="app"?"#":Lt(n),snippet:"Submit a product issue or regression",group:t,keywords:["bug","issue","support","feedback"],action:"bug-report"}:{title:e.label,href:kt(e,i,n),snippet:`Open ${e.label}`,group:t,keywords:[e.label.toLowerCase(),t.toLowerCase()]},Fe=[{title:"Viking-UI Components",snippet:"Browse the design system showcase",group:"Resources",keywords:["viking","ui","design system","components","showcase"]},{title:"Design tokens",snippet:"Canonical --viking-* token matrix",group:"Resources",keywords:["tokens","theme","css","variables"]},{title:"API reference",snippet:"OpenAPI schema and REST endpoints",group:"Resources",keywords:["api","openapi","swagger","rest"]}],je=(e,t,i)=>{switch(e.title){case"Viking-UI Components":return"https://ui.dataengineeringformachinelearning.com/components";case"Design tokens":return"https://ui.dataengineeringformachinelearning.com/tokens";case"API reference":return`${i.backend.replace(/\/$/,"")}/api/v1/docs`;default:return t==="app"?i.app:i.marketing}},Ye=[{title:"Components",href:"/components",snippet:"Browse all documented primitives",group:"Viking-UI",keywords:["components","showcase","registry"]},{title:"Playground",href:"/playground",snippet:"Live Web Component sandbox",group:"Viking-UI",keywords:["playground","sandbox","demo"]},{title:"Architecture",href:"/architecture",snippet:"CSS + WC + Angular layers",group:"Viking-UI",keywords:["architecture","layers","web component"]},{title:"Design tokens",href:"/tokens",snippet:"Canonical --viking-* token matrix",group:"Viking-UI",keywords:["tokens","theme","css","variables"]},{title:"Theming",href:"/theming",snippet:"Light/dark mode and sync pipeline",group:"Viking-UI",keywords:["theming","dark","light","mode"]},{title:"Framework guides",href:"/frameworks",snippet:"Angular, Astro, Django setup",group:"Viking-UI",keywords:["frameworks","angular","astro","django"]},{title:"Contributing",href:"/contributing",snippet:"Extend the Viking-UI kit",group:"Viking-UI",keywords:["contributing","extend","primitives"]}],Xe=(e,t)=>e.startsWith("http")?e:`${t.replace(/\/$/,"")}${e.startsWith("/")?e:`/${e}`}`,$t=(e,t,i)=>{let n=e==="docs"?"marketing":e,r=i?.authenticated??e==="app",o=[...U.filter(s=>!s.requireAuth||r).map(s=>We(s,n,t)),...ut.flatMap(s=>s.links.filter(l=>!l.requireAuth||r).map(l=>Ge(l,s.title,n,t))),...Fe.map(s=>({...s,href:je(s,n,t)}))];if(e==="app"&&o.push({title:"Dashboard",href:"/dashboard",snippet:"CES overview, KPIs, and performance telemetry",group:"App",keywords:["dashboard","ces","home","overview","kpi"]},{title:"Analytics",href:"/analytics",snippet:"Latency, origins, threat charts, and gauges",group:"App",keywords:["analytics","charts","latency","map","threat"]},{title:"Explore status pages",href:"/explore",snippet:"Public status directory",group:"App",keywords:["explore","status","directory","public"]},{title:"Status pages",href:"/status",snippet:"Your published and draft status surfaces",group:"App",keywords:["status","pages","uptime","incidents"]},{title:"Platform status",href:"/status/platform-status",snippet:"Live public sentinel for the DEML stack",group:"App",keywords:["platform-status","tenant0","health","sla"]},{title:"Vulnerabilities",href:"/vulnerabilities",snippet:"SOC triage and vulnerability Kanban",group:"App",keywords:["vulnerabilities","soc","semgrep","trivy","kanban"]},{title:"Account",href:"/account",snippet:"Profile, MFA enrollment, and linked accounts",group:"App",keywords:["account","profile","mfa","oauth"]},{title:"Settings",href:"/settings",snippet:"Workspace domains, billing, and security",group:"App",keywords:["settings","sites","workspace","configuration"]},{title:"Billing & subscription",href:"/settings/billing",snippet:"Manage plan, invoices, and payment methods",group:"App",keywords:["billing","stripe","subscription","payment"]},{title:"Security settings",href:"/settings/security",snippet:"Keys, sessions, and access controls",group:"App",keywords:["security","keys","auth","rbac"]},{title:"Login",href:"/login",snippet:"Sign in or complete SMS MFA",group:"App",keywords:["login","sign in","auth","mfa"]}),e==="backend"&&o.push({title:"OpenAPI / Swagger",href:`${t.backend.replace(/\/$/,"")}/api/v1/docs`,snippet:"Interactive public ingest and predict sandbox",group:"Backend",keywords:["swagger","openapi","docs","api"]},{title:"Backend home",href:t.backend,snippet:"API landing and service overview",group:"Backend",keywords:["backend","api","home"]}),e==="docs"){let s=i?.docsOrigin??"https://ui.dataengineeringformachinelearning.com";o.push(...Ye.map(l=>({...l,href:Xe(l.href,s)})))}return Ue(o)};var se=["dataengineeringformachinelearning_com_zjafyosh2v_pages","deml_app_pages","deml_backend_pages","DEML UI"],Je={dataengineeringformachinelearning_com_zjafyosh2v_pages:"https://dataengineeringformachinelearning.com",deml_app_pages:"https://deml.app",deml_backend_pages:"https://backend.deml.app","DEML UI":"https://ui.dataengineeringformachinelearning.com"},Qe=()=>typeof globalThis>"u"?{}:globalThis.ALGOLIA_CONFIG??{},w=e=>typeof e=="string"&&e.trim()?e:null,Z=e=>{let t=w(e);return t&&/^https?:\/\//i.test(t)?t:null},Ze=(e,t)=>{let i=w(e.objectID),n=Z(e.url)||Z(e.url_without_anchor)||Z(e.permalink)||Z(e.link)||Z(i);if(n)return n;let r=w(e.path);if(!r)return null;let o=w(e.hostname),s=Je[t]||(o?`https://${o.replace(/^https?:\/\//i,"")}`:null);if(!s)return null;try{return new URL(r.startsWith("/")?r:`/${r}`,s).href}catch{return null}},ti=(e,t)=>{let i=Ze(e,t);if(!i)return null;let n=e.hierarchy,r=w(e.title)||w(n?.lvl1)||w(n?.lvl0)||w(e.path)||i,o=w(e.description)||(w(e.content)??"").slice(0,160)||w(n?.lvl2)||"",s=w(e.hostname);if(!s)try{s=new URL(i).hostname}catch{s=t}let l=w(e.keywords),c=w(e.path)??"";return{title:r.slice(0,120),href:i,snippet:o?`${s} \xB7 ${o.slice(0,140)}`:s,group:"Live pages",keywords:[s,t,c,...l?l.split(/[,\s]+/).filter(Boolean):[]].filter(Boolean)}},xt=async(e,t)=>{let i=e.trim();if(!i||i.length<2)return[];let n=Qe(),r=n.appId||"ZJAFYOSH2V",o=n.apiKey||"";if(!o)return[];let s=n.indexNames&&n.indexNames.length>0?n.indexNames:[...se],l=t?.hitsPerPage??8,c=`https://${r}-dsn.algolia.net/1/indexes/*/queries`;try{let g=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json","X-Algolia-Application-Id":r,"X-Algolia-API-Key":o},body:JSON.stringify({requests:s.map(x=>({indexName:x,params:new URLSearchParams({query:i,hitsPerPage:String(l),attributesToRetrieve:["url","url_without_anchor","permalink","link","title","description","content","path","hostname","keywords","hierarchy","type","objectID"].join(","),attributesToHighlight:"[]"}).toString()}))}),signal:t?.signal});if(!g.ok)return[];let m=await g.json(),f=new Set,b=[];return(m.results??[]).forEach((x,A)=>{let Et=x.index||s[A]||"pages";(x.hits??[]).forEach(At=>{let z=ti(At,Et);!z||f.has(z.href)||(f.add(z.href),b.push(z))})}),b}catch{return[]}},le=se;var de=new Set(["app","marketing","backend","docs"]),ei=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&de.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.includes("deml.app")&&!t.startsWith("backend.")?"app":t.startsWith("backend.")?"backend":"marketing"},ii=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??y.app,marketing:e.getAttribute("marketing-url")??t.marketing??y.marketing,backend:e.getAttribute("backend-url")??t.backend??y.backend}},ni=e=>{let t=e.getAttribute("context");return t&&de.has(t)?t:ei()},ce=e=>e.hasAttribute("authenticated")||document.documentElement.dataset.authenticated==="true",S=class extends v{static tag="viking-suite-command-palette";static searchTag="viking-suite-search-palette";static legacyTag="viking-suite-search-palette-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","placeholder","global-shortcut","authenticated"]}paletteEl=null;itemsLoaded=!1;curatedItems=[];queryTimer=null;searchAbort=null;onAuthState=t=>{let i=t.detail;this.toggleAttribute("authenticated",i?.isAuthenticated===!0)};onPaletteQuery=t=>{let n=t.detail?.query??"";this.scheduleAlgoliaSearch(n)};connectedCallback(){q(),window.addEventListener("deml:auth-state",this.onAuthState),this.ensurePalette(),this.loadItems()}disconnectedCallback(){window.removeEventListener("deml:auth-state",this.onAuthState),this.paletteEl?.removeEventListener("viking-query",this.onPaletteQuery),this.queryTimer&&(clearTimeout(this.queryTimer),this.queryTimer=null),this.searchAbort?.abort()}attributeChangedCallback(t){if(this.isConnected){if(t==="placeholder"&&this.paletteEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",i);return}if(t==="global-shortcut"&&this.paletteEl){k(this,"global-shortcut")||!this.hasAttribute("global-shortcut")?this.paletteEl.setAttribute("global-shortcut",""):this.paletteEl.removeAttribute("global-shortcut");return}(t==="context"||t==="app-url"||t==="marketing-url"||t==="backend-url"||t==="authenticated")&&this.loadItems(!0)}}openPalette(){this.ensurePalette(),this.loadItems().then(()=>this.paletteEl?.openPalette())}closePalette(){this.paletteEl?.closePalette()}ensurePalette(){if(this.paletteEl)return;this.paletteEl=document.createElement("viking-command-palette"),this.paletteEl.id="deml-command-palette";let t=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",t),(k(this,"global-shortcut")||!this.hasAttribute("global-shortcut"))&&this.paletteEl.setAttribute("global-shortcut",""),this.paletteEl.addEventListener("viking-query",this.onPaletteQuery),this.append(this.paletteEl)}scheduleAlgoliaSearch(t){this.queryTimer&&clearTimeout(this.queryTimer),this.queryTimer=setTimeout(()=>{this.mergeAlgoliaResults(t)},180)}async mergeAlgoliaResults(t){if(!this.paletteEl)return;this.itemsLoaded||await this.loadItems();let i=t.trim();if(i.length<2){this.paletteEl.setAttribute("items",JSON.stringify(this.curatedItems));return}this.searchAbort?.abort(),this.searchAbort=new AbortController;let n=await xt(i,{hitsPerPage:6,signal:this.searchAbort.signal}),r=this.curatedItems,o=new Set(r.map(l=>l.href.replace(/\/$/,""))),s=n.filter(l=>{let c=l.href.replace(/\/$/,"");return o.has(c)?!1:(o.add(c),!0)}).map(l=>({...l,keywords:[...l.keywords??[],i,l.title,l.href]}));this.paletteEl.setAttribute("items",JSON.stringify([...r,...s]))}async loadItems(t=!1){if(!this.paletteEl||this.itemsLoaded&&!t)return;let i=ni(this),n=ii(this),r=i==="docs"?window.location.origin:"https://ui.dataengineeringformachinelearning.com",o=$t(i,n,{docsOrigin:r,authenticated:ce(this)});try{(await fetch("/assets/site-drakkar.json",{cache:"no-cache"})).ok&&(o=$t(i,n,{docsOrigin:r,authenticated:ce(this)}))}catch{}this.curatedItems=o,this.paletteEl.setAttribute("items",JSON.stringify(o)),this.itemsLoaded=!0}},F=()=>{d(S.tag,S),h(S.searchTag,S),h(S.legacyTag,S)};var ge=`
:host {
  display: inline-flex;
}
...
`,ri=e=>{document.documentElement.setAttribute("data-theme",e),document.documentElement.classList.toggle("dark",e==="dark"),localStorage.setItem("theme",e)},ve=()=>{let e=localStorage.getItem("theme");return e==="light"||e==="dark"?e:window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"},j=class extends v{static tag="viking-theme-toggle-wc";shadow;button=null;sunIcon=null;moonIcon=null;onStorage=()=>{this.syncIcons()};constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,ge)}connectedCallback(){this.setAttribute("role","button"),this.render(),this.syncIcons(),this.button?.addEventListener("click",this.onClick),window.addEventListener("storage",this.onStorage),window.addEventListener("viking-theme-change",this.onThemeChange)}disconnectedCallback(){this.button?.removeEventListener("click",this.onClick),window.removeEventListener("storage",this.onStorage),window.removeEventListener("viking-theme-change",this.onThemeChange)}onThemeChange=()=>{this.syncIcons()};onClick=()=>{let t=ve()==="light"?"dark":"light";ri(t),this.syncIcons(),this.dispatchEvent(new CustomEvent("viking-theme-change",{bubbles:!0,composed:!0,detail:{theme:t}}))};syncIcons=()=>{let t=ve()==="dark";this.sunIcon?.classList.toggle("is-visible",t),this.moonIcon?.classList.toggle("is-visible",!t)};render(){let t=this.getAttribute("aria-label")??"Toggle light and dark theme";this.shadow.innerHTML=`
    ...
    `,p(this.shadow,ge),this.button=this.shadow.querySelector("button"),this.sunIcon=this.shadow.querySelector(".theme-icon-sun"),this.moonIcon=this.shadow.querySelector(".theme-icon-moon")}},Y=()=>{d(j.tag,j)};var he=new Set(["app","marketing","backend","docs"]),ai=`
:host {
  display: block;
  position: sticky;
  top: 0;
...
`,oi=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&he.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},si=e=>{let t=e.getAttribute("context");return t&&he.has(t)?t:oi()},li=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??y.app,marketing:e.getAttribute("marketing-url")??t.marketing??y.marketing,backend:e.getAttribute("backend-url")??t.backend??y.backend}},ci=e=>{try{let t=new URL(e,window.location.origin);return t.origin===window.location.origin&&t.pathname===window.location.pathname}catch{return!1}},X=class extends v{static tag="viking-suite-header";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","user-name","user-email","sign-in-href","dashboard-href"]}shadow;menuOpen=!1;userMenuOpen=!1;paletteEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,ai)}connectedCallback(){Y(),F(),this.render(),document.addEventListener("click",this.onDocumentClick)}disconnectedCallback(){document.removeEventListener("click",this.onDocumentClick)}attributeChangedCallback(){this.isConnected&&this.render()}openSearch(){this.paletteEl?.openPalette(),this.dispatchEvent(new CustomEvent("viking-search-open",{bubbles:!0,composed:!0}))}onDocumentClick=t=>{t.composedPath().includes(this)||(this.menuOpen||this.userMenuOpen)&&(this.menuOpen=!1,this.userMenuOpen=!1,this.syncOpenState())};onSearchClick=()=>{this.openSearch()};onMenuClick=()=>{this.menuOpen=!this.menuOpen,this.syncOpenState()};onUserClick=()=>{this.userMenuOpen=!this.userMenuOpen,this.syncOpenState()};onSignInClick=t=>{let i=new CustomEvent("viking-sign-in",{bubbles:!0,cancelable:!0,composed:!0,detail:t});this.dispatchEvent(i),i.defaultPrevented&&t.preventDefault()};onSignOutClick=()=>{this.userMenuOpen=!1,this.menuOpen=!1,this.syncOpenState(),this.dispatchEvent(new CustomEvent("viking-sign-out",{bubbles:!0,composed:!0}))};syncOpenState(){let t=this.shadow.querySelector(".suite-header"),i=this.shadow.querySelector("[data-menu-toggle]"),n=this.shadow.querySelector(".suite-header__user"),r=this.shadow.querySelector("[data-user-toggle]");t?.setAttribute("data-menu-open",String(this.menuOpen)),i?.setAttribute("aria-expanded",String(this.menuOpen)),n?.setAttribute("data-open",String(this.userMenuOpen)),r?.setAttribute("aria-expanded",String(this.userMenuOpen))}renderNavLinks(t,i,n){return te(U,n).map(r=>{let o=W(r,t,i),s=ci(o)?' aria-current="page"':"";return`
          <a class="suite-header__link" href="${a(o)}"${s}>
            ${u(r.icon,16,"suite-header__link-icon")}
            <span>${a(r.label)}</span>
          </a>
      ...
      `}if(n)return`
        <a class="suite-header__menu-link" href="${a(this.getDashboardHref(t))}">
      ...
      `;let r=this.getAttribute("user-name")??"Account",o=this.getAttribute("user-email")??"";return`
      <div class="suite-header__user" data-open="${String(this.userMenuOpen)}">
        ...
        >
          ...
          <span class="suite-header__user-text">${a(r)}</span>
        ...
        <div class="suite-header__menu" role="menu">
          ...
          <a class="suite-header__menu-link" role="menuitem" href="${a(this.getDashboardHref(t))}">
            ${u("home",16,"suite-header__menu-icon")}
            <span>Dashboard</span>
          </a>
    ...
    `}getDashboardHref(t){return this.getAttribute("dashboard-href")??`${t.app}/dashboard`}render(){let t=si(this),i=li(this),n=k(this,"authenticated"),r=this.renderNavLinks(t,i,n),o=mt(t,i),s=`${nt()}K`;this.shadow.innerHTML=`
      <header class="suite-header" data-menu-open="${String(this.menuOpen)}">
        <div class="suite-header__bar">
          <a class="suite-header__brand" href="${a(o)}" aria-label="Data Engineering for AI Engineering and Cybersecurity home">
    ...
    `,this.selectEl=this.shadow.querySelector("select");let c=this.getAttribute("value");this.selectEl&&c&&(this.selectEl.value=c),this.syncFormValue()}syncOptions(){if(!this.selectEl)return;let t=this.selectEl.value;this.selectEl.innerHTML="";let i=this.querySelectorAll("option");if(i.length===0){let r=this.getAttribute("placeholder")??"Select\u2026",o=document.createElement("option");o.value="",o.textContent=r,o.disabled=!0,o.selected=!this.getAttribute("value"),this.selectEl.append(o);return}i.forEach(r=>{this.selectEl?.append(r.cloneNode(!0))});let n=this.getAttribute("value");n?this.selectEl.value=n:t&&(this.selectEl.value=t),this.syncFormValue()}},_t=()=>{d(D.tag,D),h(D.legacyTag,D)};var St=()=>{ot(),ct(),dt(),at(),st(),lt(),_t(),gt(),q(),vt(),ht(),wt(),F(),yt(),Q(),Y()};typeof globalThis<"u"&&typeof document<"u"&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",St):St());return fe(di);})();
