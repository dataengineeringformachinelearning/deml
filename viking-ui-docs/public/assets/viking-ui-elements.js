...
`;var ce=new Set(["accent","secondary","success","warning","danger","info","muted","subtle"]),S=class extends c{static tag="viking-badge";static legacyTag="viking-badge-wc";static get observedAttributes(){return["tone","size","icon","removable"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,Ot)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"";return ce.has(t)?t:null}get size(){return this.getAttribute("size")==="sm"?"sm":null}get removable(){return this.hasAttribute("removable")&&this.getAttribute("removable")!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone;t&&this.getAttribute("tone")!==t?this.setAttribute("tone",t):!t&&this.hasAttribute("tone")&&this.removeAttribute("tone");let i=this.size;i&&this.getAttribute("size")!==i?this.setAttribute("size",i):!i&&this.hasAttribute("size")&&this.removeAttribute("size");let r=this.getAttribute("icon")??(t?J[t]:null),a=r?h(r,16):"";this.shadow.innerHTML=`
      ${a}
      <span part="label"><slot></slot></span>
      ${this.removable?`<button type="button" class="viking-badge-remove" part="remove" aria-label="Remove">${h("x",14)}</button>`:""}
    `,this.shadow.querySelector(".viking-badge-remove")?.addEventListener("click",this.onRemove)}},Q=()=>{l(S.tag,S),d(S.legacyTag,S)};var de=new Set(["outline","primary","secondary","filled","danger","ghost","subtle"]),ge=new Set(["sm","xs"]),R=class extends c{static angularTag="viking-button";static tag="viking-button-wc";static get observedAttributes(){return["variant","size","type","disabled","loading","href","target","aria-label","aria-busy","square","full-width","compact"]}shadow;control=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,Nt)}connectedCallback(){this.render(),this.control?.addEventListener("click",this.onClick)}disconnectedCallback(){this.control?.removeEventListener("click",this.onClick)}attributeChangedCallback(){this.isConnected&&this.render()}onClick=t=>{if(this.disabled||this.loading){t.preventDefault(),t.stopPropagation();return}this.dispatchEvent(new CustomEvent("viking-press",{bubbles:!0,composed:!0,detail:t}))};get variant(){let t=this.getAttribute("variant")??"outline";return de.has(t)?t:"outline"}get size(){let t=this.getAttribute("size");return t&&ge.has(t)?t:null}get disabled(){return k(this,"disabled")}get loading(){return k(this,"loading")}get square(){return k(this,"square")}render(){let t=this.getAttribute("href"),i=!!t,r=i?"a":"button",a=["viking-btn",`viking-btn-${this.variant}`,this.size?`viking-btn-${this.size}`:"",this.square?"viking-btn-square":""].filter(Boolean).join(" "),o=this.getAttribute("aria-label")??"",s=this.getAttribute("aria-busy")==="true"||this.loading?"true":null,v=n(this.getAttribute("type")??"button"),g=t?n(t):"",p=this.getAttribute("target"),f=p?n(p):"";this.shadow.innerHTML=`
      <${r}
        class="${a}"
        part="control"
        ${i?`href="${g}"`:`type="${v}"`}
    ...
    `,this.control=this.shadow.querySelector(r)}},tt=()=>{l(R.tag,R)};var ve=new Set(["accent","secondary","success","warning","danger","info","muted"]),E=class extends c{static tag="viking-callout";static legacyTag="viking-callout-wc";static get observedAttributes(){return["tone","heading","icon","dismissible","hidden"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,zt)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"info";return ve.has(t)?t:"info"}get dismissible(){return this.hasAttribute("dismissible")&&this.getAttribute("dismissible")!=="false"}onDismiss=()=>{this.setAttribute("hidden",""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};render(){let t=this.getAttribute("heading")??"",i=this.getAttribute("icon")??J[this.tone]??"info",r=h(i,22,"viking-callout-icon");this.shadow.innerHTML=`
    ...
    `,this.shadow.querySelector(".viking-callout-close")?.addEventListener("click",this.onDismiss)}},et=()=>{l(E.tag,E),d(E.legacyTag,E)};var I=class extends c{static tag="viking-card";static legacyTag="viking-card-wc";static get observedAttributes(){return["compact","interactive","title","loading"]}connectedCallback(){this.syncClasses()}attributeChangedCallback(){this.isConnected&&this.syncClasses()}syncClasses(){this.classList.add("viking-card"),this.classList.toggle("viking-card-compact",this.hasAttribute("compact")),this.classList.toggle("viking-card-interactive",this.hasAttribute("interactive")),this.classList.toggle("viking-card-loading",this.hasAttribute("loading"));let t=this.getAttribute("title");t?(this.setAttribute("role","region"),this.setAttribute("aria-label",t)):(this.removeAttribute("role"),this.removeAttribute("aria-label"))}},it=()=>{l(I.tag,I),d(I.legacyTag,I)};var A=class extends c{static tag="viking-field";static legacyTag="viking-field-wc";static get observedAttributes(){return["label","description","error","required","width"]}shadow;labelId=_("viking-field-label");descriptionId=_("viking-field-description");errorId=_("viking-field-error");slotEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,Dt)}connectedCallback(){this.render(),this.syncControlA11y()}disconnectedCallback(){this.slotEl?.removeEventListener("slotchange",this.syncControlA11y)}attributeChangedCallback(){this.isConnected&&(this.render(),this.syncControlA11y())}get control(){return(this.slotEl?.assignedElements({flatten:!0})??[]).find(i=>i instanceof HTMLElement)??null}focusControl=()=>{this.control?.focus?.()};syncControlA11y=()=>{let t=this.control;if(!t)return;let i=[this.getAttribute("description")??"",this.getAttribute("error")??""].filter(Boolean).join(" "),r=this.getAttribute("label")??"";r&&!t.hasAttribute("aria-label")&&t.setAttribute("aria-label",r),i?t.setAttribute("aria-description",i):t.removeAttribute("aria-description"),this.getAttribute("error")?(t.setAttribute("aria-invalid","true"),t.setAttribute("error",this.getAttribute("error")??"")):(t.removeAttribute("aria-invalid"),t.getAttribute("error")===""&&t.removeAttribute("error")),k(this,"required")&&t.setAttribute("required","")};render(){let t=this.getAttribute("label")??"",i=this.getAttribute("description")??"",r=this.getAttribute("error")??"",a=k(this,"required");this.shadow.innerHTML=`
      <div class="viking-field" part="field" role="group" aria-labelledby="${this.labelId}">
        ...
        ${i?`<p id="${this.descriptionId}" class="viking-field-description" part="description">${n(i)}</p>`:""}
        ${r?`<p id="${this.errorId}" class="viking-field-error" part="error" role="alert">${n(r)}</p>`:""}
      </div>
    `,this.slotEl=this.shadow.querySelector("slot"),this.slotEl?.addEventListener("slotchange",this.syncControlA11y),this.shadow.querySelector(".viking-field-label")?.addEventListener("click",this.focusControl)}},rt=()=>{l(A.tag,A),d(A.legacyTag,A)};var M=class extends c{static formAssociated=!0;static tag="viking-input";static legacyTag="viking-input-wc";static get observedAttributes(){return["type","placeholder","value","disabled","loading","clearable","name","autocomplete","required","readonly","minlength","maxlength","pattern","error","aria-label","aria-describedby","bare"]}shadow;internals;input=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.internals=Y(this),u(this.shadow,Ht)}connectedCallback(){this.render(),this.syncFormValue()}attributeChangedCallback(t){if(this.isConnected){if(t==="value"&&this.input){this.input.value=this.getAttribute("value")??"",this.syncFormValue();return}this.render()}}get value(){return this.input?.value??this.getAttribute("value")??""}set value(t){let i=t??"";this.setAttribute("value",i),this.input&&(this.input.value=i),this.syncFormValue()}get disabled(){return k(this,"disabled")}get loading(){return k(this,"loading")}get clearable(){return k(this,"clearable")}get bare(){return k(this,"bare")}onInput=()=>{let t=this.input?.value??"";this.setAttribute("value",t),this.syncFormValue(),this.dispatchEvent(new Event("input",{bubbles:!0,composed:!0})),this.dispatchEvent(new Event("change",{bubbles:!0,composed:!0}))};onBlur=()=>{this.dispatchEvent(new Event("blur",{bubbles:!0,composed:!0}))};onClear=()=>{this.value="",this.input?.focus(),this.dispatchEvent(new CustomEvent("viking-cleared",{bubbles:!0,composed:!0})),this.dispatchEvent(new Event("input",{bubbles:!0,composed:!0}))};syncFormValue(){j(this.internals,this.value)}render(){let t=["viking-input-shell",this.disabled?"viking-disabled":"",this.loading?"viking-loading":""].filter(Boolean).join(" "),i=n(this.getAttribute("type")??"text"),r=this.getAttribute("placeholder")??"",a=n(r),o=n(this.getAttribute("value")??""),s=this.getAttribute("aria-label")??(r||"Text input"),v=n(this.getAttribute("autocomplete")??""),g=n(this.getAttribute("aria-describedby")??""),p=n(this.getAttribute("minlength")??""),f=n(this.getAttribute("maxlength")??""),m=n(this.getAttribute("pattern")??""),x=this.getAttribute("error")??"",H=k(this,"required"),D=k(this,"readonly"),Jt=this.clearable&&o.length>0&&!this.loading&&!this.bare,St=`
      ${this.disabled||this.loading?"disabled":""}
      ${H?"required":""}
      ${D?"readonly":""}
      aria-label="${n(s)}"
    ...
    `;this.bare?this.shadow.innerHTML=`
      ...
      `:this.shadow.innerHTML=`
        <div class="${t}" part="shell">
          ...
          ${Jt?'<button type="button" class="viking-input-clear" aria-label="Clear input" part="clear">\xD7</button>':""}
          <slot name="trailing"></slot>
        </div>
      `,this.input=this.shadow.querySelector("input"),this.input?.addEventListener("input",this.onInput),this.input?.addEventListener("blur",this.onBlur),this.shadow.querySelector(".viking-input-clear")?.addEventListener("click",this.onClear)}},nt=()=>{l(M.tag,M),d(M.legacyTag,M)};var C=class extends c{static angularTag="viking-modal";static tag="viking-modal-wc";static dialogTag="viking-dialog";static get observedAttributes(){return["open","title","dismissible"]}shadow;dialogEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,Pt)}connectedCallback(){this.render(),this.syncOpen(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick)}attributeChangedCallback(t){if(this.isConnected){if(t==="open"){this.syncOpen();return}if(t==="title"){this.updateTitle();return}t==="dismissible"&&(this.render(),this.syncOpen())}}openModal(){this.setAttribute("open",""),this.syncOpen()}closeModal(){this.removeAttribute("open"),O(this.dialogEl)}open(){this.openModal()}close(){this.closeModal()}get dismissible(){return this.getAttribute("dismissible")!=="false"}onClose=()=>{this.removeAttribute("open"),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{this.dismissible&&t.target===this.dialogEl&&this.closeModal()};syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(X(this.dialogEl),queueMicrotask(()=>{(this.shadow.querySelector(".viking-modal-close")??this.dialogEl)?.focus()})):!t&&this.dialogEl.open&&O(this.dialogEl)}updateTitle(){let t=this.getAttribute("title")??"Dialog",i=this.shadow.querySelector(".viking-modal-heading");i&&(i.textContent=t),this.dialogEl?.setAttribute("aria-label",t)}render(){let t=this.getAttribute("title")??"Dialog";this.shadow.innerHTML=`
      <dialog class="viking-modal-backdrop" aria-label="${n(t)}" aria-modal="true">
        <div class="viking-modal-panel" part="panel" role="document">
          <header class="viking-modal-header" part="header">
            <h2 class="viking-modal-heading" part="title">${n(t)}</h2>
            ${this.dismissible?`<button type="button" class="viking-modal-close" part="close" aria-label="Close dialog">${h("x",20)}</button>`:""}
          </header>
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.shadow.querySelector(".viking-modal-close")?.addEventListener("click",()=>this.closeModal()),this.dialogEl?.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissible&&(i.preventDefault(),this.closeModal())})}},at=()=>{l(C.tag,C),d(C.dialogTag,C)};var he=e=>{let t=e.getAttribute("items");if(!t)return[];try{let i=JSON.parse(t);return Array.isArray(i)?i:[]}catch{return[]}},ue=(e,t)=>[e.title,e.snippet??"",e.group??"",e.href,...e.keywords??[]].join(" ").toLowerCase().includes(t),pe=(e,t)=>{let i=t.trim().toLowerCase();return i?e.filter(r=>ue(r,i)):e},ke=e=>{let t=new Map;return e.forEach(i=>{let r=i.group??null,a=t.get(r)??[];a.push(i),t.set(r,a)}),Array.from(t.entries()).map(([i,r])=>({group:i,items:r}))},y=class extends c{static tag="viking-command-palette";static searchTag="viking-search-palette";static legacyTag="viking-search-palette-wc";static get observedAttributes(){return["open","placeholder","items","global-shortcut"]}shadow;dialogEl=null;inputEl=null;resultsEl=null;globalKeyHandler=null;resultsId=_("viking-search-results");inputId=_("viking-search-input");query="";activeIndex=0;flatResults=[];constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,Bt)}connectedCallback(){this.render(),this.removeAttribute("open"),this.syncOpen(),this.bindGlobalShortcut(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick),this.inputEl?.addEventListener("input",this.onInput),this.inputEl?.addEventListener("keydown",this.onInputKeydown)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick),this.inputEl?.removeEventListener("input",this.onInput),this.inputEl?.removeEventListener("keydown",this.onInputKeydown),this.unbindGlobalShortcut()}attributeChangedCallback(t){if(this.isConnected&&(t==="open"&&this.syncOpen(),t==="global-shortcut"&&(this.unbindGlobalShortcut(),this.bindGlobalShortcut()),t==="items"||t==="placeholder"))if(t==="placeholder"&&this.inputEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026";this.inputEl.placeholder=i,this.inputEl.setAttribute("aria-label",i)}else this.renderResults()}openPalette(){this.setAttribute("open",""),this.syncOpen()}closePalette(){this.removeAttribute("open"),O(this.dialogEl)}search(t){this.query=t,this.inputEl&&(this.inputEl.value=t),this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))}onClose=()=>{this.removeAttribute("open"),this.query="",this.activeIndex=0,this.inputEl&&(this.inputEl.value=""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{t.target===this.dialogEl&&this.closePalette()};onInput=t=>{this.query=t.target.value??this.inputEl?.value??"",this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))};onInputKeydown=t=>{if(this.flatResults.length===0){t.key==="Escape"&&(t.preventDefault(),this.closePalette());return}if(t.key==="ArrowDown"){t.preventDefault(),this.activeIndex=Math.min(this.flatResults.length-1,this.activeIndex+1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="ArrowUp"){t.preventDefault(),this.activeIndex=Math.max(0,this.activeIndex-1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="Enter"){t.preventDefault();let i=this.flatResults[this.activeIndex];i&&this.activateItem(i)}};bindGlobalShortcut(){k(this,"global-shortcut")&&(this.globalKeyHandler=t=>{(t.metaKey||t.ctrlKey)&&t.key.toLowerCase()==="k"&&(t.preventDefault(),this.hasAttribute("open")?this.closePalette():this.openPalette())},document.addEventListener("keydown",this.globalKeyHandler))}unbindGlobalShortcut(){this.globalKeyHandler&&(document.removeEventListener("keydown",this.globalKeyHandler),this.globalKeyHandler=null)}syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(this.dialogEl.removeAttribute("aria-hidden"),X(this.dialogEl),this.activeIndex=0,this.renderResults(),queueMicrotask(()=>this.inputEl?.focus())):!t&&this.dialogEl.open?(O(this.dialogEl),this.dialogEl.setAttribute("aria-hidden","true")):t||this.dialogEl.setAttribute("aria-hidden","true")}scrollActiveIntoView(){this.resultsEl?.querySelector(".viking-search-result.is-selected")?.scrollIntoView({block:"nearest"})}activateItem(t){if(this.dispatchEvent(new CustomEvent("viking-select",{bubbles:!0,composed:!0,detail:{item:t}})),this.closePalette(),t.action==="cookie-settings"){globalThis.DemlWidgets?.openCookieSettings?.();return}if(t.action==="bug-report"){let i=globalThis.DemlWidgets;if(i?.openBugReport){i.openBugReport();return}}t.href&&t.href!=="#"&&window.location.assign(t.href)}renderResults(){if(!this.resultsEl)return;let t=he(this),i=this.query.trim().toLowerCase(),r=pe(t,i);if(this.flatResults=r,r.length===0){let g=this.query.trim();this.resultsEl.innerHTML=`<p class="viking-search-empty" role="status">${g?"No results found":"Start typing to search\u2026"}</p>`,this.inputEl?.removeAttribute("aria-activedescendant");return}this.activeIndex>=r.length&&(this.activeIndex=r.length-1);let a=0,s=ke(r).map(({group:g,items:p})=>{let f=g?`<p class="viking-search-group-label" role="presentation">${n(g)}</p>`:"",m=p.map(x=>{let H=`${this.resultsId}-result-${a}`,D=a===this.activeIndex;return a+=1,`
              <a
                id="${H}"
              ...
              >
                <div>
                  <div class="viking-search-result-title">${n(x.title)}</div>
                  ${x.snippet?`<div class="viking-search-result-snippet">${n(x.snippet)}</div>`:""}
                </div>
              </a>`}).join("");return`${f}${m}`}).join("");this.resultsEl.innerHTML=`<div class="viking-search-results" id="${this.resultsId}" role="listbox" aria-label="Search results">${s}</div>`;let v=`${this.resultsId}-result-${this.activeIndex}`;this.inputEl?.setAttribute("aria-activedescendant",v),this.inputEl?.setAttribute("role","combobox"),this.inputEl?.setAttribute("aria-expanded","true"),this.inputEl?.setAttribute("aria-controls",this.resultsId),this.resultsEl.querySelectorAll(".viking-search-result").forEach(g=>{g.addEventListener("click",p=>{p.preventDefault();let f=Number(g.dataset.index??0),m=this.flatResults[f];m&&this.activateItem(m)}),g.addEventListener("mouseenter",()=>{let p=Number(g.dataset.index??0);this.activeIndex=p,this.renderResults()})})}render(){let t=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026",i=Z();this.shadow.innerHTML=`
      <dialog class="viking-search-palette-backdrop" aria-label="Search" aria-hidden="true">
        <div class="viking-search-palette" part="panel" role="document">
          <div class="viking-search-palette-header" part="header">
            <span class="viking-search-palette-icon" aria-hidden="true">${h("search",24)}</span>
            <input
              id="${this.inputId}"
              type="search"
              ...
              spellcheck="false"
            />
            <button type="button" class="viking-search-palette-close" part="close" aria-label="Close search">${h("x",20)}</button>
          </div>
    ...
    `;this.shadow.innerHTML=`
      ...
      <${H}
        ...
        part="card"
        ${m?`href="${m}"`:""}
        ${this.target?`target="${n(this.target)}"`:""}
        ${m?g:""}
        ${x?`aria-label="${x}"`:""}
      >
        ${D}
...
`,ot=()=>{l(L.tag,L),d(L.legacyTag,L)};var fe=new Set(["accent","secondary","success","warning","danger","info","muted"]),T=class extends c{static tag="viking-status-pill";static legacyTag="viking-status-pill-wc";static get observedAttributes(){return["tone","icon","href","target","compact","dot","removable","aria-label"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,xe)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"muted";return fe.has(t)?t:"muted"}get compact(){let t=this.getAttribute("compact");return t!==null&&t!=="false"}get removable(){let t=this.getAttribute("removable");return t!==null&&t!=="false"}get href(){return this.getAttribute("href")}get icon(){return this.getAttribute("icon")}get showDot(){let t=this.getAttribute("dot");return t!==null&&t!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-pill-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone,i=this.compact?" status-pill--compact":"",r=this.icon?h(this.icon,14,"status-pill__icon"):"",a=this.showDot?'<span class="status-pill__dot" aria-hidden="true"></span>':"",o='<span class="status-pill__label" part="label"><slot></slot></span>',s=n(this.getAttribute("aria-label")??""),v=this.href,g=this.getAttribute("target"),p=v&&g==="_blank"?' rel="noopener noreferrer"':"";if(v){this.shadow.innerHTML=`
        <a
    ...
    `,this.shadow.querySelector(".status-pill__remove")?.addEventListener("click",this.onRemove)}},xe=`
:host {
  display: inline-flex;
}
...
`,st=()=>{l(T.tag,T),d(T.legacyTag,T)};var B=[{id:"explore",label:"Explore",icon:"globe",appHref:"/explore",marketingHref:"/explore",platform:!0},{id:"documentation",label:"Documentation",icon:"file",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{id:"dashboard",label:"Dashboard",icon:"home",appHref:"/dashboard",marketingHref:"/dashboard",requireAuth:!0,platform:!0},{id:"sites",label:"Sites",icon:"building",appHref:"/settings",marketingHref:"/settings",requireAuth:!0,platform:!0},{id:"account",label:"Account",icon:"user",appHref:"/account",marketingHref:"/account",requireAuth:!0,platform:!0}],lt=[{title:"Platform",links:[{label:"Explore",appHref:"/explore",marketingHref:"/explore",platform:!0},{label:"Dashboard",appHref:"/dashboard",marketingHref:"/dashboard",platform:!0},{label:"Sites",appHref:"/settings",marketingHref:"/settings",platform:!0},{label:"Account",appHref:"/account",marketingHref:"/account",platform:!0},{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0}]},{title:"Resources",links:[{label:"Documentation",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{label:"Whitepaper",appHref:"https://dataengineeringformachinelearning.com/whitepaper/",marketingHref:"/whitepaper"},{label:"Book",appHref:"https://dataengineeringformachinelearning.com/book/",marketingHref:"/book"}]},{title:"Support",links:[{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0},{label:"Report a Bug",appHref:"#",marketingHref:"#",action:"bug-report"}]},{title:"Legal & Compliance",links:[{label:"Privacy Policy",appHref:"https://dataengineeringformachinelearning.com/privacy/",marketingHref:"/privacy"},{label:"Terms of Service",appHref:"https://dataengineeringformachinelearning.com/terms/",marketingHref:"/terms"},{label:"SOC2 Compliance",appHref:"https://dataengineeringformachinelearning.com/compliance/",marketingHref:"/compliance"},{label:"GDPR Compliance",appHref:"https://dataengineeringformachinelearning.com/privacy/#gdpr",marketingHref:"/privacy#gdpr"},{label:"Cookie Settings",appHref:"#",marketingHref:"#",action:"cookie-settings"}]}],ye="reportBug=1",we="cookieSettings=1",Kt=e=>/^https?:\/\//i.test(e),ct=(e,t)=>{if(Kt(t))return t;let i=e.replace(/\/$/,""),r=t.startsWith("/")?t:`/${t}`;return`${i}${r}`},xt=e=>`${ct(e.app,"/")}?${ye}`,yt=e=>`${ct(e.marketing,"/")}?${we}`,Wt=(e,t)=>Kt(e)||e.startsWith("mailto:")||e.startsWith("/")?e:ct(t.marketing,e),Ut=(e,t,i)=>t==="app"?e:ct(i.app,e),K=(e,t,i)=>e.platform?Ut(e.appHref,t,i):t==="app"?e.appHref:Wt(e.marketingHref,i),dt=(e,t,i)=>e.action==="bug-report"?t==="app"?"#":xt(i):e.action==="cookie-settings"?yt(i):e.platform?Ut(e.appHref,t,i):t==="app"?e.appHref:Wt(e.marketingHref,i),gt=(e,t)=>e==="marketing"?"/":t.marketing,Gt=(e,t=!1)=>e.filter(i=>!i.requireAuth||t);var b={app:"https://deml.app",marketing:"https://dataengineeringformachinelearning.com",backend:"https://backend.deml.app"};var qt=new Set(["app","marketing","backend","docs"]),_e=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&qt.has(e))return e;let t=window.location.hostname;return t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},Se=e=>{let t=e.getAttribute("context");return t&&qt.has(t)?t:_e()},Ee=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??b.app,marketing:e.getAttribute("marketing-url")??t.marketing??b.marketing,backend:e.getAttribute("backend-url")??t.backend??b.backend}},Ie=e=>{let t=e.getAttribute("year"),i=t?Number(t):new Date().getFullYear();return Number.isFinite(i)?i:new Date().getFullYear()},Ae=(e,t)=>lt.map(i=>{let r=i.links.map(a=>{let o=dt(a,e,t);return`
          <li>
            <a href="${n(o)}">${n(a.label)}</a>
          </li>
        `}).join("");return`
      <div class="footer-column">
        <h3 class="footer-column-title">${n(i.title)}</h3>
        <ul class="footer-list">
          ${r}
        </ul>
      </div>
    `}).join(""),V=class extends HTMLElement{static tag="viking-site-footer";static legacyTag="viking-site-footer-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","year"]}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}render(){let t=Se(this),i=Ee(this),r=Ie(this),a=Ae(t,i);this.innerHTML=`
    ...
    `}},vt=()=>{l(V.tag,V),d(V.legacyTag,V)};var Ft=new Set(["app","marketing","backend","docs"]),Me=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&Ft.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},Ce=e=>{let t=e.getAttribute("context");return t&&Ft.has(t)?t:Me()},jt=(e,t)=>{let i=e.getAttribute(t);return i!==null&&i!=="false"},Le=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??b.app,marketing:e.getAttribute("marketing-url")??t.marketing??b.marketing,backend:e.getAttribute("backend-url")??t.backend??b.backend}},Te=e=>jt(e,"authenticated"),Ve=e=>e.getAttribute("show-search")===null?!0:jt(e,"show-search"),$e=(e,t)=>{let i=e.getAttribute("sign-in-href");if(i)return i;let r=e.getAttribute("return-url")??window.location.href;return`${t.app}/login?returnUrl=${encodeURIComponent(r)}`},$=class extends HTMLElement{static tag="viking-site-navbar";static legacyTag="viking-site-navbar-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","show-search","sign-in-href","dashboard-href","return-url"]}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}renderNavLinks(t,i,r="desktop"){let a=r==="mobile"?"mobile-nav-btn":"nav-btn";return B.map(o=>{let s=K(o,t,i),v=o.requireAuth,g=v?" hidden":"",p=v?' data-require-auth="true"':"";return`
        <a
          href="${n(s)}"
          class="${a}"
          data-nav-id="${n(o.id)}"
          ...
          ${g}
        >
          <span data-viking-icon="${n(o.icon)}" data-viking-icon-size="16" aria-hidden="true"></span>
          <span>${n(o.label)}</span>
        </a>
      ...
      <div class="desktop-auth">
        ...
        >
          <span
            ...
            data-viking-icon="${t?"home":"arrow-right"}"
    ...
    `:""}render(){let t=Ce(this),i=Le(this),r=Te(this),a=Ve(this),o=gt(t,i),s=$e(this,i),v=this.getAttribute("dashboard-href")??`${i.app}/dashboard`,g=this.renderNavLinks(t,i,"desktop"),p=this.renderNavLinks(t,i,"mobile"),f=this.renderAuthDesktop(r,s,v),m=this.renderAuthMobile(r,v,s),x=this.renderSearchButton(a);this.innerHTML=`
      <header class="navbar">
        <div class="navbar-content">
          <div class="navbar-left">
            <a href="${n(o)}" class="navbar-brand" aria-label="Go to homepage" id="navbar-brand-link">
              <span
    ...
    `}},ht=()=>{l($.tag,$),d($.legacyTag,$)};var Ne=e=>{let t=new Set;return e.filter(i=>{let r=`${i.title}:${i.href}:${i.action??""}`;return t.has(r)?!1:(t.add(r),!0)})},He=(e,t,i)=>({title:e.label,href:K(e,t,i),snippet:`Open ${e.label}`,group:"Platform",keywords:[e.id,e.label.toLowerCase(),"navigate","go"]}),De=(e,t,i,r)=>e.action==="cookie-settings"?{title:e.label,href:yt(r),snippet:"Manage analytics and cookie preferences",group:t,keywords:["cookies","consent","privacy","gdpr"],action:"cookie-settings"}:e.action==="bug-report"?{title:e.label,href:i==="app"?"#":xt(r),snippet:"Submit a product issue or regression",group:t,keywords:["bug","issue","support","feedback"],action:"bug-report"}:{title:e.label,href:dt(e,i,r),snippet:`Open ${e.label}`,group:t,keywords:[e.label.toLowerCase(),t.toLowerCase()]},Oe=[{title:"Viking-UI Components",snippet:"Browse the design system showcase",group:"Resources",keywords:["viking","ui","design system","components","showcase"]},{title:"Design tokens",snippet:"Canonical --viking-* token matrix",group:"Resources",keywords:["tokens","theme","css","variables"]},{title:"API reference",snippet:"OpenAPI schema and REST endpoints",group:"Resources",keywords:["api","openapi","swagger","rest"]}],ze=(e,t,i)=>{switch(e.title){case"Viking-UI Components":return"https://ui.dataengineeringformachinelearning.com/components";case"Design tokens":return"https://ui.dataengineeringformachinelearning.com/tokens";case"API reference":return`${i.backend.replace(/\/$/,"")}/api/docs`;default:return t==="app"?i.app:i.marketing}},Re=[{title:"Components",href:"/components",snippet:"Browse all documented primitives",group:"Viking-UI",keywords:["components","showcase","registry"]},{title:"Playground",href:"/playground",snippet:"Live Web Component sandbox",group:"Viking-UI",keywords:["playground","sandbox","demo"]},{title:"Architecture",href:"/architecture",snippet:"CSS + WC + Angular layers",group:"Viking-UI",keywords:["architecture","layers","web component"]},{title:"Design tokens",href:"/tokens",snippet:"Canonical --viking-* token matrix",group:"Viking-UI",keywords:["tokens","theme","css","variables"]},{title:"Theming",href:"/theming",snippet:"Light/dark mode and sync pipeline",group:"Viking-UI",keywords:["theming","dark","light","mode"]},{title:"Framework guides",href:"/frameworks",snippet:"Angular, Astro, Django setup",group:"Viking-UI",keywords:["frameworks","angular","astro","django"]},{title:"Contributing",href:"/contributing",snippet:"Extend the Viking-UI kit",group:"Viking-UI",keywords:["contributing","extend","primitives"]}],Pe=(e,t)=>e.startsWith("http")?e:`${t.replace(/\/$/,"")}${e.startsWith("/")?e:`/${e}`}`,wt=(e,t,i)=>{let r=e==="docs"?"marketing":e,a=[...B.map(o=>He(o,r,t)),...lt.flatMap(o=>o.links.map(s=>De(s,o.title,r,t))),...Oe.map(o=>({...o,href:ze(o,r,t)}))];if(e==="app"&&a.push({title:"Settings",href:"/settings",snippet:"Workspace domains, billing, and security",group:"Platform",keywords:["settings","sites","workspace","configuration"]},{title:"Billing & subscription",href:"/settings/billing",snippet:"Manage plan, invoices, and payment methods",group:"Platform",keywords:["billing","stripe","subscription","payment"]},{title:"Security settings",href:"/settings/security",snippet:"Keys, sessions, and access controls",group:"Platform",keywords:["security","keys","auth","rbac"]}),e==="docs"){let o=i?.docsOrigin??"https://ui.dataengineeringformachinelearning.com";a.push(...Re.map(s=>({...s,href:Pe(s.href,o)})))}return Ne(a)};var Yt=new Set(["app","marketing","backend","docs"]),Be=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&Yt.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.includes("deml.app")&&!t.startsWith("backend.")?"app":t.startsWith("backend.")?"backend":"marketing"},Ke=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??b.app,marketing:e.getAttribute("marketing-url")??t.marketing??b.marketing,backend:e.getAttribute("backend-url")??t.backend??b.backend}},We=e=>{let t=e.getAttribute("context");return t&&Yt.has(t)?t:Be()},w=class extends c{static tag="viking-suite-command-palette";static searchTag="viking-suite-search-palette";static legacyTag="viking-suite-search-palette-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","placeholder","global-shortcut"]}paletteEl=null;itemsLoaded=!1;connectedCallback(){P(),this.ensurePalette(),this.loadItems()}attributeChangedCallback(t){if(this.isConnected){if(t==="placeholder"&&this.paletteEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",i);return}if(t==="global-shortcut"&&this.paletteEl){k(this,"global-shortcut")||!this.hasAttribute("global-shortcut")?this.paletteEl.setAttribute("global-shortcut",""):this.paletteEl.removeAttribute("global-shortcut");return}(t==="context"||t==="app-url"||t==="marketing-url"||t==="backend-url")&&this.loadItems(!0)}}openPalette(){this.ensurePalette(),this.loadItems().then(()=>this.paletteEl?.openPalette())}closePalette(){this.paletteEl?.closePalette()}ensurePalette(){if(this.paletteEl)return;this.paletteEl=document.createElement("viking-command-palette"),this.paletteEl.id="deml-command-palette";let t=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",t),(k(this,"global-shortcut")||!this.hasAttribute("global-shortcut"))&&this.paletteEl.setAttribute("global-shortcut",""),this.append(this.paletteEl)}async loadItems(t=!1){if(!this.paletteEl||this.itemsLoaded&&!t)return;let i=We(this),r=Ke(this),a=wt(i,r,{docsOrigin:window.location.origin});try{let o=await fetch("/assets/site-drakkar.json",{cache:"no-cache"});if(o.ok){let s=await o.json();(s.navLinks?.length||s.footerColumns?.length)&&(a=wt(i,r,{docsOrigin:window.location.origin}))}}catch{}this.paletteEl.setAttribute("items",JSON.stringify(a)),this.itemsLoaded=!0}},W=()=>{l(w.tag,w),d(w.searchTag,w),d(w.legacyTag,w)};var Ue=`
:host {
  display: inline-flex;
}
...
`,Ge=e=>{document.documentElement.setAttribute("data-theme",e),document.documentElement.classList.toggle("dark",e==="dark"),localStorage.setItem("theme",e)},Xt=()=>{let e=localStorage.getItem("theme");return e==="light"||e==="dark"?e:window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"},U=class extends c{static tag="viking-theme-toggle-wc";shadow;button=null;sunIcon=null;moonIcon=null;onStorage=()=>{this.syncIcons()};constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,Ue)}connectedCallback(){this.render(),this.syncIcons(),this.button?.addEventListener("click",this.onClick),window.addEventListener("storage",this.onStorage),window.addEventListener("viking-theme-change",this.onThemeChange)}disconnectedCallback(){this.button?.removeEventListener("click",this.onClick),window.removeEventListener("storage",this.onStorage),window.removeEventListener("viking-theme-change",this.onThemeChange)}onThemeChange=()=>{this.syncIcons()};onClick=()=>{let t=Xt()==="light"?"dark":"light";Ge(t),this.syncIcons(),this.dispatchEvent(new CustomEvent("viking-theme-change",{bubbles:!0,composed:!0,detail:{theme:t}}))};syncIcons=()=>{let t=Xt()==="dark";this.sunIcon?.classList.toggle("is-visible",t),this.moonIcon?.classList.toggle("is-visible",!t)};render(){let t=this.getAttribute("aria-label")??"Toggle light and dark theme";this.shadow.innerHTML=`
    ...
    `,this.button=this.shadow.querySelector("button"),this.sunIcon=this.shadow.querySelector(".theme-icon-sun"),this.moonIcon=this.shadow.querySelector(".theme-icon-moon")}},G=()=>{l(U.tag,U)};var Zt=new Set(["app","marketing","backend","docs"]),qe=`
:host {
  display: block;
  position: sticky;
  top: 0;
...
`,Fe=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&Zt.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},je=e=>{let t=e.getAttribute("context");return t&&Zt.has(t)?t:Fe()},Ye=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??b.app,marketing:e.getAttribute("marketing-url")??t.marketing??b.marketing,backend:e.getAttribute("backend-url")??t.backend??b.backend}},Xe=e=>{try{let t=new URL(e,window.location.origin);return t.origin===window.location.origin&&t.pathname===window.location.pathname}catch{return!1}},q=class extends c{static tag="viking-suite-header";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","user-name","user-email","sign-in-href","dashboard-href"]}shadow;menuOpen=!1;userMenuOpen=!1;paletteEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),u(this.shadow,qe)}connectedCallback(){G(),W(),this.render(),document.addEventListener("click",this.onDocumentClick)}disconnectedCallback(){document.removeEventListener("click",this.onDocumentClick)}attributeChangedCallback(){this.isConnected&&this.render()}openSearch(){this.paletteEl?.openPalette(),this.dispatchEvent(new CustomEvent("viking-search-open",{bubbles:!0,composed:!0}))}onDocumentClick=t=>{t.composedPath().includes(this)||(this.menuOpen||this.userMenuOpen)&&(this.menuOpen=!1,this.userMenuOpen=!1,this.syncOpenState())};onSearchClick=()=>{this.openSearch()};onMenuClick=()=>{this.menuOpen=!this.menuOpen,this.syncOpenState()};onUserClick=()=>{this.userMenuOpen=!this.userMenuOpen,this.syncOpenState()};onSignInClick=t=>{let i=new CustomEvent("viking-sign-in",{bubbles:!0,cancelable:!0,composed:!0,detail:t});this.dispatchEvent(i),i.defaultPrevented&&t.preventDefault()};onSignOutClick=()=>{this.userMenuOpen=!1,this.menuOpen=!1,this.syncOpenState(),this.dispatchEvent(new CustomEvent("viking-sign-out",{bubbles:!0,composed:!0}))};syncOpenState(){let t=this.shadow.querySelector(".suite-header"),i=this.shadow.querySelector("[data-menu-toggle]"),r=this.shadow.querySelector(".suite-header__user"),a=this.shadow.querySelector("[data-user-toggle]");t?.setAttribute("data-menu-open",String(this.menuOpen)),i?.setAttribute("aria-expanded",String(this.menuOpen)),r?.setAttribute("data-open",String(this.userMenuOpen)),a?.setAttribute("aria-expanded",String(this.userMenuOpen))}renderNavLinks(t,i,r){return Gt(B,r).map(a=>{let o=K(a,t,i),s=Xe(o)?' aria-current="page"':"";return`
      ...
      <div class="suite-header__user" data-open="${String(this.userMenuOpen)}">
        ...
        >
          <span class="suite-header__avatar" aria-hidden="true">
            ${h("user",16,"suite-header__avatar-icon")}
          </span>
          <span class="suite-header__user-text">${n(a)}</span>
          ${h("chevron-down",16,"suite-header__chevron")}
        </button>
        <div class="suite-header__menu" role="menu">
          ...
          <a class="suite-header__menu-link" role="menuitem" href="${n(this.getDashboardHref(t))}">
            ${h("home",16,"suite-header__menu-icon")}
            <span>Dashboard</span>
          </a>
    ...
    `}getDashboardHref(t){return this.getAttribute("dashboard-href")??`${t.app}/dashboard`}render(){let t=je(this),i=Ye(this),r=k(this,"authenticated"),a=this.renderNavLinks(t,i,r),o=gt(t,i),s=`${Z()}K`;this.shadow.innerHTML=`
      <header class="suite-header" data-menu-open="${String(this.menuOpen)}">
        <div class="suite-header__bar">
          <a class="suite-header__brand" href="${n(o)}" aria-label="Data Engineering for Machine Learning home">
            <span class="suite-header__mark" aria-hidden="true">
              ${h("drakkar",28,"suite-header__brand-icon")}
            </span>
          ...
          <div class="suite-header__actions">
            <button
              ...
              data-search-trigger
            >
              ${h("search",18,"suite-header__action-icon")}
            </button>
            ${this.renderAuth(i,r)}
            ...
            <button
              ...
              data-menu-toggle
            >
              ${h(this.menuOpen?"x":"menu",18,"suite-header__action-icon")}
            </button>
          </div>
        </div>
      ...
      <viking-suite-command-palette
        context="${t}"
        app-url="${n(i.app)}"
        marketing-url="${n(i.marketing)}"
        backend-url="${n(i.backend)}"
        global-shortcut
      ></viking-suite-command-palette>
    `,this.paletteEl=this.shadow.querySelector("viking-suite-command-palette"),this.shadow.querySelector("[data-search-trigger]")?.addEventListener("click",this.onSearchClick),this.shadow.querySelector("[data-menu-toggle]")?.addEventListener("click",this.onMenuClick),this.shadow.querySelector("[data-user-toggle]")?.addEventListener("click",this.onUserClick),this.shadow.querySelectorAll("[data-sign-in]").forEach(v=>v.addEventListener("click",this.onSignInClick)),this.shadow.querySelectorAll("[data-sign-out]").forEach(v=>v.addEventListener("click",this.onSignOutClick))}},ut=()=>{l(q.tag,q)};var N=class extends c{static formAssociated=!0;static tag="viking-select";static legacyTag="viking-select-wc";static get observedAttributes(){return["label","name","value","placeholder","description","error","width","disabled","required"]}shadow;internals;selectEl=null;optionObserver=null;controlId=_("viking-select");constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.internals=Y(this),u(this.shadow,Rt)}connectedCallback(){this.render(),this.syncOptions(),this.observeOptions(),this.selectEl?.addEventListener("change",this.onChange)}disconnectedCallback(){this.selectEl?.removeEventListener("change",this.onChange),this.optionObserver?.disconnect(),this.optionObserver=null}attributeChangedCallback(t){if(this.isConnected){if(t==="value"&&this.selectEl){this.selectEl.value=this.getAttribute("value")??"",this.syncFormValue();return}if(t==="error"||t==="description"||t==="label"){this.render(),this.syncOptions();return}this.render(),this.syncOptions()}}get value(){return this.selectEl?.value??this.getAttribute("value")??""}set value(t){this.setAttribute("value",t),this.selectEl&&(this.selectEl.value=t),this.syncFormValue()}onChange=()=>{let t=this.selectEl?.value??"";this.setAttribute("value",t),this.syncFormValue(),this.dispatchEvent(new CustomEvent("viking-change",{bubbles:!0,composed:!0,detail:{value:t}}))};syncFormValue(){j(this.internals,this.value)}observeOptions(){this.optionObserver?.disconnect(),this.optionObserver=new MutationObserver(()=>this.syncOptions()),this.optionObserver.observe(this,{childList:!0,subtree:!0,characterData:!0})}render(){let t=this.getAttribute("label")??"",i=this.getAttribute("name")??"",r=this.hasAttribute("disabled"),a=this.hasAttribute("required"),o=this.getAttribute("error")??"",s=this.getAttribute("description")??"",v=[s&&`${this.controlId}-desc`,o&&`${this.controlId}-error`].filter(Boolean).join(" ");this.shadow.innerHTML=`
      <div class="viking-field" part="field">
        ${t?`<label class="viking-field-label" part="label" for="${this.controlId}">${n(t)}</label>`:""}
        <select
          id="${this.controlId}"
