import tokensJson from '../../../frontend/projects/viking-ui/src/tokens/viking-tokens.json';
...
const flattenColors = (): TokenGroup => {
  const color = tokensJson.color as Record<string, Record<string, string> | string>;
  const entries: TokenGroup['tokens'] = [];

  for (const [group, values] of Object.entries(color)) {
    if (typeof values === 'string') {
      entries.push({ name: `--viking-${group}`, value: values, category: group });
      continue;
    }
...
const flattenTypography = (): TokenGroup => {
  const { fontSize, fontWeight, lineHeight, letterSpacing } = tokensJson.typography;
  const tokens: TokenGroup['tokens'] = [];

  for (const [key, value] of Object.entries(fontSize)) {
    tokens.push({ name: `--viking-font-size-${key}`, value, category: 'fontSize' });
  }
  for (const [key, value] of Object.entries(fontWeight)) {
    tokens.push({ name: `--viking-font-weight-${key}`, value: String(value), category: 'fontWeight' });
  }
  for (const [key, value] of Object.entries(lineHeight)) {
    tokens.push({ name: `--viking-line-height-${key}`, value: String(value), category: 'lineHeight' });
  }
  for (const [key, value] of Object.entries(letterSpacing)) {
    tokens.push({ name: `--viking-letter-spacing-${key}`, value, category: 'letterSpacing' });
  }
