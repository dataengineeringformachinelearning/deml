...
import { getDefaultDisclosureStore } from "../../core/disclosure";
import {
...
} from "../../core/suite-data-pack";
import type { SuiteThemePreference } from "../../core/theme";
import { clearRecentSearches } from "../../web/search-palette/recent-searches";
import { VikingToastService } from "../toast/toast";
import { VikingThemeService } from "../theme/theme.service";

/**
 * Suite preferences — persist + cross-tab sync; open via ⌘, (FORJD ADR-0024).
 * Export / import soft chrome via suite data pack (FORJD ADR-0026).
 * Important actions → suite activity log (FORJD ADR-0027).
 */
@Injectable({ providedIn: "root" })
export class VikingPreferencesService {
  private readonly platformId = inject(PLATFORM_ID);
  private readonly destroyRef = inject(DestroyRef);
  private readonly theme = inject(VikingThemeService);
  private readonly toast = inject(VikingToastService, { optional: true });
  ...
  private readonly activitySignal = signal<readonly SuiteActivityEntry[]>(
    this.activity.list(),
  );
  ...
  constructor() {
    ...
    getDefaultShortcutRegistry().register({
      ...
      group: "Navigation",
      description: "Appearance and local UI preferences (FORJD ADR-0024).",
    });
  ...
  /** Download soft UI chrome as a suite data pack JSON file (FORJD ADR-0026). */
  exportDataPack(options?: { readonly includeRecentSearches?: boolean }): void {
    ...
    recordSuiteActivity({
      ...
      detail: options?.includeRecentSearches
        ? "Included search history"
        : undefined,
      source: "deml",
    });
    this.toast?.success("Soft UI chrome only — no tokens or secrets.", {
      heading: "Exported local preferences",
    });
  }

  /** Import a suite data pack file (merge or replace). */
  async importDataPack(
    file: File,
    ...
    if (!parsed.ok) {
      this.toast?.critical(parsed.error, { heading: "Import failed" });
      return false;
    }
    ...
    this.snapshotSignal.set(this.store.get());
    const label = result.applied.length
      ? result.applied.join(", ")
      : "nothing to apply";
    recordSuiteActivity({
    ...
    this.toast?.success(label, {
      heading: mode === "replace" ? "Replaced local data" : "Merged local data",
    });
    return true;
  }
