/**
 * Progressive disclosure store — smart defaults + remembered expand state
 * (FORJD ADR-0022). Dual-adapter: keep API aligned with forjd-ui core/a11y/disclosure.
 *
...
export type DisclosureStore = {
  ...
  /** Remembered expand map (suite data pack — FORJD ADR-0026). */
  readonly snapshot: () => Readonly<Record<string, boolean>>;
  /** Merge or replace remembered expand state (suite data pack — FORJD ADR-0026). */
  readonly importMap: (
...
export function createDisclosureStore(
  options?: CreateDisclosureStoreOptions,
): DisclosureStore {
  ...
  return {
    ...
    importMap: (map, mode = "merge") => {
      ...
      remembered =
        mode === "replace" ? incoming : { ...remembered, ...incoming };
      writeMap(storage, storageKey, remembered);
      notify();
    },
    subscribe: (listener) => {
