/**
 * Priority-aware toast queue — shared shape for VikingToastService / FjToastService.
 * Non-intrusive: capped visible stack, importance ordering, dedupe, sticky critical.
 * Dual-adapter: keep API aligned with forjd-ui core/a11y/toast-store (FORJD ADR-0020).
 */
...
export function toastPriorityFromTone(
...
export function defaultToastDurationMs(
  ...
  return (
    overrides?.[priority] ?? DEFAULT_DURATION_BY_PRIORITY[priority] ?? 4000
  );
}
...
function sortByImportance<T extends ToastStoreItem>(list: readonly T[]): T[] {
  return [...list].sort((a, b) => {
    const byPriority =
      toastPriorityRank(b.priority) - toastPriorityRank(a.priority);
    if (byPriority !== 0) {
      return byPriority;
    }
...
export function createToastStore<TMessage extends ToastStoreItem>(options?: {
  ...
  const fallbackDuration = options?.defaultDurationMs ?? 4500;
  let seq = 0;
  ...
  return {
    ...
    pause: (id) => {
      const targets =
        id == null ? [...timers.keys()] : timers.has(id) ? [id] : [];
