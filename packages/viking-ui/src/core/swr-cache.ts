 ...
 * Dual-adapter: keep API aligned with FORJD `app/core/fetch/swr-cache`
 * (FORJD ADR-0012). One cache instance per resource — compose; no mega keyed store.
 */
...
export function createSwrCache<T>(
  options: CreateSwrCacheOptions<T>,
): SwrCache<T> {
