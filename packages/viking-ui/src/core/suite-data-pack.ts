/**
 * Suite data pack — export / import soft UI chrome (FORJD ADR-0026).
 * Dual-adapter: keep API aligned with forjd-ui core/a11y/suite-data-pack.
 *
...
export type SuiteDataPackV1 = {
  ...
  readonly recentSearches?: readonly VikingRecentSearch[];
};
...
function sanitizeRecent(rows: unknown): VikingRecentSearch[] {
  ...
  for (const row of rows) {
    ...
    const query = String((row as VikingRecentSearch).query ?? "")
      .trim()
      .replace(/\s+/g, " ")
      .slice(0, 120);
    if (!query) {
    ...
    out.push({
      ...
      at: Number((row as VikingRecentSearch).at) || Date.now(),
    });
...
export function exportSuiteDataPack(
  options?: ExportSuiteDataPackOptions,
): SuiteDataPackV1 {
  ...
  if (options?.includeRecentSearches) {
    return {
      ...
      recentSearches: readRecentSearches(
        options.recentStorageKey ?? DEFAULT_RECENT_STORAGE_KEY,
      ),
    };
  }
  return pack;
...
export function parseSuiteDataPack(raw: unknown): ParseSuiteDataPackResult {
  ...
  const pack: SuiteDataPackV1 = {
    ...
    recentSearches: row["recentSearches"]
      ? sanitizeRecent(row["recentSearches"])
      : undefined,
  };
  return { ok: true, pack };
}
...
export async function readSuiteDataPackFile(
  file: File,
): Promise<ParseSuiteDataPackResult> {
