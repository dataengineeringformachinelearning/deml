/**
 * Suite preferences store — persist + cross-tab sync (FORJD ADR-0024).
 * Dual-adapter: keep API aligned with forjd-ui core/a11y/preferences.
 *
...
function sanitize(
  raw: unknown,
  fallbackTheme: SuiteThemePreference,
): SuitePreferences {
  const base = defaults();
...
function readBlob(
  ...
  try {
    ...
    return sanitize(
      ...
      readSuiteThemePreference(storage),
    );
  } catch {
    return null;
  }
...
export function createPreferencesStore(
  ...
  return {
    ...
    reset: () =>
      applyLocal({ theme: "system", updatedAt: Date.now() }, "local"),
    subscribe: (listener) => {
