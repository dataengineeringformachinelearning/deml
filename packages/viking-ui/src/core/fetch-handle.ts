 ...
 * Dual-adapter: keep API aligned with FORJD `app/core/fetch/fetch-handle`
 * (FORJD ADR-0011). Presentation binds isLoading / isError / isSuccess;
 * domain code owns fetchers — viking-ui does not call backends.
 */
...
export type FetchHandle<TData, TError = string> = {
   ...
   * Use for continuity probes that return ok | soft-failure.
   */
  runSettled(
  ...
  /** Return to idle and drop data/error. */
  reset(): void;
  /**
 ...
 * Flat signals only — no nested `{ status, data, error }` VM object.
 */
export function createFetchHandle<TData, TError = string>(
  ...
  async function run(
    fetcher: (signal: AbortSignal) => Promise<TData>,
    mapError: (cause: unknown) => TError = (cause) =>
      fetchErrorMessage(cause) as TError,
  ): Promise<TData | null> {
