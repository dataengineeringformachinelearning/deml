/**
 * Suite activity log — important soft-chrome actions (FORJD ADR-0027).
 * Dual-adapter: keep API aligned with forjd-ui core/a11y/activity-log.
 *
...
const SENSITIVE =
  /fjsvc_|Bearer\s|eyJ[A-Za-z0-9_-]{20,}|sk-[A-Za-z0-9]{10,}|api[_-]?key\s*[:=]/i;
...
function sanitizeEntry(raw: unknown): SuiteActivityEntry | null {
  ...
  const detail =
    typeof detailRaw === "string"
      ? (scrubText(detailRaw, DETAIL_MAX) ?? undefined)
      : undefined;
  ...
  return {
    ...
    record: (input) => {
      ...
      const detail = input.detail
        ? (scrubText(input.detail, DETAIL_MAX) ?? undefined)
        : undefined;
...
export function recordSuiteActivity(
  input: ActivityLogRecordInput,
): SuiteActivityEntry | null {
