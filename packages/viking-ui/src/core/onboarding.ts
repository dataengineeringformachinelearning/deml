/**
 * Suite onboarding progress — first-time guidance (FORJD ADR-0025).
 * Dual-adapter: keep API aligned with forjd-ui core/a11y/onboarding.
 *
 * Journey state only (dismiss / complete / step ids). Never secrets or tokens.
 * Separate from suite-preferences-v1 (FORJD ADR-0024 soft chrome).
 */
...
export type OnboardingStore = {
  ...
  readonly reset: () => void;
  /** Merge or replace journey state (suite data pack — FORJD ADR-0026). */
  ...
  try {
    ...
    return {
      ...
      completedSteps: complete
        ? ["welcome", "site", "endpoint", "publish", "done"]
        : [],
...
export function createOnboardingStore(
  ...
  return {
    ...
    bindSync: (bindOpts) => {
      const win =
        bindOpts?.target ?? (typeof window !== "undefined" ? window : null);
      if (!win) {
        return () => undefined;
      }
