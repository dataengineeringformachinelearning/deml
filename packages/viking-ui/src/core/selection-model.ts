/**
 * Multi-select model for list/table bulk actions (FORJD ADR-0021).
 * Dual-adapter: keep API aligned with forjd-ui core/a11y/selection-model.
 *
...
export function createSelectionModel(
  ...
  const selected = new Set<string>(
    (options?.initial ?? []).map(String).filter(Boolean),
  );
