...
| Docs & Book | Synced content + Viking chrome | THEME.md tokens in prose and components |
...
Machined aluminum feel — borders, axis lines, muted chrome.
...
| `--viking-metallic-100` | `#BBBBBB` | `rgb(187, 187, 187)` | Disabled chrome |
...
| `--ui-font-size` | `14px` chrome floor |
...
| Token | Stack | Usage |
|-------|-------|-------|
| `--viking-font-family` | `'Inter', 'Segoe UI', system-ui, -apple-system, sans-serif` | Body, UI chrome, tables, forms |
...
- **14px minimum** for UI chrome (`--viking-font-size-sm` / `--ui-font-size`).
...
```html
<html data-theme="dark">   <!-- default; dark-first -->
...
<div style="background: var(--viking-bg); color: var(--viking-text);">
  …
</div>
```
