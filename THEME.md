...
- `--text-muted` maps to:
  - *(Dark Mode)* `--color-primary-container`
  - *(Light Mode)* `#4a5450` (ensures a compliant contrast ratio of 5.8:1 against white surfaces)
- `--card-bg` maps to `--color-surface`
- `--border` maps to:
  - *(Dark Mode)* `--color-surface`
  - *(Light Mode)* `#C8D3CE` (a crisp, structural light sage border token)
