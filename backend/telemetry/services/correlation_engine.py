"""Built-in correlation rules and playbook trigger evaluation."""
...
def evaluate_playbook_trigger(playbook: Playbook, context: dict[str, Any]) -> bool:
  """Evaluate playbook.trigger_conditions against context. Empty conditions = never auto-fire."""
  conditions = playbook.trigger_conditions or {}
  if not conditions or not playbook.is_active:
    return False
  ...
  if min_anomaly is not None:
    if float(context.get("anomaly_score") or 0) < float(min_anomaly):
      return False
