...
`;var we=new Set(["accent","secondary","success","warning","danger","info","muted","subtle"]),A=class extends v{static tag="viking-badge";static legacyTag="viking-badge-wc";static get observedAttributes(){return["tone","size","icon","removable"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Ut)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"";return we.has(t)?t:null}get size(){return this.getAttribute("size")==="sm"?"sm":null}get removable(){return this.hasAttribute("removable")&&this.getAttribute("removable")!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone;t&&this.getAttribute("tone")!==t?this.setAttribute("tone",t):!t&&this.hasAttribute("tone")&&this.removeAttribute("tone");let i=this.size;i&&this.getAttribute("size")!==i?this.setAttribute("size",i):!i&&this.hasAttribute("size")&&this.removeAttribute("size");let r=this.getAttribute("icon")??(t?nt[t]:null),a=r?u(r,16):"";this.shadow.innerHTML=`
      ${a}
      <span part="label"><slot></slot></span>
      ${this.removable?`<button type="button" class="viking-badge-remove" part="remove" aria-label="Remove">${u("x",14)}</button>`:""}
    `,this.shadow.querySelector(".viking-badge-remove")?.addEventListener("click",this.onRemove)}},at=()=>{d(A.tag,A),h(A.legacyTag,A)};var _e=new Set(["outline","primary","secondary","filled","danger","ghost","subtle"]),Se=new Set(["sm","xs"]),K=class extends v{static angularTag="viking-button";static tag="viking-button-wc";static get observedAttributes(){return["variant","size","type","disabled","loading","href","target","aria-label","aria-busy","square","full-width","compact"]}shadow;control=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Bt)}connectedCallback(){this.render(),this.addEventListener("click",this.onClick)}disconnectedCallback(){this.removeEventListener("click",this.onClick)}attributeChangedCallback(){this.isConnected&&this.render()}onClick=t=>{if(this.disabled||this.loading){t.preventDefault(),t.stopPropagation();return}this.dispatchEvent(new CustomEvent("viking-press",{bubbles:!0,composed:!0,detail:t}))};get variant(){let t=this.getAttribute("variant")??"outline";return _e.has(t)?t:"outline"}get size(){let t=this.getAttribute("size");return t&&Se.has(t)?t:null}get disabled(){return k(this,"disabled")}get loading(){return k(this,"loading")}get square(){return k(this,"square")}render(){let t=this.getAttribute("href"),i=!!t,r=i?"a":"button",a=["viking-btn",`viking-btn-${this.variant}`,this.size?`viking-btn-${this.size}`:"",this.square?"viking-btn-square":""].filter(Boolean).join(" "),n=this.getAttribute("aria-label")??"",l=this.getAttribute("aria-busy")==="true"||this.loading?"true":null,s=o(this.getAttribute("type")??"button"),c=t?o(t):"",g=this.getAttribute("target"),m=g?o(g):"";this.hasAttribute("type")&&!i&&this.removeAttribute("type"),this.shadow.innerHTML=`
      <${r}
        class="${a}"
        part="control"
        ${i?`href="${c}"`:`type="${s}"`}
    ...
    `,this.control=this.shadow.querySelector(r)}},ot=()=>{d(K.tag,K)};var Ee=new Set(["accent","secondary","success","warning","danger","info","muted"]),M=class extends v{static tag="viking-callout";static legacyTag="viking-callout-wc";static get observedAttributes(){return["tone","heading","icon","dismissible","hidden"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Gt)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"info";return Ee.has(t)?t:"info"}get dismissible(){return this.hasAttribute("dismissible")&&this.getAttribute("dismissible")!=="false"}onDismiss=()=>{this.setAttribute("hidden",""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};render(){let t=this.getAttribute("heading")??"",i=this.getAttribute("icon")??nt[this.tone]??"info",r=u(i,22,"viking-callout-icon");this.shadow.innerHTML=`
    ...
    `;this.bare?this.shadow.innerHTML=`
      ...
      `:this.shadow.innerHTML=`
        <div class="${t}" part="shell">
          ...
          ${Et?'<button type="button" class="viking-input-clear" aria-label="Clear input" part="clear">\xD7</button>':""}
          <slot name="trailing"></slot>
        </div>
      `,this.input=this.shadow.querySelector("input"),this.input?.addEventListener("input",this.onInput),this.input?.addEventListener("blur",this.onBlur),this.shadow.querySelector(".viking-input-clear")?.addEventListener("click",this.onClear)}},dt=()=>{d(L.tag,L),h(L.legacyTag,L)};var V=class extends v{static angularTag="viking-modal";static tag="viking-modal-wc";static dialogTag="viking-dialog";static get observedAttributes(){return["open","title","dismissible"]}shadow;dialogEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,jt)}connectedCallback(){this.render(),this.syncOpen(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick)}attributeChangedCallback(t){if(this.isConnected){if(t==="open"){this.syncOpen();return}if(t==="title"){this.updateTitle();return}t==="dismissible"&&(this.render(),this.syncOpen())}}openModal(){this.setAttribute("open",""),this.syncOpen()}closeModal(){this.removeAttribute("open"),P(this.dialogEl)}open(){this.openModal()}close(){this.closeModal()}get dismissible(){return this.getAttribute("dismissible")!=="false"}onClose=()=>{this.removeAttribute("open"),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{this.dismissible&&t.target===this.dialogEl&&this.closeModal()};syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(it(this.dialogEl),queueMicrotask(()=>{(this.shadow.querySelector(".viking-modal-close")??this.dialogEl)?.focus()})):!t&&this.dialogEl.open&&P(this.dialogEl)}updateTitle(){let t=this.getAttribute("title")??"Dialog",i=this.shadow.querySelector(".viking-modal-heading");i&&(i.textContent=t),this.dialogEl?.setAttribute("aria-label",t)}render(){let t=this.getAttribute("title")??"Dialog";this.shadow.innerHTML=`
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.shadow.querySelector(".viking-modal-close")?.addEventListener("click",()=>this.closeModal()),this.dialogEl?.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissible&&(i.preventDefault(),this.closeModal())})}},gt=()=>{d(V.tag,V),h(V.dialogTag,V)};var Ie=e=>{let t=e.getAttribute("items");if(!t)return[];try{let i=JSON.parse(t);return Array.isArray(i)?i:[]}catch{return[]}},Ae=(e,t)=>[e.title,e.snippet??"",e.group??"",e.href,...e.keywords??[]].join(" ").toLowerCase().includes(t),Me=(e,t)=>{let i=t.trim().toLowerCase();return i?e.filter(r=>Ae(r,i)):e},Ce=e=>{let t=new Map;return e.forEach(i=>{let r=i.group??null,a=t.get(r)??[];a.push(i),t.set(r,a)}),Array.from(t.entries()).map(([i,r])=>({group:i,items:r}))},_=class extends v{static tag="viking-command-palette";static searchTag="viking-search-palette";static legacyTag="viking-search-palette-wc";static get observedAttributes(){return["open","placeholder","items","global-shortcut"]}shadow;dialogEl=null;inputEl=null;resultsEl=null;globalKeyHandler=null;resultsId=E("viking-search-results");inputId=E("viking-search-input");query="";activeIndex=0;flatResults=[];constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,Yt)}connectedCallback(){this.render(),this.removeAttribute("open"),this.syncOpen(),this.bindGlobalShortcut(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick),this.inputEl?.addEventListener("input",this.onInput),this.inputEl?.addEventListener("keydown",this.onInputKeydown)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick),this.inputEl?.removeEventListener("input",this.onInput),this.inputEl?.removeEventListener("keydown",this.onInputKeydown),this.unbindGlobalShortcut()}attributeChangedCallback(t){if(this.isConnected&&(t==="open"&&this.syncOpen(),t==="global-shortcut"&&(this.unbindGlobalShortcut(),this.bindGlobalShortcut()),t==="items"||t==="placeholder"))if(t==="placeholder"&&this.inputEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026";this.inputEl.placeholder=i,this.inputEl.setAttribute("aria-label",i)}else this.renderResults()}openPalette(){this.setAttribute("open",""),this.syncOpen()}closePalette(){this.removeAttribute("open"),P(this.dialogEl)}search(t){this.query=t,this.inputEl&&(this.inputEl.value=t),this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))}onClose=()=>{this.removeAttribute("open"),this.query="",this.activeIndex=0,this.inputEl&&(this.inputEl.value=""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{t.target===this.dialogEl&&this.closePalette()};onInput=t=>{this.query=t.target.value??this.inputEl?.value??"",this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))};onInputKeydown=t=>{if(this.flatResults.length===0){t.key==="Escape"&&(t.preventDefault(),this.closePalette());return}if(t.key==="ArrowDown"){t.preventDefault(),this.activeIndex=Math.min(this.flatResults.length-1,this.activeIndex+1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="ArrowUp"){t.preventDefault(),this.activeIndex=Math.max(0,this.activeIndex-1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="Enter"){t.preventDefault();let i=this.flatResults[this.activeIndex];i&&this.activateItem(i)}};bindGlobalShortcut(){k(this,"global-shortcut")&&(this.globalKeyHandler=t=>{(t.metaKey||t.ctrlKey)&&t.key.toLowerCase()==="k"&&(t.preventDefault(),this.hasAttribute("open")?this.closePalette():this.openPalette())},document.addEventListener("keydown",this.globalKeyHandler))}unbindGlobalShortcut(){this.globalKeyHandler&&(document.removeEventListener("keydown",this.globalKeyHandler),this.globalKeyHandler=null)}syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(this.dialogEl.removeAttribute("aria-hidden"),it(this.dialogEl),this.activeIndex=0,this.renderResults(),queueMicrotask(()=>this.inputEl?.focus())):!t&&this.dialogEl.open?(P(this.dialogEl),this.dialogEl.setAttribute("aria-hidden","true")):t||this.dialogEl.setAttribute("aria-hidden","true")}scrollActiveIntoView(){this.resultsEl?.querySelector(".viking-search-result.is-selected")?.scrollIntoView({block:"nearest"})}activateItem(t){if(this.dispatchEvent(new CustomEvent("viking-select",{bubbles:!0,composed:!0,detail:{item:t}})),this.closePalette(),t.action==="cookie-settings"){globalThis.DemlWidgets?.openCookieSettings?.();return}if(t.action==="bug-report"){let i=globalThis.DemlWidgets;if(i?.openBugReport){i.openBugReport();return}}if(t.href&&t.href!=="#")try{let i=new URL(t.href,window.location.href);i.origin===window.location.origin?window.location.assign(`${i.pathname}${i.search}${i.hash}`):window.location.assign(i.href)}catch{window.location.assign(t.href)}}renderResults(){if(!this.resultsEl)return;let t=Ie(this),i=this.query.trim().toLowerCase(),r=Me(t,i);if(this.flatResults=r,r.length===0){let c=this.query.trim();this.resultsEl.innerHTML=`<p class="viking-search-empty" role="status">${c?"No results found":"Start typing to search\u2026"}</p>`,this.inputEl?.removeAttribute("aria-activedescendant");return}this.activeIndex>=r.length&&(this.activeIndex=r.length-1);let a=0,l=Ce(r).map(({group:c,items:g})=>{let m=c?`<p class="viking-search-group-label" role="presentation">${o(c)}</p>`:"",b=g.map(f=>{let x=`${this.resultsId}-result-${a}`,I=a===this.activeIndex;return a+=1,`
              <a
                id="${x}"
              ...
              >
                <div>
                  <div class="viking-search-result-title">${o(f.title)}</div>
                  ${f.snippet?`<div class="viking-search-result-snippet">${o(f.snippet)}</div>`:""}
                </div>
              </a>`}).join("");return`${m}${b}`}).join("");this.resultsEl.innerHTML=`<div class="viking-search-results" id="${this.resultsId}" role="listbox" aria-label="Search results">${l}</div>`;let s=`${this.resultsId}-result-${this.activeIndex}`;this.inputEl?.setAttribute("aria-activedescendant",s),this.inputEl?.setAttribute("role","combobox"),this.inputEl?.setAttribute("aria-expanded","true"),this.inputEl?.setAttribute("aria-controls",this.resultsId),this.resultsEl.querySelectorAll(".viking-search-result").forEach(c=>{c.addEventListener("click",g=>{let m=g;if(m.metaKey||m.ctrlKey||m.shiftKey||m.button===1){this.closePalette();return}g.preventDefault();let b=Number(c.dataset.index??0),f=this.flatResults[b];f&&this.activateItem(f)}),c.addEventListener("mouseenter",()=>{let g=Number(c.dataset.index??0);if(g===this.activeIndex)return;this.activeIndex=g,this.resultsEl?.querySelectorAll(".viking-search-result").forEach((b,f)=>{let x=f===g;b.classList.toggle("is-selected",x),b.setAttribute("aria-selected",x?"true":"false")});let m=`${this.resultsId}-result-${g}`;this.inputEl?.setAttribute("aria-activedescendant",m)})})}render(){let t=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026",i=rt();this.shadow.innerHTML=`
...
`,vt=()=>{d($.tag,$),h($.legacyTag,$)};var Ve=new Set(["accent","secondary","success","warning","danger","info","muted"]),N=class extends v{static tag="viking-status-pill";static legacyTag="viking-status-pill-wc";static get observedAttributes(){return["tone","icon","href","target","compact","dot","removable","aria-label"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,$e)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"muted";return Ve.has(t)?t:"muted"}get compact(){let t=this.getAttribute("compact");return t!==null&&t!=="false"}get removable(){let t=this.getAttribute("removable");return t!==null&&t!=="false"}get href(){return this.getAttribute("href")}get icon(){return this.getAttribute("icon")}get showDot(){let t=this.getAttribute("dot");return t!==null&&t!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-pill-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone,i=this.compact?" status-pill--compact":"",r=this.icon?u(this.icon,14,"status-pill__icon"):"",a=this.showDot?'<span class="status-pill__dot" aria-hidden="true"></span>':"",n='<span class="status-pill__label" part="label"><slot></slot></span>',l=o(this.getAttribute("aria-label")??""),s=this.href,c=this.getAttribute("target"),g=s&&c==="_blank"?' rel="noopener noreferrer"':"";if(s){this.shadow.innerHTML=`
        <a
    ...
    `,this.shadow.querySelector(".status-pill__remove")?.addEventListener("click",this.onRemove)}},$e=`
:host {
  display: inline-flex;
}
...
`,ht=()=>{d(N.tag,N),h(N.legacyTag,N)};var q=[{id:"explore",label:"Explore",icon:"globe",appHref:"/explore",marketingHref:"/explore",platform:!0},{id:"documentation",label:"Documentation",icon:"file",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{id:"dashboard",label:"Dashboard",icon:"home",appHref:"/dashboard",marketingHref:"/dashboard",requireAuth:!0,platform:!0},{id:"sites",label:"Sites",icon:"building",appHref:"/settings",marketingHref:"/settings",requireAuth:!0,platform:!0},{id:"account",label:"Account",icon:"user",appHref:"/account",marketingHref:"/account",requireAuth:!0,platform:!0}],ut=[{title:"Platform",links:[{label:"Explore",appHref:"/explore",marketingHref:"/explore",platform:!0},{label:"Dashboard",appHref:"/dashboard",marketingHref:"/dashboard",platform:!0},{label:"Sites",appHref:"/settings",marketingHref:"/settings",platform:!0},{label:"Account",appHref:"/account",marketingHref:"/account",platform:!0},{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0}]},{title:"Resources",links:[{label:"Documentation",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{label:"Whitepaper",appHref:"https://dataengineeringformachinelearning.com/whitepaper/",marketingHref:"/whitepaper"},{label:"Book",appHref:"https://dataengineeringformachinelearning.com/book/",marketingHref:"/book"}]},{title:"Support",links:[{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0},{label:"Report a Bug",appHref:"#",marketingHref:"#",action:"bug-report"}]},{title:"Legal & Compliance",links:[{label:"Privacy Policy",appHref:"https://dataengineeringformachinelearning.com/privacy/",marketingHref:"/privacy"},{label:"Terms of Service",appHref:"https://dataengineeringformachinelearning.com/terms/",marketingHref:"/terms"},{label:"SOC2 Compliance",appHref:"https://dataengineeringformachinelearning.com/compliance/",marketingHref:"/compliance"},{label:"GDPR Compliance",appHref:"https://dataengineeringformachinelearning.com/privacy/#gdpr",marketingHref:"/privacy#gdpr"},{label:"Cookie Settings",appHref:"#",marketingHref:"#",action:"cookie-settings"}]}],Ne="reportBug=1",He="cookieSettings=1",Xt=e=>/^https?:\/\//i.test(e),pt=(e,t)=>{if(Xt(t))return t;let i=e.replace(/\/$/,""),r=t.startsWith("/")?t:`/${t}`;return`${i}${r}`},Tt=e=>`${pt(e.app,"/")}?${Ne}`,Lt=e=>`${pt(e.marketing,"/")}?${He}`,Jt=(e,t)=>Xt(e)||e.startsWith("mailto:")||e.startsWith("/")?e:pt(t.marketing,e),Qt=(e,t,i)=>t==="app"?e:pt(i.app,e),W=(e,t,i)=>e.platform?Qt(e.appHref,t,i):t==="app"?e.appHref:Jt(e.marketingHref,i),kt=(e,t,i)=>e.action==="bug-report"?t==="app"?"#":Tt(i):e.action==="cookie-settings"?Lt(i):e.platform?Qt(e.appHref,t,i):t==="app"?e.appHref:Jt(e.marketingHref,i),mt=(e,t)=>e==="marketing"?"/":t.marketing,Zt=(e,t=!1)=>e.filter(i=>!i.requireAuth||t);var y={app:"https://deml.app",marketing:"https://dataengineeringformachinelearning.com",backend:"https://backend.deml.app"};var te=e=>{if(e==null)return!1;let t=e.trim().toLowerCase();return t===""||t==="true"||t==="1"?!0:!(t==="false"||t==="0"||t==="off"||t==="no")},bt=(e,t=0)=>{if(e==null||e.trim()==="")return t;let i=Number(e);return Number.isFinite(i)?i:t},ee=e=>{if(!(e==null||e.trim()===""))try{return JSON.parse(e)}catch{return}},ft=(e,t,i)=>{let r=i??t[0];if(r===void 0)throw new Error("parseSelect requires a non-empty options list");return e==null?r:t.includes(e)?e:r};var U=class extends v{_props={};_updateScheduled=!1;connectedCallback(){this.scheduleRender()}attributeChangedCallback(t,i,r){i!==r&&this.isConnected&&this.scheduleRender()}attr(t,i){let r=this.getAttribute(t);return i?.parser?r==null&&i.default!==void 0?i.default:i.parser(r):r??(i?.default!==void 0?String(i.default):null)}setProp(t,i){Object.is(this._props[t],i)||(this._props[t]=i,this.isConnected&&this.scheduleRender())}getProp(t){return this._props[t]}requestUpdate(){this.scheduleRender()}scheduleRender(){this._updateScheduled||(this._updateScheduled=!0,queueMicrotask(()=>{this._updateScheduled=!1,this.isConnected&&this.render()}))}};var re=["app","marketing","backend","docs"],ie=()=>{if(typeof document>"u")return"marketing";let e=document.documentElement.getAttribute("data-deml-context");if(e&&re.includes(e))return e;if(typeof window>"u")return"marketing";let t=window.location.hostname;return t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},H=class extends U{static tag="viking-site-footer";static legacyTag="viking-site-footer-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","year"]}resolveContext(){return this.attr("context",{parser:t=>ft(t,re,ie()),default:ie()})}resolveUrls(){let t=globalThis.__DEML??{};return{app:this.getAttribute("app-url")??t.app??y.app,marketing:this.getAttribute("marketing-url")??t.marketing??y.marketing,backend:this.getAttribute("backend-url")??t.backend??y.backend}}resolveYear(){return this.attr("year",{parser:t=>bt(t,new Date().getFullYear()),default:new Date().getFullYear()})}render(){let t=this.resolveContext(),i=this.resolveUrls(),r=this.resolveYear(),a=ut.map(n=>{let l=n.links.map(s=>{let c=kt(s,t,i);return`
          <li>
            <a href="${o(c)}">${o(s.label)}</a>
          </li>
        `}).join("");return`
    ...
    `,this.bindUsaBadge()}emitUsaBadgeHover=t=>{if(t.type==="keydown"){let i=t.key;if(i!=="Enter"&&i!==" ")return;t.preventDefault()}this.dispatchEvent(new CustomEvent("usaBadgeHover",{bubbles:!0,composed:!0,detail:t}))};bindUsaBadge(){let t=this.querySelector("#usa-badge");t&&(t.addEventListener("mouseenter",this.emitUsaBadgeHover),t.addEventListener("focus",this.emitUsaBadgeHover),t.addEventListener("keydown",this.emitUsaBadgeHover))}},Q=()=>{d(H.tag,H),h(H.legacyTag,H)};Q();var ne=new Set(["app","marketing","backend","docs"]),Oe=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&ne.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},De=e=>{let t=e.getAttribute("context");return t&&ne.has(t)?t:Oe()},ae=(e,t)=>{let i=e.getAttribute(t);return i!==null&&i!=="false"},ze=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??y.app,marketing:e.getAttribute("marketing-url")??t.marketing??y.marketing,backend:e.getAttribute("backend-url")??t.backend??y.backend}},Pe=e=>ae(e,"authenticated"),Re=e=>e.getAttribute("show-search")===null?!0:ae(e,"show-search"),Ke=(e,t)=>{let i=e.getAttribute("sign-in-href");if(i)return i;let r=e.getAttribute("return-url")??window.location.href;return`${t.app}/login?returnUrl=${encodeURIComponent(r)}`},z=(e,t,i={})=>{let r=i.id?` id="${o(i.id)}"`:"",a=i.color?` data-viking-icon-color="${o(i.color)}"`:"",n=i.className?` class="${o(i.className)}"`:"";return`<span${r}${n} data-viking-icon="${o(e)}" data-viking-icon-size="${t}"${a} aria-hidden="true">${u(e,t,"viking-navbar-inline-icon")}</span>`},O=class extends HTMLElement{static tag="viking-site-navbar";static legacyTag="viking-site-navbar-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","show-search","sign-in-href","dashboard-href","return-url"]}mobileMenuOpen=!1;connectedCallback(){this.render(),this.addEventListener("click",this.onHostClick),document.addEventListener("click",this.onDocumentClick),document.addEventListener("keydown",this.onDocumentKeydown),window.addEventListener("resize",this.onWindowResize)}disconnectedCallback(){this.removeEventListener("click",this.onHostClick),document.removeEventListener("click",this.onDocumentClick),document.removeEventListener("keydown",this.onDocumentKeydown),window.removeEventListener("resize",this.onWindowResize)}attributeChangedCallback(){this.isConnected&&this.render()}renderNavLinks(t,i,r="desktop"){let a=r==="mobile"?"mobile-nav-btn":"nav-btn";return q.map(n=>{let l=W(n,t,i),s=n.requireAuth,c=s?" hidden":"",g=s?' data-require-auth="true"':"";return`
        <a
          href="${o(l)}"
          class="${a}"
          data-nav-id="${o(n.id)}"
      ...
      <div class="desktop-auth">
        <viking-button-wc
          ...
          id="auth-btn-desktop"
        >
          ${z(t?"home":"arrow-right",16,{id:"auth-icon-desktop"})}
    ...
    `}renderAuthMobile(t,i,r){return`
      ...
      <viking-button-wc
        ...
        id="auth-btn-mobile"
      >
        ${z(t?"home":"arrow-right",16,{id:"auth-icon-mobile"})}
    ...
    `:""}render(){let t=De(this),i=ze(this),r=Pe(this),a=Re(this),n=mt(t,i),l=Ke(this,i),s=this.getAttribute("dashboard-href")??`${i.app}/dashboard`,c=this.renderNavLinks(t,i,"desktop"),g=this.renderNavLinks(t,i,"mobile"),m=this.renderAuthDesktop(r,l,s),b=this.renderAuthMobile(r,s,l),f=this.renderSearchButton(a);this.innerHTML=`
      <header class="navbar">
    ...
    `,this.syncMobileMenu()}onHostClick=t=>{let i=typeof t.composedPath=="function"?t.composedPath():[];if(i.some(n=>n instanceof HTMLElement&&n.id==="mobile-menu-btn")){t.preventDefault(),this.toggleMobileMenu();return}i.some(n=>n instanceof HTMLElement&&n.closest?.("#mobile-menu")&&(n.matches("a")||n.matches("viking-button-wc")))&&this.closeMobileMenu()};onDocumentClick=t=>{if(!this.mobileMenuOpen)return;let i=t.target;i instanceof Node&&!this.contains(i)&&this.closeMobileMenu()};onDocumentKeydown=t=>{t.key==="Escape"&&this.closeMobileMenu()};onWindowResize=()=>{window.matchMedia("(min-width: 768px)").matches&&this.closeMobileMenu()};toggleMobileMenu(){this.mobileMenuOpen=!this.mobileMenuOpen,this.syncMobileMenu()}closeMobileMenu(){this.mobileMenuOpen&&(this.mobileMenuOpen=!1,this.syncMobileMenu())}syncMobileMenu(){let t=this.querySelector("#mobile-menu"),i=this.querySelector("#mobile-menu-btn");!t||!i||(t.classList.toggle("open",this.mobileMenuOpen),t.toggleAttribute("hidden",!this.mobileMenuOpen),i.setAttribute("aria-expanded",this.mobileMenuOpen?"true":"false"),i.setAttribute("aria-label",this.mobileMenuOpen?"Close navigation menu":"Toggle navigation menu"),i.innerHTML=z(this.mobileMenuOpen?"x":"menu",24))}},yt=()=>{d(O.tag,O),h(O.legacyTag,O)};var Be=e=>{let t=new Set;return e.filter(i=>{let r=`${i.title}:${i.href}:${i.action??""}`;return t.has(r)?!1:(t.add(r),!0)})},qe=(e,t,i)=>({title:e.label,href:W(e,t,i),snippet:`Open ${e.label}`,group:"Platform",keywords:[e.id,e.label.toLowerCase(),"navigate","go"]}),We=(e,t,i,r)=>e.action==="cookie-settings"?{title:e.label,href:Lt(r),snippet:"Manage analytics and cookie preferences",group:t,keywords:["cookies","consent","privacy","gdpr"],action:"cookie-settings"}:e.action==="bug-report"?{title:e.label,href:i==="app"?"#":Tt(r),snippet:"Submit a product issue or regression",group:t,keywords:["bug","issue","support","feedback"],action:"bug-report"}:{title:e.label,href:kt(e,i,r),snippet:`Open ${e.label}`,group:t,keywords:[e.label.toLowerCase(),t.toLowerCase()]},Ue=[{title:"Viking-UI Components",snippet:"Browse the design system showcase",group:"Resources",keywords:["viking","ui","design system","components","showcase"]},{title:"Design tokens",snippet:"Canonical --viking-* token matrix",group:"Resources",keywords:["tokens","theme","css","variables"]},{title:"API reference",snippet:"OpenAPI schema and REST endpoints",group:"Resources",keywords:["api","openapi","swagger","rest"]}],Ge=(e,t,i)=>{switch(e.title){case"Viking-UI Components":return"https://ui.dataengineeringformachinelearning.com/components";case"Design tokens":return"https://ui.dataengineeringformachinelearning.com/tokens";case"API reference":return`${i.backend.replace(/\/$/,"")}/api/v1/docs`;default:return t==="app"?i.app:i.marketing}},Fe=[{title:"Components",href:"/components",snippet:"Browse all documented primitives",group:"Viking-UI",keywords:["components","showcase","registry"]},{title:"Playground",href:"/playground",snippet:"Live Web Component sandbox",group:"Viking-UI",keywords:["playground","sandbox","demo"]},{title:"Architecture",href:"/architecture",snippet:"CSS + WC + Angular layers",group:"Viking-UI",keywords:["architecture","layers","web component"]},{title:"Design tokens",href:"/tokens",snippet:"Canonical --viking-* token matrix",group:"Viking-UI",keywords:["tokens","theme","css","variables"]},{title:"Theming",href:"/theming",snippet:"Light/dark mode and sync pipeline",group:"Viking-UI",keywords:["theming","dark","light","mode"]},{title:"Framework guides",href:"/frameworks",snippet:"Angular, Astro, Django setup",group:"Viking-UI",keywords:["frameworks","angular","astro","django"]},{title:"Contributing",href:"/contributing",snippet:"Extend the Viking-UI kit",group:"Viking-UI",keywords:["contributing","extend","primitives"]}],je=(e,t)=>e.startsWith("http")?e:`${t.replace(/\/$/,"")}${e.startsWith("/")?e:`/${e}`}`,Vt=(e,t,i)=>{let r=e==="docs"?"marketing":e,a=[...q.map(n=>qe(n,r,t)),...ut.flatMap(n=>n.links.map(l=>We(l,n.title,r,t))),...Ue.map(n=>({...n,href:Ge(n,r,t)}))];if(e==="app"&&a.push({title:"Dashboard",href:"/dashboard",snippet:"CES overview, KPIs, and performance telemetry",group:"App",keywords:["dashboard","ces","home","overview","kpi"]},{title:"Analytics",href:"/analytics",snippet:"Latency, origins, threat charts, and gauges",group:"App",keywords:["analytics","charts","latency","map","threat"]},{title:"Explore status pages",href:"/explore",snippet:"Public status directory",group:"App",keywords:["explore","status","directory","public"]},{title:"Status pages",href:"/status",snippet:"Your published and draft status surfaces",group:"App",keywords:["status","pages","uptime","incidents"]},{title:"Platform status",href:"/status/platform-status",snippet:"Live public sentinel for the DEML stack",group:"App",keywords:["platform-status","tenant0","health","sla"]},{title:"Vulnerabilities",href:"/vulnerabilities",snippet:"SOC triage and vulnerability Kanban",group:"App",keywords:["vulnerabilities","soc","semgrep","trivy","kanban"]},{title:"Account",href:"/account",snippet:"Profile, MFA enrollment, and linked accounts",group:"App",keywords:["account","profile","mfa","oauth"]},{title:"Settings",href:"/settings",snippet:"Workspace domains, billing, and security",group:"App",keywords:["settings","sites","workspace","configuration"]},{title:"Billing & subscription",href:"/settings/billing",snippet:"Manage plan, invoices, and payment methods",group:"App",keywords:["billing","stripe","subscription","payment"]},{title:"Security settings",href:"/settings/security",snippet:"Keys, sessions, and access controls",group:"App",keywords:["security","keys","auth","rbac"]},{title:"Login",href:"/login",snippet:"Sign in or complete SMS MFA",group:"App",keywords:["login","sign in","auth","mfa"]}),e==="backend"&&a.push({title:"OpenAPI / Swagger",href:`${t.backend.replace(/\/$/,"")}/api/v1/docs`,snippet:"Interactive public ingest and predict sandbox",group:"Backend",keywords:["swagger","openapi","docs","api"]},{title:"Backend home",href:t.backend,snippet:"API landing and service overview",group:"Backend",keywords:["backend","api","home"]}),e==="docs"){let n=i?.docsOrigin??"https://ui.dataengineeringformachinelearning.com";a.push(...Fe.map(l=>({...l,href:je(l.href,n)})))}return Be(a)};var oe=["dataengineeringformachinelearning_com_zjafyosh2v_pages","deml_app_pages","deml_backend_pages","DEML UI"],Ye=()=>typeof globalThis>"u"?{}:globalThis.ALGOLIA_CONFIG??{},w=e=>typeof e=="string"&&e.trim()?e:null,Xe=(e,t)=>{let i=w(e.objectID),r=w(e.url)||w(e.url_without_anchor)||(i?.startsWith("http")?i:null);if(!r)return null;let a=e.hierarchy,n=w(e.title)||w(a?.lvl1)||w(a?.lvl0)||w(e.path)||r,l=w(e.description)||(w(e.content)??"").slice(0,160)||w(a?.lvl2)||"",s=w(e.hostname);if(!s)try{s=new URL(r).hostname}catch{s=t}let c=w(e.keywords),g=w(e.path)??"";return{title:n.slice(0,120),href:r,snippet:l?`${s} \xB7 ${l.slice(0,140)}`:s,group:"Live pages",keywords:[s,t,g,...c?c.split(/[,\s]+/).filter(Boolean):[]].filter(Boolean)}},xt=async(e,t)=>{let i=e.trim();if(!i||i.length<2)return[];let r=Ye(),a=r.appId||"ZJAFYOSH2V",n=r.apiKey||"";if(!n)return[];let l=r.indexNames&&r.indexNames.length>0?r.indexNames:[...oe],s=t?.hitsPerPage??8,c=`https://${a}-dsn.algolia.net/1/indexes/*/queries`;try{let g=await fetch(c,{method:"POST",headers:{"Content-Type":"application/json","X-Algolia-Application-Id":a,"X-Algolia-API-Key":n},body:JSON.stringify({requests:l.map(x=>({indexName:x,params:new URLSearchParams({query:i,hitsPerPage:String(s),attributesToRetrieve:["url","url_without_anchor","title","description","content","path","hostname","keywords","hierarchy","type"].join(","),attributesToHighlight:"[]"}).toString()}))}),signal:t?.signal});if(!g.ok)return[];let m=await g.json(),b=new Set,f=[];return(m.results??[]).forEach((x,I)=>{let Et=x.index||l[I]||"pages";(x.hits??[]).forEach(Z=>{let X=Xe(Z,Et);!X||b.has(X.href)||(b.add(X.href),f.push(X))})}),f}catch{return[]}},se=oe;var le=new Set(["app","marketing","backend","docs"]),Je=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&le.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.includes("deml.app")&&!t.startsWith("backend.")?"app":t.startsWith("backend.")?"backend":"marketing"},Qe=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??y.app,marketing:e.getAttribute("marketing-url")??t.marketing??y.marketing,backend:e.getAttribute("backend-url")??t.backend??y.backend}},Ze=e=>{let t=e.getAttribute("context");return t&&le.has(t)?t:Je()},S=class extends v{static tag="viking-suite-command-palette";static searchTag="viking-suite-search-palette";static legacyTag="viking-suite-search-palette-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","placeholder","global-shortcut"]}paletteEl=null;itemsLoaded=!1;curatedItems=[];queryTimer=null;searchAbort=null;onPaletteQuery=t=>{let r=t.detail?.query??"";this.scheduleAlgoliaSearch(r)};connectedCallback(){B(),this.ensurePalette(),this.loadItems()}disconnectedCallback(){this.paletteEl?.removeEventListener("viking-query",this.onPaletteQuery),this.queryTimer&&(clearTimeout(this.queryTimer),this.queryTimer=null),this.searchAbort?.abort()}attributeChangedCallback(t){if(this.isConnected){if(t==="placeholder"&&this.paletteEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",i);return}if(t==="global-shortcut"&&this.paletteEl){k(this,"global-shortcut")||!this.hasAttribute("global-shortcut")?this.paletteEl.setAttribute("global-shortcut",""):this.paletteEl.removeAttribute("global-shortcut");return}(t==="context"||t==="app-url"||t==="marketing-url"||t==="backend-url")&&this.loadItems(!0)}}openPalette(){this.ensurePalette(),this.loadItems().then(()=>this.paletteEl?.openPalette())}closePalette(){this.paletteEl?.closePalette()}ensurePalette(){if(this.paletteEl)return;this.paletteEl=document.createElement("viking-command-palette"),this.paletteEl.id="deml-command-palette";let t=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",t),(k(this,"global-shortcut")||!this.hasAttribute("global-shortcut"))&&this.paletteEl.setAttribute("global-shortcut",""),this.paletteEl.addEventListener("viking-query",this.onPaletteQuery),this.append(this.paletteEl)}scheduleAlgoliaSearch(t){this.queryTimer&&clearTimeout(this.queryTimer),this.queryTimer=setTimeout(()=>{this.mergeAlgoliaResults(t)},180)}async mergeAlgoliaResults(t){if(!this.paletteEl)return;this.itemsLoaded||await this.loadItems();let i=t.trim();if(i.length<2){this.paletteEl.setAttribute("items",JSON.stringify(this.curatedItems));return}this.searchAbort?.abort(),this.searchAbort=new AbortController;let r=await xt(i,{hitsPerPage:6,signal:this.searchAbort.signal}),a=this.curatedItems,n=new Set(a.map(s=>s.href.replace(/\/$/,""))),l=r.filter(s=>{let c=s.href.replace(/\/$/,"");return n.has(c)?!1:(n.add(c),!0)}).map(s=>({...s,keywords:[...s.keywords??[],i,s.title,s.href]}));this.paletteEl.setAttribute("items",JSON.stringify([...a,...l]))}async loadItems(t=!1){if(!this.paletteEl||this.itemsLoaded&&!t)return;let i=Ze(this),r=Qe(this),a=i==="docs"?window.location.origin:"https://ui.dataengineeringformachinelearning.com",n=Vt(i,r,{docsOrigin:a});try{(await fetch("/assets/site-drakkar.json",{cache:"no-cache"})).ok&&(n=Vt(i,r,{docsOrigin:a}))}catch{}this.curatedItems=n,this.paletteEl.setAttribute("items",JSON.stringify(n)),this.itemsLoaded=!0}},G=()=>{d(S.tag,S),h(S.searchTag,S),h(S.legacyTag,S)};var ce=`
:host {
  display: inline-flex;
}
...
`,ti=e=>{document.documentElement.setAttribute("data-theme",e),document.documentElement.classList.toggle("dark",e==="dark"),localStorage.setItem("theme",e)},de=()=>{let e=localStorage.getItem("theme");return e==="light"||e==="dark"?e:window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"},F=class extends v{static tag="viking-theme-toggle-wc";shadow;button=null;sunIcon=null;moonIcon=null;onStorage=()=>{this.syncIcons()};constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,ce)}connectedCallback(){this.setAttribute("role","button"),this.render(),this.syncIcons(),this.button?.addEventListener("click",this.onClick),window.addEventListener("storage",this.onStorage),window.addEventListener("viking-theme-change",this.onThemeChange)}disconnectedCallback(){this.button?.removeEventListener("click",this.onClick),window.removeEventListener("storage",this.onStorage),window.removeEventListener("viking-theme-change",this.onThemeChange)}onThemeChange=()=>{this.syncIcons()};onClick=()=>{let t=de()==="light"?"dark":"light";ti(t),this.syncIcons(),this.dispatchEvent(new CustomEvent("viking-theme-change",{bubbles:!0,composed:!0,detail:{theme:t}}))};syncIcons=()=>{let t=de()==="dark";this.sunIcon?.classList.toggle("is-visible",t),this.moonIcon?.classList.toggle("is-visible",!t)};render(){let t=this.getAttribute("aria-label")??"Toggle light and dark theme";this.shadow.innerHTML=`
    ...
    `,p(this.shadow,ce),this.button=this.shadow.querySelector("button"),this.sunIcon=this.shadow.querySelector(".theme-icon-sun"),this.moonIcon=this.shadow.querySelector(".theme-icon-moon")}},j=()=>{d(F.tag,F)};var ge=new Set(["app","marketing","backend","docs"]),ei=`
:host {
  display: block;
  position: sticky;
  top: 0;
...
`,ii=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&ge.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.startsWith("backend.")?"backend":t.includes("deml.app")?"app":"marketing"},ri=e=>{let t=e.getAttribute("context");return t&&ge.has(t)?t:ii()},ni=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??y.app,marketing:e.getAttribute("marketing-url")??t.marketing??y.marketing,backend:e.getAttribute("backend-url")??t.backend??y.backend}},ai=e=>{try{let t=new URL(e,window.location.origin);return t.origin===window.location.origin&&t.pathname===window.location.pathname}catch{return!1}},Y=class extends v{static tag="viking-suite-header";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","user-name","user-email","sign-in-href","dashboard-href"]}shadow;menuOpen=!1;userMenuOpen=!1;paletteEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),p(this.shadow,ei)}connectedCallback(){j(),G(),this.render(),document.addEventListener("click",this.onDocumentClick)}disconnectedCallback(){document.removeEventListener("click",this.onDocumentClick)}attributeChangedCallback(){this.isConnected&&this.render()}openSearch(){this.paletteEl?.openPalette(),this.dispatchEvent(new CustomEvent("viking-search-open",{bubbles:!0,composed:!0}))}onDocumentClick=t=>{t.composedPath().includes(this)||(this.menuOpen||this.userMenuOpen)&&(this.menuOpen=!1,this.userMenuOpen=!1,this.syncOpenState())};onSearchClick=()=>{this.openSearch()};onMenuClick=()=>{this.menuOpen=!this.menuOpen,this.syncOpenState()};onUserClick=()=>{this.userMenuOpen=!this.userMenuOpen,this.syncOpenState()};onSignInClick=t=>{let i=new CustomEvent("viking-sign-in",{bubbles:!0,cancelable:!0,composed:!0,detail:t});this.dispatchEvent(i),i.defaultPrevented&&t.preventDefault()};onSignOutClick=()=>{this.userMenuOpen=!1,this.menuOpen=!1,this.syncOpenState(),this.dispatchEvent(new CustomEvent("viking-sign-out",{bubbles:!0,composed:!0}))};syncOpenState(){let t=this.shadow.querySelector(".suite-header"),i=this.shadow.querySelector("[data-menu-toggle]"),r=this.shadow.querySelector(".suite-header__user"),a=this.shadow.querySelector("[data-user-toggle]");t?.setAttribute("data-menu-open",String(this.menuOpen)),i?.setAttribute("aria-expanded",String(this.menuOpen)),r?.setAttribute("data-open",String(this.userMenuOpen)),a?.setAttribute("aria-expanded",String(this.userMenuOpen))}renderNavLinks(t,i,r){return Zt(q,r).map(a=>{let n=W(a,t,i),l=ai(n)?' aria-current="page"':"";return`
    ...
    `}getDashboardHref(t){return this.getAttribute("dashboard-href")??`${t.app}/dashboard`}render(){let t=ri(this),i=ni(this),r=k(this,"authenticated"),a=this.renderNavLinks(t,i,r),n=mt(t,i),l=`${rt()}K`;this.shadow.innerHTML=`
        ...
        <div class="suite-header__bar">
          ...
          <div class="suite-header__actions">
            <button
              ...
              type="button"
              aria-label="Open command palette (${l})"
