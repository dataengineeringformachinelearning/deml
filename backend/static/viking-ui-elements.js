...
`;var Qe=new Set(["accent","secondary","success","warning","danger","info","muted","subtle"]),x=class extends l{static tag="viking-badge";static legacyTag="viking-badge-wc";static get observedAttributes(){return["tone","size","icon","removable"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,Ie)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let e=this.getAttribute("tone")??"";return Qe.has(e)?e:null}get size(){return this.getAttribute("size")==="sm"?"sm":null}get removable(){return this.hasAttribute("removable")&&this.getAttribute("removable")!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-removed",{bubbles:!0,composed:!0}))};render(){let e=this.tone;e&&this.getAttribute("tone")!==e?this.setAttribute("tone",e):!e&&this.hasAttribute("tone")&&this.removeAttribute("tone");let i=this.size;i&&this.getAttribute("size")!==i?this.setAttribute("size",i):!i&&this.hasAttribute("size")&&this.removeAttribute("size");let n=this.getAttribute("icon")??(e?W[e]:null),r=n?d(n,16):"";this.shadow.innerHTML=`
      ${r}
      <span part="label"><slot></slot></span>
      ${this.removable?`<button type="button" class="viking-badge-remove" part="remove" aria-label="Remove">${d("x",14)}</button>`:""}
    `,this.shadow.querySelector(".viking-badge-remove")?.addEventListener("click",this.onRemove)}},U=()=>{c(x.tag,x),v(x.legacyTag,x)};var Je=new Set(["outline","primary","secondary","filled","danger","ghost","subtle"]),et=new Set(["sm","xs"]),N=class extends l{static angularTag="viking-button";static tag="viking-button-wc";static get observedAttributes(){return["variant","size","type","disabled","loading","href","target","aria-label","aria-busy","square","full-width","compact"]}shadow;control=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,Ee)}connectedCallback(){this.render(),this.control?.addEventListener("click",this.onClick)}disconnectedCallback(){this.control?.removeEventListener("click",this.onClick)}attributeChangedCallback(){this.isConnected&&this.render()}onClick=e=>{if(this.disabled||this.loading){e.preventDefault(),e.stopPropagation();return}this.dispatchEvent(new CustomEvent("viking-press",{bubbles:!0,composed:!0,detail:e}))};get variant(){let e=this.getAttribute("variant")??"outline";return Je.has(e)?e:"outline"}get size(){let e=this.getAttribute("size");return e&&et.has(e)?e:null}get disabled(){return g(this,"disabled")}get loading(){return g(this,"loading")}get square(){return g(this,"square")}render(){let e=this.getAttribute("href"),i=!!e,n=i?"a":"button",r=["viking-btn",`viking-btn-${this.variant}`,this.size?`viking-btn-${this.size}`:"",this.square?"viking-btn-square":""].filter(Boolean).join(" "),o=this.getAttribute("aria-label")??"",s=this.getAttribute("aria-busy")==="true"||this.loading?"true":null,u=a(this.getAttribute("type")??"button"),p=e?a(e):"",k=this.getAttribute("target"),f=k?a(k):"";this.shadow.innerHTML=`
      <${n}
        class="${r}"
        part="control"
        ${i?`href="${p}"`:`type="${u}"`}
    ...
    `,this.control=this.shadow.querySelector(n)}},j=()=>{c(N.tag,N)};var tt=new Set(["accent","secondary","success","warning","danger","info","muted"]),w=class extends l{static tag="viking-callout";static legacyTag="viking-callout-wc";static get observedAttributes(){return["tone","heading","icon","dismissible","hidden"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,Me)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let e=this.getAttribute("tone")??"info";return tt.has(e)?e:"info"}get dismissible(){return this.hasAttribute("dismissible")&&this.getAttribute("dismissible")!=="false"}onDismiss=()=>{this.setAttribute("hidden",""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};render(){let e=this.getAttribute("heading")??"",i=this.getAttribute("icon")??W[this.tone]??"info",n=d(i,22,"viking-callout-icon");this.shadow.innerHTML=`
      <div class="viking-callout viking-callout-${this.tone}" role="note" part="surface">
        ...
        ${this.dismissible?`<button type="button" class="viking-callout-close" part="close" aria-label="Dismiss">${d("x",18)}</button>`:""}
      </div>
    `,this.shadow.querySelector(".viking-callout-close")?.addEventListener("click",this.onDismiss)}},F=()=>{c(w.tag,w),v(w.legacyTag,w)};var E=class extends l{static tag="viking-card";static legacyTag="viking-card-wc";static get observedAttributes(){return["compact","interactive","title","loading"]}connectedCallback(){this.syncClasses()}attributeChangedCallback(){this.isConnected&&this.syncClasses()}syncClasses(){this.classList.add("viking-card"),this.classList.toggle("viking-card-compact",this.hasAttribute("compact")),this.classList.toggle("viking-card-interactive",this.hasAttribute("interactive")),this.classList.toggle("viking-card-loading",this.hasAttribute("loading"));let e=this.getAttribute("title");e?(this.setAttribute("role","region"),this.setAttribute("aria-label",e)):(this.removeAttribute("role"),this.removeAttribute("aria-label"))}},Y=()=>{c(E.tag,E),v(E.legacyTag,E)};var _=class extends l{static tag="viking-field";static legacyTag="viking-field-wc";static get observedAttributes(){return["label","description","error","required","width"]}shadow;labelId=y("viking-field-label");descriptionId=y("viking-field-description");errorId=y("viking-field-error");slotEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,Se)}connectedCallback(){this.render(),this.syncControlA11y()}disconnectedCallback(){this.slotEl?.removeEventListener("slotchange",this.syncControlA11y)}attributeChangedCallback(){this.isConnected&&(this.render(),this.syncControlA11y())}get control(){return(this.slotEl?.assignedElements({flatten:!0})??[]).find(i=>i instanceof HTMLElement)??null}focusControl=()=>{this.control?.focus?.()};syncControlA11y=()=>{let e=this.control;if(!e)return;let i=[this.getAttribute("description")??"",this.getAttribute("error")??""].filter(Boolean).join(" "),n=this.getAttribute("label")??"";n&&!e.hasAttribute("aria-label")&&e.setAttribute("aria-label",n),i?e.setAttribute("aria-description",i):e.removeAttribute("aria-description"),this.getAttribute("error")?(e.setAttribute("aria-invalid","true"),e.setAttribute("error",this.getAttribute("error")??"")):(e.removeAttribute("aria-invalid"),e.getAttribute("error")===""&&e.removeAttribute("error")),g(this,"required")&&e.setAttribute("required","")};render(){let e=this.getAttribute("label")??"",i=this.getAttribute("description")??"",n=this.getAttribute("error")??"",r=g(this,"required");this.shadow.innerHTML=`
    ...
    `,this.slotEl=this.shadow.querySelector("slot"),this.slotEl?.addEventListener("slotchange",this.syncControlA11y),this.shadow.querySelector(".viking-field-label")?.addEventListener("click",this.focusControl)}},X=()=>{c(_.tag,_),v(_.legacyTag,_)};var S=class extends l{static formAssociated=!0;static tag="viking-input";static legacyTag="viking-input-wc";static get observedAttributes(){return["type","placeholder","value","disabled","loading","clearable","name","autocomplete","required","readonly","minlength","maxlength","pattern","error","aria-label","aria-describedby","bare"]}shadow;internals;input=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.internals=B(this),h(this.shadow,_e)}connectedCallback(){this.render(),this.syncFormValue()}attributeChangedCallback(e){if(this.isConnected){if(e==="value"&&this.input){this.input.value=this.getAttribute("value")??"",this.syncFormValue();return}this.render()}}get value(){return this.input?.value??this.getAttribute("value")??""}set value(e){let i=e??"";this.setAttribute("value",i),this.input&&(this.input.value=i),this.syncFormValue()}get disabled(){return g(this,"disabled")}get loading(){return g(this,"loading")}get clearable(){return g(this,"clearable")}get bare(){return g(this,"bare")}onInput=()=>{let e=this.input?.value??"";this.setAttribute("value",e),this.syncFormValue(),this.dispatchEvent(new Event("input",{bubbles:!0,composed:!0})),this.dispatchEvent(new Event("change",{bubbles:!0,composed:!0}))};onBlur=()=>{this.dispatchEvent(new Event("blur",{bubbles:!0,composed:!0}))};onClear=()=>{this.value="",this.input?.focus(),this.dispatchEvent(new CustomEvent("viking-cleared",{bubbles:!0,composed:!0})),this.dispatchEvent(new Event("input",{bubbles:!0,composed:!0}))};syncFormValue(){K(this.internals,this.value)}render(){let e=["viking-input-shell",this.disabled?"viking-disabled":"",this.loading?"viking-loading":""].filter(Boolean).join(" "),i=a(this.getAttribute("type")??"text"),n=this.getAttribute("placeholder")??"",r=a(n),o=a(this.getAttribute("value")??""),s=this.getAttribute("aria-label")??(n||"Text input"),u=a(this.getAttribute("autocomplete")??""),p=a(this.getAttribute("aria-describedby")??""),k=a(this.getAttribute("minlength")??""),f=a(this.getAttribute("maxlength")??""),C=a(this.getAttribute("pattern")??""),L=this.getAttribute("error")??"",re=g(this,"required"),P=g(this,"readonly"),Ke=this.clearable&&o.length>0&&!this.loading&&!this.bare,ve=`
      ${this.disabled||this.loading?"disabled":""}
      ${re?"required":""}
      ${P?"readonly":""}
      aria-label="${a(s)}"
    ...
    `;this.bare?this.shadow.innerHTML=`
      ...
      `,this.input=this.shadow.querySelector("input"),this.input?.addEventListener("input",this.onInput),this.input?.addEventListener("blur",this.onBlur),this.shadow.querySelector(".viking-input-clear")?.addEventListener("click",this.onClear)}},Z=()=>{c(S.tag,S),v(S.legacyTag,S)};var I=class extends l{static angularTag="viking-modal";static tag="viking-modal-wc";static dialogTag="viking-dialog";static get observedAttributes(){return["open","title","dismissible"]}shadow;dialogEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,Ce)}connectedCallback(){this.render(),this.syncOpen(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick)}attributeChangedCallback(e){if(this.isConnected){if(e==="open"){this.syncOpen();return}if(e==="title"){this.updateTitle();return}e==="dismissible"&&(this.render(),this.syncOpen())}}openModal(){this.setAttribute("open",""),this.syncOpen()}closeModal(){this.removeAttribute("open"),V(this.dialogEl)}open(){this.openModal()}close(){this.closeModal()}get dismissible(){return this.getAttribute("dismissible")!=="false"}onClose=()=>{this.removeAttribute("open"),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=e=>{this.dismissible&&e.target===this.dialogEl&&this.closeModal()};syncOpen(){if(!this.dialogEl)return;let e=this.hasAttribute("open");e&&!this.dialogEl.open?(G(this.dialogEl),queueMicrotask(()=>{(this.shadow.querySelector(".viking-modal-close")??this.dialogEl)?.focus()})):!e&&this.dialogEl.open&&V(this.dialogEl)}updateTitle(){let e=this.getAttribute("title")??"Dialog",i=this.shadow.querySelector(".viking-modal-heading");i&&(i.textContent=e),this.dialogEl?.setAttribute("aria-label",e)}render(){let e=this.getAttribute("title")??"Dialog";this.shadow.innerHTML=`
      <dialog class="viking-modal-backdrop" aria-label="${a(e)}" aria-modal="true">
        <div class="viking-modal-panel" part="panel" role="document">
          <header class="viking-modal-header" part="header">
            ...
            ${this.dismissible?`<button type="button" class="viking-modal-close" part="close" aria-label="Close dialog">${d("x",20)}</button>`:""}
          </header>
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.shadow.querySelector(".viking-modal-close")?.addEventListener("click",()=>this.closeModal()),this.dialogEl?.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissible&&(i.preventDefault(),this.closeModal())})}},Q=()=>{c(I.tag,I),v(I.dialogTag,I)};var it=t=>{let e=t.getAttribute("items");if(!e)return[];try{let i=JSON.parse(e);return Array.isArray(i)?i:[]}catch{return[]}},nt=(t,e)=>[t.title,t.snippet??"",t.group??"",t.href,...t.keywords??[]].join(" ").toLowerCase().includes(e),rt=(t,e)=>{let i=e.trim().toLowerCase();return i?t.filter(n=>nt(n,i)):t},at=t=>{let e=new Map;return t.forEach(i=>{let n=i.group??null,r=e.get(n)??[];r.push(i),e.set(n,r)}),Array.from(e.entries()).map(([i,n])=>({group:i,items:n}))},m=class extends l{static tag="viking-command-palette";static searchTag="viking-search-palette";static legacyTag="viking-search-palette-wc";static get observedAttributes(){return["open","placeholder","items","global-shortcut"]}shadow;dialogEl=null;inputEl=null;resultsEl=null;globalKeyHandler=null;resultsId=y("viking-search-results");inputId=y("viking-search-input");query="";activeIndex=0;flatResults=[];constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,Le)}connectedCallback(){this.render(),this.removeAttribute("open"),this.syncOpen(),this.bindGlobalShortcut(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick),this.inputEl?.addEventListener("input",this.onInput),this.inputEl?.addEventListener("keydown",this.onInputKeydown)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick),this.inputEl?.removeEventListener("input",this.onInput),this.inputEl?.removeEventListener("keydown",this.onInputKeydown),this.unbindGlobalShortcut()}attributeChangedCallback(e){if(this.isConnected&&(e==="open"&&this.syncOpen(),e==="global-shortcut"&&(this.unbindGlobalShortcut(),this.bindGlobalShortcut()),e==="items"||e==="placeholder"))if(e==="placeholder"&&this.inputEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026";this.inputEl.placeholder=i,this.inputEl.setAttribute("aria-label",i)}else this.renderResults()}openPalette(){this.setAttribute("open",""),this.syncOpen()}closePalette(){this.removeAttribute("open"),V(this.dialogEl)}search(e){this.query=e,this.inputEl&&(this.inputEl.value=e),this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))}onClose=()=>{this.removeAttribute("open"),this.query="",this.activeIndex=0,this.inputEl&&(this.inputEl.value=""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=e=>{e.target===this.dialogEl&&this.closePalette()};onInput=e=>{this.query=e.target.value??this.inputEl?.value??"",this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))};onInputKeydown=e=>{if(this.flatResults.length===0){e.key==="Escape"&&(e.preventDefault(),this.closePalette());return}if(e.key==="ArrowDown"){e.preventDefault(),this.activeIndex=Math.min(this.flatResults.length-1,this.activeIndex+1),this.renderResults(),this.scrollActiveIntoView();return}if(e.key==="ArrowUp"){e.preventDefault(),this.activeIndex=Math.max(0,this.activeIndex-1),this.renderResults(),this.scrollActiveIntoView();return}if(e.key==="Enter"){e.preventDefault();let i=this.flatResults[this.activeIndex];i&&this.activateItem(i)}};bindGlobalShortcut(){g(this,"global-shortcut")&&(this.globalKeyHandler=e=>{(e.metaKey||e.ctrlKey)&&e.key.toLowerCase()==="k"&&(e.preventDefault(),this.hasAttribute("open")?this.closePalette():this.openPalette())},document.addEventListener("keydown",this.globalKeyHandler))}unbindGlobalShortcut(){this.globalKeyHandler&&(document.removeEventListener("keydown",this.globalKeyHandler),this.globalKeyHandler=null)}syncOpen(){if(!this.dialogEl)return;let e=this.hasAttribute("open");e&&!this.dialogEl.open?(this.dialogEl.removeAttribute("aria-hidden"),G(this.dialogEl),this.activeIndex=0,this.renderResults(),queueMicrotask(()=>this.inputEl?.focus())):!e&&this.dialogEl.open?(V(this.dialogEl),this.dialogEl.setAttribute("aria-hidden","true")):e||this.dialogEl.setAttribute("aria-hidden","true")}scrollActiveIntoView(){this.resultsEl?.querySelector(".viking-search-result.is-selected")?.scrollIntoView({block:"nearest"})}activateItem(e){if(this.dispatchEvent(new CustomEvent("viking-select",{bubbles:!0,composed:!0,detail:{item:e}})),this.closePalette(),e.action==="cookie-settings"){globalThis.DemlWidgets?.openCookieSettings?.();return}if(e.action==="bug-report"){let i=globalThis.DemlWidgets;if(i?.openBugReport){i.openBugReport();return}}e.href&&e.href!=="#"&&window.location.assign(e.href)}renderResults(){if(!this.resultsEl)return;let e=it(this),i=this.query.trim().toLowerCase(),n=rt(e,i);if(this.flatResults=n,n.length===0){let p=this.query.trim();this.resultsEl.innerHTML=`<p class="viking-search-empty" role="status">${p?"No results found":"Start typing to search\u2026"}</p>`,this.inputEl?.removeAttribute("aria-activedescendant");return}this.activeIndex>=n.length&&(this.activeIndex=n.length-1);let r=0,s=at(n).map(({group:p,items:k})=>{let f=p?`<p class="viking-search-group-label" role="presentation">${a(p)}</p>`:"",C=k.map(L=>{let re=`${this.resultsId}-result-${r}`,P=r===this.activeIndex;return r+=1,`
              <a
                id="${re}"
              ...
              </a>`}).join("");return`${f}${C}`}).join("");this.resultsEl.innerHTML=`<div class="viking-search-results" id="${this.resultsId}" role="listbox" aria-label="Search results">${s}</div>`;let u=`${this.resultsId}-result-${this.activeIndex}`;this.inputEl?.setAttribute("aria-activedescendant",u),this.inputEl?.setAttribute("role","combobox"),this.inputEl?.setAttribute("aria-expanded","true"),this.inputEl?.setAttribute("aria-controls",this.resultsId),this.resultsEl.querySelectorAll(".viking-search-result").forEach(p=>{p.addEventListener("click",k=>{k.preventDefault();let f=Number(p.dataset.index??0),C=this.flatResults[f];C&&this.activateItem(C)}),p.addEventListener("mouseenter",()=>{let k=Number(p.dataset.index??0);this.activeIndex=k,this.renderResults()})})}render(){let e=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026",i=q();this.shadow.innerHTML=`
      <dialog class="viking-search-palette-backdrop" aria-label="Search" aria-hidden="true">
        <div class="viking-search-palette" part="panel" role="document">
          <div class="viking-search-palette-header" part="header">
            <span class="viking-search-palette-icon" aria-hidden="true">${d("search",24)}</span>
            <input
              id="${this.inputId}"
              type="search"
              ...
              spellcheck="false"
            />
            <button type="button" class="viking-search-palette-close" part="close" aria-label="Close search">${d("x",20)}</button>
          </div>
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.inputEl=this.shadow.querySelector("input"),this.resultsEl=this.shadow.querySelector(".viking-search-results-host"),this.shadow.querySelector(".viking-search-palette-close")?.addEventListener("click",()=>this.closePalette()),this.dialogEl?.addEventListener("keydown",n=>{n.key==="Escape"&&(n.preventDefault(),this.closePalette())})}},H=()=>{c(m.tag,m),v(m.searchTag,m),v(m.legacyTag,m)};var J=[{id:"explore",label:"Explore",icon:"globe",appHref:"/explore",marketingHref:"/explore",platform:!0},{id:"documentation",label:"Documentation",icon:"file",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{id:"dashboard",label:"Dashboard",icon:"home",appHref:"/dashboard",marketingHref:"/dashboard",requireAuth:!0,platform:!0},{id:"sites",label:"Sites",icon:"building",appHref:"/settings",marketingHref:"/settings",requireAuth:!0,platform:!0},{id:"account",label:"Account",icon:"user",appHref:"/account",marketingHref:"/account",requireAuth:!0,platform:!0}],Ve=[{title:"Platform",links:[{label:"Explore",appHref:"/explore",marketingHref:"/explore",platform:!0},{label:"Dashboard",appHref:"/dashboard",marketingHref:"/dashboard",platform:!0},{label:"Sites",appHref:"/settings",marketingHref:"/settings",platform:!0},{label:"Account",appHref:"/account",marketingHref:"/account",platform:!0},{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0}]},{title:"Resources",links:[{label:"Documentation",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{label:"Whitepaper",appHref:"https://dataengineeringformachinelearning.com/whitepaper/",marketingHref:"/whitepaper"},{label:"Book",appHref:"https://dataengineeringformachinelearning.com/book/",marketingHref:"/book"}]},{title:"Support",links:[{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0},{label:"Report a Bug",appHref:"#",marketingHref:"#",action:"bug-report"}]},{title:"Legal & Compliance",links:[{label:"Privacy Policy",appHref:"https://dataengineeringformachinelearning.com/privacy/",marketingHref:"/privacy"},{label:"Terms of Service",appHref:"https://dataengineeringformachinelearning.com/terms/",marketingHref:"/terms"},{label:"SOC2 Compliance",appHref:"https://dataengineeringformachinelearning.com/compliance/",marketingHref:"/compliance"},{label:"GDPR Compliance",appHref:"https://dataengineeringformachinelearning.com/privacy/#gdpr",marketingHref:"/privacy#gdpr"},{label:"Cookie Settings",appHref:"#",marketingHref:"#",action:"cookie-settings"}]}],ot="reportBug=1",st="cookieSettings=1",Te=t=>/^https?:\/\//i.test(t),ee=(t,e)=>{if(Te(e))return e;let i=t.replace(/\/$/,""),n=e.startsWith("/")?e:`/${e}`;return`${i}${n}`},ce=t=>`${ee(t.app,"/")}?${ot}`,de=t=>`${ee(t.marketing,"/")}?${st}`,Ne=(t,e)=>Te(t)||t.startsWith("mailto:")||t.startsWith("/")?t:ee(e.marketing,t),He=(t,e,i)=>e==="app"?t:ee(i.app,t),te=(t,e,i)=>t.platform?He(t.appHref,e,i):e==="app"?t.appHref:Ne(t.marketingHref,i),$e=(t,e,i)=>t.action==="bug-report"?e==="app"?"#":ce(i):t.action==="cookie-settings"?de(i):t.platform?He(t.appHref,e,i):e==="app"?t.appHref:Ne(t.marketingHref,i),Oe=(t,e)=>t==="marketing"?"/":e.marketing,ze=(t,e=!1)=>t.filter(i=>!i.requireAuth||e);var M={app:"https://deml.app",marketing:"https://dataengineeringformachinelearning.com",backend:"https://backend.deml.app"};var lt=t=>{let e=new Set;return t.filter(i=>{let n=`${i.title}:${i.href}:${i.action??""}`;return e.has(n)?!1:(e.add(n),!0)})},ct=(t,e,i)=>({title:t.label,href:te(t,e,i),snippet:`Open ${t.label}`,group:"Platform",keywords:[t.id,t.label.toLowerCase(),"navigate","go"]}),dt=(t,e,i,n)=>t.action==="cookie-settings"?{title:t.label,href:de(n),snippet:"Manage analytics and cookie preferences",group:e,keywords:["cookies","consent","privacy","gdpr"],action:"cookie-settings"}:t.action==="bug-report"?{title:t.label,href:i==="app"?"#":ce(n),snippet:"Submit a product issue or regression",group:e,keywords:["bug","issue","support","feedback"],action:"bug-report"}:{title:t.label,href:$e(t,i,n),snippet:`Open ${t.label}`,group:e,keywords:[t.label.toLowerCase(),e.toLowerCase()]},gt=[{title:"Viking-UI Components",snippet:"Browse the design system showcase",group:"Resources",keywords:["viking","ui","design system","components","showcase"]},{title:"Design tokens",snippet:"Canonical --viking-* token matrix",group:"Resources",keywords:["tokens","theme","css","variables"]},{title:"API reference",snippet:"OpenAPI schema and REST endpoints",group:"Resources",keywords:["api","openapi","swagger","rest"]}],ht=(t,e,i)=>{switch(t.title){case"Viking-UI Components":return"https://ui.dataengineeringformachinelearning.com/components";case"Design tokens":return"https://ui.dataengineeringformachinelearning.com/tokens";case"API reference":return`${i.backend.replace(/\/$/,"")}/api/docs`;default:return e==="app"?i.app:i.marketing}},vt=[{title:"Components",href:"/components",snippet:"Browse all documented primitives",group:"Viking-UI",keywords:["components","showcase","registry"]},{title:"Playground",href:"/playground",snippet:"Live Web Component sandbox",group:"Viking-UI",keywords:["playground","sandbox","demo"]},{title:"Architecture",href:"/architecture",snippet:"CSS + WC + Angular layers",group:"Viking-UI",keywords:["architecture","layers","web component"]},{title:"Design tokens",href:"/tokens",snippet:"Canonical --viking-* token matrix",group:"Viking-UI",keywords:["tokens","theme","css","variables"]},{title:"Theming",href:"/theming",snippet:"Light/dark mode and sync pipeline",group:"Viking-UI",keywords:["theming","dark","light","mode"]},{title:"Framework guides",href:"/frameworks",snippet:"Angular, Astro, Django setup",group:"Viking-UI",keywords:["frameworks","angular","astro","django"]},{title:"Contributing",href:"/contributing",snippet:"Extend the Viking-UI kit",group:"Viking-UI",keywords:["contributing","extend","primitives"]}],pt=(t,e)=>t.startsWith("http")?t:`${e.replace(/\/$/,"")}${t.startsWith("/")?t:`/${t}`}`,ge=(t,e,i)=>{let n=t==="docs"?"marketing":t,r=[...J.map(o=>ct(o,n,e)),...Ve.flatMap(o=>o.links.map(s=>dt(s,o.title,n,e))),...gt.map(o=>({...o,href:ht(o,n,e)}))];if(t==="app"&&r.push({title:"Settings",href:"/settings",snippet:"Workspace domains, billing, and security",group:"Platform",keywords:["settings","sites","workspace","configuration"]},{title:"Billing & subscription",href:"/settings/billing",snippet:"Manage plan, invoices, and payment methods",group:"Platform",keywords:["billing","stripe","subscription","payment"]},{title:"Security settings",href:"/settings/security",snippet:"Keys, sessions, and access controls",group:"Platform",keywords:["security","keys","auth","rbac"]}),t==="docs"){let o=i?.docsOrigin??"https://ui.dataengineeringformachinelearning.com";r.push(...vt.map(s=>({...s,href:pt(s.href,o)})))}return lt(r)};var De=new Set(["app","marketing","backend","docs"]),ut=()=>{let t=document.documentElement.getAttribute("data-deml-context");if(t&&De.has(t))return t;let e=window.location.hostname;return e.startsWith("ui.")?"docs":e.includes("deml.app")&&!e.startsWith("backend.")?"app":e.startsWith("backend.")?"backend":"marketing"},kt=t=>{let e=globalThis.__DEML??{};return{app:t.getAttribute("app-url")??e.app??M.app,marketing:t.getAttribute("marketing-url")??e.marketing??M.marketing,backend:t.getAttribute("backend-url")??e.backend??M.backend}},mt=t=>{let e=t.getAttribute("context");return e&&De.has(e)?e:ut()},b=class extends l{static tag="viking-suite-command-palette";static searchTag="viking-suite-search-palette";static legacyTag="viking-suite-search-palette-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","placeholder","global-shortcut"]}paletteEl=null;itemsLoaded=!1;connectedCallback(){H(),this.ensurePalette(),this.loadItems()}attributeChangedCallback(e){if(this.isConnected){if(e==="placeholder"&&this.paletteEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",i);return}if(e==="global-shortcut"&&this.paletteEl){g(this,"global-shortcut")||!this.hasAttribute("global-shortcut")?this.paletteEl.setAttribute("global-shortcut",""):this.paletteEl.removeAttribute("global-shortcut");return}(e==="context"||e==="app-url"||e==="marketing-url"||e==="backend-url")&&this.loadItems(!0)}}openPalette(){this.ensurePalette(),this.loadItems().then(()=>this.paletteEl?.openPalette())}closePalette(){this.paletteEl?.closePalette()}ensurePalette(){if(this.paletteEl)return;this.paletteEl=document.createElement("viking-command-palette"),this.paletteEl.id="deml-command-palette";let e=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",e),(g(this,"global-shortcut")||!this.hasAttribute("global-shortcut"))&&this.paletteEl.setAttribute("global-shortcut",""),this.append(this.paletteEl)}async loadItems(e=!1){if(!this.paletteEl||this.itemsLoaded&&!e)return;let i=mt(this),n=kt(this),r=ge(i,n,{docsOrigin:window.location.origin});try{let o=await fetch("/assets/site-drakkar.json",{cache:"no-cache"});if(o.ok){let s=await o.json();(s.navLinks?.length||s.footerColumns?.length)&&(r=ge(i,n,{docsOrigin:window.location.origin}))}}catch{}this.paletteEl.setAttribute("items",JSON.stringify(r)),this.itemsLoaded=!0}},$=()=>{c(b.tag,b),v(b.searchTag,b),v(b.legacyTag,b)};var bt=`
:host {
  display: inline-flex;
}
...
`,ft=t=>{document.documentElement.setAttribute("data-theme",t),document.documentElement.classList.toggle("dark",t==="dark"),localStorage.setItem("theme",t)},Re=()=>{let t=localStorage.getItem("theme");return t==="light"||t==="dark"?t:window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"},O=class extends l{static tag="viking-theme-toggle-wc";shadow;button=null;sunIcon=null;moonIcon=null;onStorage=()=>{this.syncIcons()};constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,bt)}connectedCallback(){this.render(),this.syncIcons(),this.button?.addEventListener("click",this.onClick),window.addEventListener("storage",this.onStorage),window.addEventListener("viking-theme-change",this.onThemeChange)}disconnectedCallback(){this.button?.removeEventListener("click",this.onClick),window.removeEventListener("storage",this.onStorage),window.removeEventListener("viking-theme-change",this.onThemeChange)}onThemeChange=()=>{this.syncIcons()};onClick=()=>{let e=Re()==="light"?"dark":"light";ft(e),this.syncIcons(),this.dispatchEvent(new CustomEvent("viking-theme-change",{bubbles:!0,composed:!0,detail:{theme:e}}))};syncIcons=()=>{let e=Re()==="dark";this.sunIcon?.classList.toggle("is-visible",e),this.moonIcon?.classList.toggle("is-visible",!e)};render(){let e=this.getAttribute("aria-label")??"Toggle light and dark theme";this.shadow.innerHTML=`
...
`,xt=()=>{let t=document.documentElement.getAttribute("data-deml-context");if(t&&Pe.has(t))return t;let e=window.location.hostname;return e.startsWith("ui.")?"docs":e.startsWith("backend.")?"backend":e.includes("deml.app")?"app":"marketing"},wt=t=>{let e=t.getAttribute("context");return e&&Pe.has(e)?e:xt()},Et=t=>{let e=globalThis.__DEML??{};return{app:t.getAttribute("app-url")??e.app??M.app,marketing:t.getAttribute("marketing-url")??e.marketing??M.marketing,backend:t.getAttribute("backend-url")??e.backend??M.backend}},_t=t=>{try{let e=new URL(t,window.location.origin);return e.origin===window.location.origin&&e.pathname===window.location.pathname}catch{return!1}},D=class extends l{static tag="viking-suite-header";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","authenticated","user-name","user-email","sign-in-href","dashboard-href"]}shadow;menuOpen=!1;userMenuOpen=!1;paletteEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),h(this.shadow,yt)}connectedCallback(){z(),$(),this.render(),document.addEventListener("click",this.onDocumentClick)}disconnectedCallback(){document.removeEventListener("click",this.onDocumentClick)}attributeChangedCallback(){this.isConnected&&this.render()}openSearch(){this.paletteEl?.openPalette(),this.dispatchEvent(new CustomEvent("viking-search-open",{bubbles:!0,composed:!0}))}onDocumentClick=e=>{e.composedPath().includes(this)||(this.menuOpen||this.userMenuOpen)&&(this.menuOpen=!1,this.userMenuOpen=!1,this.syncOpenState())};onSearchClick=()=>{this.openSearch()};onMenuClick=()=>{this.menuOpen=!this.menuOpen,this.syncOpenState()};onUserClick=()=>{this.userMenuOpen=!this.userMenuOpen,this.syncOpenState()};onSignInClick=e=>{let i=new CustomEvent("viking-sign-in",{bubbles:!0,cancelable:!0,composed:!0,detail:e});this.dispatchEvent(i),i.defaultPrevented&&e.preventDefault()};onSignOutClick=()=>{this.userMenuOpen=!1,this.menuOpen=!1,this.syncOpenState(),this.dispatchEvent(new CustomEvent("viking-sign-out",{bubbles:!0,composed:!0}))};syncOpenState(){let e=this.shadow.querySelector(".suite-header"),i=this.shadow.querySelector("[data-menu-toggle]"),n=this.shadow.querySelector(".suite-header__user"),r=this.shadow.querySelector("[data-user-toggle]");e?.setAttribute("data-menu-open",String(this.menuOpen)),i?.setAttribute("aria-expanded",String(this.menuOpen)),n?.setAttribute("data-open",String(this.userMenuOpen)),r?.setAttribute("aria-expanded",String(this.userMenuOpen))}renderNavLinks(e,i,n){return ze(J,n).map(r=>{let o=te(r,e,i),s=_t(o)?' aria-current="page"':"";return`
      ...
      <div class="suite-header__user" data-open="${String(this.userMenuOpen)}">
        ...
        >
          <span class="suite-header__avatar" aria-hidden="true">
            ${d("user",16,"suite-header__avatar-icon")}
          </span>
          <span class="suite-header__user-text">${a(r)}</span>
          ${d("chevron-down",16,"suite-header__chevron")}
        </button>
    ...
    `}getDashboardHref(e){return this.getAttribute("dashboard-href")??`${e.app}/dashboard`}render(){let e=wt(this),i=Et(this),n=g(this,"authenticated"),r=this.renderNavLinks(e,i,n),o=Oe(e,i),s=`${q()}K`;this.shadow.innerHTML=`
        ...
        <div class="suite-header__bar">
          <a class="suite-header__brand" href="${a(o)}" aria-label="Data Engineering for Machine Learning home">
            <span class="suite-header__mark" aria-hidden="true">
              ${d("drakkar",28,"suite-header__brand-icon")}
            </span>
          ...
          <div class="suite-header__actions">
            <button
              ...
              data-search-trigger
            >
              ${d("search",18,"suite-header__action-icon")}
            </button>
            ${this.renderAuth(i,n)}
            ...
            <button
              ...
              data-menu-toggle
            >
              ${d(this.menuOpen?"x":"menu",18,"suite-header__action-icon")}
            </button>
          </div>
        </div>
