...
- **Grid-first layout composition:** All pages compose `viking-page-shell` → `viking-section` with `viking-stack`, `viking-grid`, and `viking-cluster`. Start as single-column grid (`grid-template-columns: 1fr`), scale at 768px (tablet) and 1024px (desktop). Application routes use `viking-app-layout` + `viking-page-template` (supports `width="narrow"`, `"default"`, `"wide"`). Unequal regions use `viking-grid columns="12"` with `viking-grid-item` spans. Never rebuild package breakpoints or gutters locally. Static surfaces use their matching `.page-inner-wrapper`, `.viking-section`, `.viking-stack`, `.viking-grid`, and `.viking-cluster` classes.
