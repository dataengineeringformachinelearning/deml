...
- **Compose layouts from the library:** `viking-page-shell`, `viking-section`, `viking-stack`, `viking-grid`, and `viking-cluster` own responsive gutters, rhythm, columns, and action alignment. Do not duplicate those rules in application styles.
- **Wide-card density law:** dense metric, KPI, status, forecast, and gauge groups are one column when constrained and a maximum of two equal columns (6/12 each) on larger canvases. Keep compact labels/values single-line with accessible overflow handling, and stretch peers to equal height; never reintroduce four-across 3/12 dense cards.
