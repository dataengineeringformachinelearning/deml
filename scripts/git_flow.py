...
def handle_feature(args):
  branch_name = f"feature/{args.name.lower().replace(' ', '-')}"
...
def handle_pr(args):
  ...
  if repo_url and "github.com" in repo_url:
    ...
    pr_url = f"{clean_url}/compare/main...{curr_branch}?expand=1"
    print("\n" + "=" * 60)
...
def handle_release(args):
  ...
  new_ver = bump_version(curr_ver, args.part)
  ...
  if status:
    ...
    lines = status.split("\n")
    non_ver_changes = [line for line in lines if line and not line.endswith("version.txt")]
    if non_ver_changes:
      print(
...
def handle_tag_history(args):
  ...
  for ms in milestones:
    ...
    commit_check = run_cmd(f"git cat-file -t {ms['commit']}", check=False)
    if commit_check == "commit":
      # Check if tag already exists
      tag_exists = run_cmd(f"git tag -l {ms['tag']}", check=False)
