...
describe('chart-zoom', () => {
  ...
  it('clamps window to valid bounds with minimum span', () => {
    expect(clampChartWindow({ start: -5, end: 20 }, 10, 3)).toEqual({ start: 0, end: 10 });
    expect(clampChartWindow({ start: 0, end: 1 }, 10, 3)).toEqual({ start: 0, end: 3 });
    expect(clampChartWindow({ start: 0, end: 0 }, 0, 3)).toEqual({ start: 0, end: 0 });
  });
  ...
  it('pans window by delta points', () => {
    expect(panChartWindow({ start: 5, end: 15 }, 20, 3, 3)).toEqual({ start: 2, end: 12 });
    expect(panChartWindow({ start: 0, end: 10 }, 20, -5, 3)).toEqual({ start: 5, end: 15 });
  });

  it('slices data for the active window', () => {
    const values = ['a', 'b', 'c', 'd', 'e'];
    expect(sliceChartData(values, { start: 1, end: 4 })).toEqual(['b', 'c', 'd']);
  });
});
