...
describe('viking forms', () => {
  ...
  it('shows error alert and invalid host class', async () => {
    ...
    fixture.detectChanges();
    const field = fixture.nativeElement.querySelector('viking-field') as HTMLElement;
    expect(field.classList.contains('viking-field-invalid')).toBe(true);
    const alert = fixture.nativeElement.querySelector('.viking-field-error') as HTMLElement;
  ...
  it('renders stacked form section with icon heading', async () => {
    ...
    expect(section.querySelector('.viking-form-section-title')?.textContent).toContain('Profile');
  });
});
