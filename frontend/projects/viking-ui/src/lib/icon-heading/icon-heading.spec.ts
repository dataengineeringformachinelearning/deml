...
describe('VikingIconHeading', () => {
  ...
  it('renders aligned icon and title', () => {
    ...
    const heading = fixture.nativeElement.querySelector('.viking-icon-heading') as HTMLElement;
    expect(heading).toBeTruthy();
    expect(heading.querySelector('.viking-icon-heading__icon')).toBeTruthy();
    expect(heading.querySelector('h3.viking-icon-heading__title')?.textContent).toContain(
      'Tenant Isolation',
    );
  });
});
