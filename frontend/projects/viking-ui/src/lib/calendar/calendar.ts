  ...
  template: `
    <div class="viking-calendar">
      <div class="viking-calendar-header">
        ...
        <span class="viking-calendar-title" aria-live="polite">{{ title() }}</span>
        <button type="button" class="viking-calendar-nav" aria-label="Next month" (click)="move(1)">
          <viking-icon name="chevron-right" [size]="18" />
        </button>
      </div>
      <div class="viking-calendar-grid" role="group" [attr.aria-label]="title()">
        @for (weekday of weekdays; track weekday) {
          <span class="viking-calendar-weekday" aria-hidden="true">{{ weekday }}</span>
        }
