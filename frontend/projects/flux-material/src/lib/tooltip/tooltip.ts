...
export class FluxTooltip implements OnDestroy {
  ...
  protected show = (): void => {
    ...
    if (this.tooltipKbd()) {
      ...
      kbd.style.cssText =
        'margin-left:9px;padding:0 5px;border:1px solid color-mix(in srgb, var(--flux-accent-content, #ffffff) 35%, transparent);border-radius:5px;';
      tip.appendChild(kbd);
    }
    tip.style.cssText = [
      'position:fixed',
      'z-index:var(--flux-z-tooltip, 1200)',
      'background:var(--jet-black, #31393c)',
      'color:var(--white, #ffffff)',
      'font-family:var(--flux-font-family, Inter, sans-serif)',
      'font-size:var(--flux-font-size, 18px)',
      'line-height:1.35',
      'padding:5px 9px',
      'border-radius:var(--flux-radius, 9px)',
      'border:1px solid var(--flux-border-strong, rgba(255,255,255,0.2))',
