...
import { defineCustomElement, escapeHtml, HTMLElementBase, modKeyLabel } from '../core/dom';
...
const VALID_CONTEXTS = new Set<SiteDrakkarContext>(['app', 'marketing', 'backend', 'docs']);
...
export class VikingSuiteHeaderWc extends HTMLElementBase {
  ...
  openSearch(): void {
    ...
    this.dispatchEvent(new CustomEvent('viking-search-open', { bubbles: true, composed: true }));
  }
  ...
  private readonly onSignOutClick = (): void => {
    ...
    this.menuOpen = false;
    this.syncOpenState();
    this.dispatchEvent(new CustomEvent('viking-sign-out', { bubbles: true, composed: true }));
  };

  private syncOpenState(): void {
    const header = this.shadow.querySelector<HTMLElement>('.suite-header');
    const menuButton = this.shadow.querySelector<HTMLButtonElement>('[data-menu-toggle]');
    const user = this.shadow.querySelector<HTMLElement>('.suite-header__user');
    const userButton = this.shadow.querySelector<HTMLButtonElement>('[data-user-toggle]');
  ...
  private renderAuth(urls: SiteUrls, authenticated: boolean, mobile = false): string {
    if (!authenticated) {
  ...
  private render(): void {
    ...
    this.shadow
      ...
      .forEach(control => control.addEventListener('click', this.onSignInClick));
    this.shadow
      .querySelectorAll('[data-sign-out]')
      .forEach(control => control.addEventListener('click', this.onSignOutClick));
  }
}
