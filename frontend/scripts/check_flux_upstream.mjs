// Compares the flux-material manifest against the live fluxui.dev component
// index so upstream additions, renames, or removals are detected early.
// Usage: npm run check:flux-upstream
import { readFile } from 'node:fs/promises';
import path from 'node:path';
import { fileURLToPath } from 'node:url';

const dirname = path.dirname(fileURLToPath(import.meta.url));
const manifestPath = path.join(dirname, '..', 'projects', 'flux-material', 'flux.manifest.json');
...
const main = async () => {
  ...
  try {
    const response = await fetch(manifest.upstream.componentsIndex, {
      headers: { 'user-agent': 'deml-flux-material-sync-check' },
    });
    if (!response.ok) {
  ...
  console.log(`Tracked in flux-material:       ${tracked.size}`);

  if (missing.length > 0) {
    console.warn(`\nNew upstream components not yet in flux-material:`);
  ...
  if (missing.length === 0 && removed.length === 0) {
    console.log('\nflux-material is in sync with the upstream Flux UI component index.');
  }
