// Builds static site-chrome JSON, viking-icon-paths, and Django partials from site-chrome.config.ts.
...
const resolveBackendNavHref = link => {
  ...
  const pathPart = link.marketingHref.startsWith('/') ? link.marketingHref : `/${link.marketingHref}`;
...
const navLinkHtml = (link, className) => `      <a href="${resolveBackendNavHref(link)}" class="${className}">
        ${iconSlot(link.icon, 16)}
        <span>${link.label}</span>
      </a>`;

const generateSiteNavbarHtml = navLinks => `{% load static %}
{# AUTO-GENERATED by frontend/scripts/build_site_chrome.mjs — do not edit by hand #}
<script>
  window.__DEML = {
...
const templateOutputs = [
  {
    ...
    content: generateSiteNavbarHtml(siteChrome.navLinks),
  },
  {
    path: path.join(rootDir, 'backend', 'templates', 'partials', 'site_footer.html'),
    content: generateSiteFooterHtml(siteChrome.footerColumns),
  },
];
...
console.log('Wrote backend/templates/partials/site_navbar.html and site_footer.html from site-chrome.config.ts.');
