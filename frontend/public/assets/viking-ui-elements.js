...
`;var Pt=new Set(["accent","secondary","success","warning","danger","info","muted","subtle"]),m=class extends s{static tag="viking-badge-wc";static get observedAttributes(){return["tone","size","icon","removable"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),d(this.shadow,ut)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"";return Pt.has(t)?t:null}get size(){return this.getAttribute("size")==="sm"?"sm":null}get removable(){return this.hasAttribute("removable")&&this.getAttribute("removable")!=="false"}onRemove=()=>{this.dispatchEvent(new CustomEvent("viking-removed",{bubbles:!0,composed:!0}))};render(){let t=this.tone;t&&this.getAttribute("tone")!==t?this.setAttribute("tone",t):!t&&this.hasAttribute("tone")&&this.removeAttribute("tone");let i=this.size;i&&this.getAttribute("size")!==i?this.setAttribute("size",i):!i&&this.hasAttribute("size")&&this.removeAttribute("size");let r=this.getAttribute("icon")??(t?H[t]:null),n=r?v(r,16):"";this.shadow.innerHTML=`
      ${n}
      <span part="label"><slot></slot></span>
      ${this.removable?`<button type="button" class="viking-badge-remove" part="remove" aria-label="Remove">${v("x",14)}</button>`:""}
    `,this.shadow.querySelector(".viking-badge-remove")?.addEventListener("click",this.onRemove)}},O=()=>{l(m.tag,m)};var Kt=new Set(["outline","primary","secondary","filled","danger","ghost","subtle"]),Bt=new Set(["sm","xs"]),b=class extends s{static tag="viking-button-wc";static get observedAttributes(){return["variant","size","type","disabled","loading","href","target","aria-label","aria-busy","square","full-width","compact"]}shadow;control=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),d(this.shadow,vt)}connectedCallback(){this.render(),this.control?.addEventListener("click",this.onClick)}disconnectedCallback(){this.control?.removeEventListener("click",this.onClick)}attributeChangedCallback(){this.isConnected&&this.render()}onClick=t=>{if(this.disabled||this.loading){t.preventDefault(),t.stopPropagation();return}this.dispatchEvent(new CustomEvent("viking-press",{bubbles:!0,composed:!0,detail:t}))};get variant(){let t=this.getAttribute("variant")??"outline";return Kt.has(t)?t:"outline"}get size(){let t=this.getAttribute("size");return t&&Bt.has(t)?t:null}get disabled(){return g(this,"disabled")}get loading(){return g(this,"loading")}get square(){return g(this,"square")}render(){let t=this.getAttribute("href"),i=!!t,r=i?"a":"button",n=["viking-btn",`viking-btn-${this.variant}`,this.size?`viking-btn-${this.size}`:"",this.square?"viking-btn-square":""].filter(Boolean).join(" "),a=this.getAttribute("aria-label")??"",o=this.getAttribute("aria-busy")==="true"||this.loading?"true":null;this.shadow.innerHTML=`
      <${r}
        class="${n}"
        part="control"
    ...
    `,this.control=this.shadow.querySelector(r)}},z=()=>{l(b.tag,b)};var Gt=new Set(["accent","secondary","success","warning","danger","info","muted"]),f=class extends s{static tag="viking-callout-wc";static get observedAttributes(){return["tone","heading","icon","dismissible","hidden"]}shadow;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),d(this.shadow,kt)}connectedCallback(){this.render()}attributeChangedCallback(){this.isConnected&&this.render()}get tone(){let t=this.getAttribute("tone")??"info";return Gt.has(t)?t:"info"}get dismissible(){return this.hasAttribute("dismissible")&&this.getAttribute("dismissible")!=="false"}onDismiss=()=>{this.setAttribute("hidden",""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};render(){let t=this.getAttribute("heading")??"",i=this.getAttribute("icon")??H[this.tone]??"info",r=v(i,22,"viking-callout-icon");this.shadow.innerHTML=`
      <div class="viking-callout viking-callout-${this.tone}" role="note" part="surface">
        ...
        <div class="viking-callout-body" part="body">
          ${t?`<p class="viking-callout-heading" part="heading">${c(t)}</p>`:""}
    ...
    `,this.shadow.querySelector(".viking-callout-close")?.addEventListener("click",this.onDismiss)}},$=()=>{l(f.tag,f)};var x=class extends s{static tag="viking-card-wc";static get observedAttributes(){return["compact","interactive","title"]}connectedCallback(){this.syncClasses()}attributeChangedCallback(){this.isConnected&&this.syncClasses()}syncClasses(){this.classList.add("viking-card"),this.classList.toggle("viking-card-compact",this.hasAttribute("compact")),this.classList.toggle("viking-card-interactive",this.hasAttribute("interactive"));let t=this.getAttribute("title");t?(this.setAttribute("role","region"),this.setAttribute("aria-label",t)):(this.removeAttribute("role"),this.removeAttribute("aria-label"))}},R=()=>{l(x.tag,x)};var y=class extends s{static tag="viking-input-wc";static get observedAttributes(){return["type","placeholder","value","disabled","loading","clearable","name","autocomplete","aria-label","bare"]}shadow;internals;input=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.internals=this.attachInternals(),d(this.shadow,pt)}connectedCallback(){this.render(),this.syncFormValue()}attributeChangedCallback(t){if(this.isConnected){if(t==="value"&&this.input){this.input.value=this.getAttribute("value")??"",this.syncFormValue();return}this.render()}}get value(){return this.input?.value??this.getAttribute("value")??""}set value(t){let i=t??"";this.setAttribute("value",i),this.input&&(this.input.value=i),this.syncFormValue()}get disabled(){return g(this,"disabled")}get loading(){return g(this,"loading")}get clearable(){return g(this,"clearable")}get bare(){return g(this,"bare")}onInput=()=>{let t=this.input?.value??"";this.setAttribute("value",t),this.syncFormValue(),this.dispatchEvent(new Event("input",{bubbles:!0,composed:!0})),this.dispatchEvent(new Event("change",{bubbles:!0,composed:!0}))};onBlur=()=>{this.dispatchEvent(new Event("blur",{bubbles:!0,composed:!0}))};onClear=()=>{this.value="",this.input?.focus(),this.dispatchEvent(new CustomEvent("viking-cleared",{bubbles:!0,composed:!0})),this.dispatchEvent(new Event("input",{bubbles:!0,composed:!0}))};syncFormValue(){N(this.internals,this.value)}render(){let t=["viking-input-shell",this.disabled?"viking-disabled":"",this.loading?"viking-loading":""].filter(Boolean).join(" "),i=this.getAttribute("type")??"text",r=this.getAttribute("placeholder")??"",n=this.getAttribute("value")??"",a=this.getAttribute("aria-label")??(r||"Text input"),o=this.getAttribute("autocomplete")??"",p=this.clearable&&n.length>0&&!this.loading&&!this.bare;this.bare?this.shadow.innerHTML=`
        <input
          part="input"
      ...
      `,this.input=this.shadow.querySelector("input"),this.input?.addEventListener("input",this.onInput),this.input?.addEventListener("blur",this.onBlur),this.shadow.querySelector(".viking-input-clear")?.addEventListener("click",this.onClear)}},D=()=>{l(y.tag,y)};var w=class extends s{static tag="viking-modal-wc";static get observedAttributes(){return["open","title","dismissible"]}shadow;dialogEl=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),d(this.shadow,bt)}connectedCallback(){this.render(),this.syncOpen(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick)}attributeChangedCallback(t){if(this.isConnected){if(t==="open"){this.syncOpen();return}if(t==="title"){this.updateTitle();return}t==="dismissible"&&(this.render(),this.syncOpen())}}openModal(){this.setAttribute("open",""),this.syncOpen()}closeModal(){this.removeAttribute("open"),u(this.dialogEl)}open(){this.openModal()}close(){this.closeModal()}get dismissible(){return this.getAttribute("dismissible")!=="false"}onClose=()=>{this.removeAttribute("open"),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{this.dismissible&&t.target===this.dialogEl&&this.closeModal()};syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(T(this.dialogEl),queueMicrotask(()=>{(this.shadow.querySelector(".viking-modal-close")??this.dialogEl)?.focus()})):!t&&this.dialogEl.open&&u(this.dialogEl)}updateTitle(){let t=this.getAttribute("title")??"Dialog",i=this.shadow.querySelector(".viking-modal-heading");i&&(i.textContent=t),this.dialogEl?.setAttribute("aria-label",t)}render(){let t=this.getAttribute("title")??"Dialog";this.shadow.innerHTML=`
      <dialog class="viking-modal-backdrop" aria-label="${c(t)}" aria-modal="true">
        <div class="viking-modal-panel" part="panel" role="document">
          <header class="viking-modal-header" part="header">
            <h2 class="viking-modal-heading" part="title">${c(t)}</h2>
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.shadow.querySelector(".viking-modal-close")?.addEventListener("click",()=>this.closeModal()),this.dialogEl?.addEventListener("keydown",i=>{i.key==="Escape"&&this.dismissible&&(i.preventDefault(),this.closeModal())})}},P=()=>{l(w.tag,w)};var Wt=e=>{let t=e.getAttribute("items");if(!t)return[];try{let i=JSON.parse(t);return Array.isArray(i)?i:[]}catch{return[]}},qt=(e,t)=>[e.title,e.snippet??"",e.group??"",e.href,...e.keywords??[]].join(" ").toLowerCase().includes(t),Ut=(e,t)=>{let i=t.trim().toLowerCase();return i?e.filter(r=>qt(r,i)):e},Ft=e=>{let t=new Map;return e.forEach(i=>{let r=i.group??null,n=t.get(r)??[];n.push(i),t.set(r,n)}),Array.from(t.entries()).map(([i,r])=>({group:i,items:r}))},I=class extends s{static tag="viking-search-palette-wc";static get observedAttributes(){return["open","placeholder","items"]}shadow;dialogEl=null;inputEl=null;resultsEl=null;globalKeyHandler=null;query="";activeIndex=0;flatResults=[];constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),d(this.shadow,ft)}connectedCallback(){this.render(),this.syncOpen(),this.bindGlobalShortcut(),this.dialogEl?.addEventListener("close",this.onClose),this.dialogEl?.addEventListener("click",this.onBackdropClick),this.inputEl?.addEventListener("input",this.onInput),this.inputEl?.addEventListener("keydown",this.onInputKeydown)}disconnectedCallback(){this.dialogEl?.removeEventListener("close",this.onClose),this.dialogEl?.removeEventListener("click",this.onBackdropClick),this.inputEl?.removeEventListener("input",this.onInput),this.inputEl?.removeEventListener("keydown",this.onInputKeydown),this.unbindGlobalShortcut()}attributeChangedCallback(t){if(this.isConnected&&(t==="open"&&this.syncOpen(),t==="items"||t==="placeholder"))if(t==="placeholder"&&this.inputEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026";this.inputEl.placeholder=i,this.inputEl.setAttribute("aria-label",i)}else this.renderResults()}openPalette(){this.setAttribute("open",""),this.syncOpen()}closePalette(){this.removeAttribute("open"),u(this.dialogEl)}search(t){this.query=t,this.inputEl&&(this.inputEl.value=t),this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))}onClose=()=>{this.removeAttribute("open"),this.query="",this.activeIndex=0,this.inputEl&&(this.inputEl.value=""),this.dispatchEvent(new CustomEvent("viking-close",{bubbles:!0,composed:!0}))};onBackdropClick=t=>{t.target===this.dialogEl&&this.closePalette()};onInput=t=>{this.query=t.target.value??this.inputEl?.value??"",this.activeIndex=0,this.renderResults(),this.dispatchEvent(new CustomEvent("viking-query",{bubbles:!0,composed:!0,detail:{query:this.query}}))};onInputKeydown=t=>{if(this.flatResults.length!==0){if(t.key==="ArrowDown"){t.preventDefault(),this.activeIndex=Math.min(this.flatResults.length-1,this.activeIndex+1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="ArrowUp"){t.preventDefault(),this.activeIndex=Math.max(0,this.activeIndex-1),this.renderResults(),this.scrollActiveIntoView();return}if(t.key==="Enter"){t.preventDefault();let i=this.flatResults[this.activeIndex];i&&this.activateItem(i)}}};bindGlobalShortcut(){g(this,"global-shortcut")&&(this.globalKeyHandler=t=>{(t.metaKey||t.ctrlKey)&&t.key.toLowerCase()==="k"&&(t.preventDefault(),this.hasAttribute("open")?this.closePalette():this.openPalette())},document.addEventListener("keydown",this.globalKeyHandler))}unbindGlobalShortcut(){this.globalKeyHandler&&(document.removeEventListener("keydown",this.globalKeyHandler),this.globalKeyHandler=null)}syncOpen(){if(!this.dialogEl)return;let t=this.hasAttribute("open");t&&!this.dialogEl.open?(this.dialogEl.removeAttribute("aria-hidden"),T(this.dialogEl),this.activeIndex=0,this.renderResults(),queueMicrotask(()=>this.inputEl?.focus())):!t&&this.dialogEl.open?(u(this.dialogEl),this.dialogEl.setAttribute("aria-hidden","true")):t||this.dialogEl.setAttribute("aria-hidden","true")}scrollActiveIntoView(){this.resultsEl?.querySelector(".viking-search-result.is-selected")?.scrollIntoView({block:"nearest"})}activateItem(t){if(this.dispatchEvent(new CustomEvent("viking-select",{bubbles:!0,composed:!0,detail:{item:t}})),this.closePalette(),t.action==="cookie-settings"){globalThis.DemlWidgets?.openCookieSettings?.();return}if(t.action==="bug-report"){let i=globalThis.DemlWidgets;if(i?.openBugReport){i.openBugReport();return}}t.href&&t.href!=="#"&&window.location.assign(t.href)}renderResults(){if(!this.resultsEl)return;let t=Wt(this),i=this.query.trim().toLowerCase(),r=Ut(t,i);if(this.flatResults=r,r.length===0){let h=this.query.trim();this.resultsEl.innerHTML=`<p class="viking-search-empty" role="status">${h?"No results found":"Start typing to search\u2026"}</p>`,this.inputEl?.removeAttribute("aria-activedescendant");return}this.activeIndex>=r.length&&(this.activeIndex=r.length-1);let n=0,o=Ft(r).map(({group:h,items:C})=>{let U=h?`<p class="viking-search-group-label" role="presentation">${c(h)}</p>`:"",_=C.map(L=>{let Vt=`viking-search-result-${n}`,et=n===this.activeIndex;return n+=1,`
              <a
                id="${Vt}"
                class="viking-search-result${et?" is-selected":""}"
                role="option"
                aria-selected="${et}"
                href="${c(L.href)}"
                part="result"
              ...
              >
                <div>
                  <div class="viking-search-result-title">${c(L.title)}</div>
                  ${L.snippet?`<div class="viking-search-result-snippet">${c(L.snippet)}</div>`:""}
                </div>
              </a>`}).join("");return`${U}${_}`}).join("");this.resultsEl.innerHTML=`<div class="viking-search-results" role="listbox">${o}</div>`;let p=`viking-search-result-${this.activeIndex}`;this.inputEl?.setAttribute("aria-activedescendant",p),this.inputEl?.setAttribute("role","combobox"),this.inputEl?.setAttribute("aria-expanded","true"),this.inputEl?.setAttribute("aria-controls","viking-search-results-list"),this.resultsEl.querySelectorAll(".viking-search-result").forEach(h=>{h.addEventListener("click",C=>{C.preventDefault();let U=Number(h.dataset.index??0),_=this.flatResults[U];_&&this.activateItem(_)}),h.addEventListener("mouseenter",()=>{let C=Number(h.dataset.index??0);this.activeIndex=C,this.renderResults()})})}render(){let t=this.getAttribute("placeholder")??"Search documentation, dashboard, API\u2026",i=rt();this.shadow.innerHTML=`
    ...
    `,this.dialogEl=this.shadow.querySelector("dialog"),this.inputEl=this.shadow.querySelector("input"),this.resultsEl=this.shadow.querySelector(".viking-search-results-host"),this.shadow.querySelector(".viking-search-palette-close")?.addEventListener("click",()=>this.closePalette()),this.dialogEl?.addEventListener("keydown",r=>{r.key==="Escape"&&(r.preventDefault(),this.closePalette())})}},E=()=>{l(I.tag,I)};var xt=[{id:"explore",label:"Explore",icon:"globe",appHref:"/explore",marketingHref:"/explore",platform:!0},{id:"documentation",label:"Documentation",icon:"file",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{id:"dashboard",label:"Dashboard",icon:"home",appHref:"/dashboard",marketingHref:"/dashboard",requireAuth:!0,platform:!0},{id:"sites",label:"Sites",icon:"building",appHref:"/settings",marketingHref:"/settings",requireAuth:!0,platform:!0},{id:"account",label:"Account",icon:"user",appHref:"/account",marketingHref:"/account",requireAuth:!0,platform:!0}],yt=[{title:"Platform",links:[{label:"Explore",appHref:"/explore",marketingHref:"/explore",platform:!0},{label:"Dashboard",appHref:"/dashboard",marketingHref:"/dashboard",platform:!0},{label:"Sites",appHref:"/settings",marketingHref:"/settings",platform:!0},{label:"Account",appHref:"/account",marketingHref:"/account",platform:!0},{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0}]},{title:"Resources",links:[{label:"Documentation",appHref:"https://dataengineeringformachinelearning.com/documentation/",marketingHref:"/documentation"},{label:"Whitepaper",appHref:"https://dataengineeringformachinelearning.com/whitepaper/",marketingHref:"/whitepaper"},{label:"Book",appHref:"https://dataengineeringformachinelearning.com/book/",marketingHref:"/book"}]},{title:"Support",links:[{label:"Platform Status",appHref:"/status/platform-status",marketingHref:"/status/platform-status",platform:!0},{label:"Report a Bug",appHref:"#",marketingHref:"#",action:"bug-report"}]},{title:"Legal & Compliance",links:[{label:"Privacy Policy",appHref:"https://dataengineeringformachinelearning.com/privacy/",marketingHref:"/privacy"},{label:"Terms of Service",appHref:"https://dataengineeringformachinelearning.com/terms/",marketingHref:"/terms"},{label:"SOC2 Compliance",appHref:"https://dataengineeringformachinelearning.com/compliance/",marketingHref:"/compliance"},{label:"GDPR Compliance",appHref:"https://dataengineeringformachinelearning.com/privacy/#gdpr",marketingHref:"/privacy#gdpr"},{label:"Cookie Settings",appHref:"#",marketingHref:"#",action:"cookie-settings"}]}],jt="reportBug=1",Yt="cookieSettings=1",wt=e=>/^https?:\/\//i.test(e),K=(e,t)=>{if(wt(t))return t;let i=e.replace(/\/$/,""),r=t.startsWith("/")?t:`/${t}`;return`${i}${r}`},X=e=>`${K(e.app,"/")}?${jt}`,Q=e=>`${K(e.marketing,"/")}?${Yt}`,It=(e,t)=>wt(e)||e.startsWith("mailto:")||e.startsWith("/")?e:K(t.marketing,e),Et=(e,t,i)=>t==="app"?e:K(i.app,e),Mt=(e,t,i)=>e.platform?Et(e.appHref,t,i):t==="app"?e.appHref:It(e.marketingHref,i),St=(e,t,i)=>e.action==="bug-report"?t==="app"?"#":X(i):e.action==="cookie-settings"?Q(i):e.platform?Et(e.appHref,t,i):t==="app"?e.appHref:It(e.marketingHref,i);var B={app:"https://deml.app",marketing:"https://dataengineeringformachinelearning.com",backend:"https://backend.deml.app"};var Zt=e=>{let t=new Set;return e.filter(i=>{let r=`${i.title}:${i.href}:${i.action??""}`;return t.has(r)?!1:(t.add(r),!0)})},Xt=(e,t,i)=>({title:e.label,href:Mt(e,t,i),snippet:`Open ${e.label}`,group:"Platform",keywords:[e.id,e.label.toLowerCase(),"navigate","go"]}),Qt=(e,t,i,r)=>e.action==="cookie-settings"?{title:e.label,href:Q(r),snippet:"Manage analytics and cookie preferences",group:t,keywords:["cookies","consent","privacy","gdpr"],action:"cookie-settings"}:e.action==="bug-report"?{title:e.label,href:i==="app"?"#":X(r),snippet:"Submit a product issue or regression",group:t,keywords:["bug","issue","support","feedback"],action:"bug-report"}:{title:e.label,href:St(e,i,r),snippet:`Open ${e.label}`,group:t,keywords:[e.label.toLowerCase(),t.toLowerCase()]},Jt=[{title:"Viking-UI Components",snippet:"Browse the design system showcase",group:"Resources",keywords:["viking","ui","design system","components","showcase"]},{title:"Design tokens",snippet:"Canonical --viking-* token matrix",group:"Resources",keywords:["tokens","theme","css","variables"]},{title:"API reference",snippet:"OpenAPI schema and REST endpoints",group:"Resources",keywords:["api","openapi","swagger","rest"]}],te=(e,t,i)=>{switch(e.title){case"Viking-UI Components":return"https://ui.dataengineeringformachinelearning.com/components";case"Design tokens":return"https://ui.dataengineeringformachinelearning.com/tokens";case"API reference":return`${i.backend.replace(/\/$/,"")}/api/docs`;default:return t==="app"?i.app:i.marketing}},ee=[{title:"Components",href:"/components",snippet:"Browse all documented primitives",group:"Viking-UI",keywords:["components","showcase","registry"]},{title:"Playground",href:"/playground",snippet:"Live Web Component sandbox",group:"Viking-UI",keywords:["playground","sandbox","demo"]},{title:"Architecture",href:"/architecture",snippet:"CSS + WC + Angular layers",group:"Viking-UI",keywords:["architecture","layers","web component"]},{title:"Design tokens",href:"/tokens",snippet:"Canonical --viking-* token matrix",group:"Viking-UI",keywords:["tokens","theme","css","variables"]},{title:"Theming",href:"/theming",snippet:"Light/dark mode and sync pipeline",group:"Viking-UI",keywords:["theming","dark","light","mode"]},{title:"Framework guides",href:"/frameworks",snippet:"Angular, Astro, Django setup",group:"Viking-UI",keywords:["frameworks","angular","astro","django"]},{title:"Contributing",href:"/contributing",snippet:"Extend the Viking-UI kit",group:"Viking-UI",keywords:["contributing","extend","primitives"]}],ie=(e,t)=>e.startsWith("http")?e:`${t.replace(/\/$/,"")}${e.startsWith("/")?e:`/${e}`}`,J=(e,t,i)=>{let r=e==="docs"?"marketing":e,n=[...xt.map(a=>Xt(a,r,t)),...yt.flatMap(a=>a.links.map(o=>Qt(o,a.title,r,t))),...Jt.map(a=>({...a,href:te(a,r,t)}))];if(e==="app"&&n.push({title:"Settings",href:"/settings",snippet:"Workspace domains, billing, and security",group:"Platform",keywords:["settings","sites","workspace","configuration"]},{title:"Billing & subscription",href:"/settings/billing",snippet:"Manage plan, invoices, and payment methods",group:"Platform",keywords:["billing","stripe","subscription","payment"]},{title:"Security settings",href:"/settings/security",snippet:"Keys, sessions, and access controls",group:"Platform",keywords:["security","keys","auth","rbac"]}),e==="docs"){let a=i?.docsOrigin??"https://ui.dataengineeringformachinelearning.com";n.push(...ee.map(o=>({...o,href:ie(o.href,a)})))}return Zt(n)};var At=new Set(["app","marketing","backend","docs"]),re=()=>{let e=document.documentElement.getAttribute("data-deml-context");if(e&&At.has(e))return e;let t=window.location.hostname;return t.startsWith("ui.")?"docs":t.includes("deml.app")&&!t.startsWith("backend.")?"app":t.startsWith("backend.")?"backend":"marketing"},ne=e=>{let t=globalThis.__DEML??{};return{app:e.getAttribute("app-url")??t.app??B.app,marketing:e.getAttribute("marketing-url")??t.marketing??B.marketing,backend:e.getAttribute("backend-url")??t.backend??B.backend}},ae=e=>{let t=e.getAttribute("context");return t&&At.has(t)?t:re()},M=class extends s{static tag="viking-suite-search-palette-wc";static get observedAttributes(){return["context","app-url","marketing-url","backend-url","placeholder","global-shortcut"]}paletteEl=null;itemsLoaded=!1;connectedCallback(){E(),this.ensurePalette(),this.loadItems()}attributeChangedCallback(t){if(this.isConnected){if(t==="placeholder"&&this.paletteEl){let i=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",i);return}if(t==="global-shortcut"&&this.paletteEl){g(this,"global-shortcut")||!this.hasAttribute("global-shortcut")?this.paletteEl.setAttribute("global-shortcut",""):this.paletteEl.removeAttribute("global-shortcut");return}(t==="context"||t==="app-url"||t==="marketing-url"||t==="backend-url")&&this.loadItems(!0)}}openPalette(){this.ensurePalette(),this.loadItems().then(()=>this.paletteEl?.openPalette())}closePalette(){this.paletteEl?.closePalette()}ensurePalette(){if(this.paletteEl)return;this.paletteEl=document.createElement("viking-search-palette-wc"),this.paletteEl.id="deml-command-palette";let t=this.getAttribute("placeholder")??"Search documentation, dashboard, settings\u2026";this.paletteEl.setAttribute("placeholder",t),(g(this,"global-shortcut")||!this.hasAttribute("global-shortcut"))&&this.paletteEl.setAttribute("global-shortcut",""),this.append(this.paletteEl)}async loadItems(t=!1){if(!this.paletteEl||this.itemsLoaded&&!t)return;let i=ae(this),r=ne(this),n=J(i,r,{docsOrigin:window.location.origin});try{let a=await fetch("/assets/site-drakkar.json",{cache:"no-cache"});if(a.ok){let o=await a.json();(o.navLinks?.length||o.footerColumns?.length)&&(n=J(i,r,{docsOrigin:window.location.origin}))}}catch{}this.paletteEl.setAttribute("items",JSON.stringify(n)),this.itemsLoaded=!0}},G=()=>{l(M.tag,M)};var S=class extends s{static tag="viking-select-wc";static get observedAttributes(){return["label","name","value","placeholder","description","error","width","disabled","required"]}shadow;internals;selectEl=null;optionObserver=null;controlId=it("viking-select");constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),this.internals=this.attachInternals(),d(this.shadow,mt)}connectedCallback(){this.render(),this.syncOptions(),this.observeOptions(),this.selectEl?.addEventListener("change",this.onChange)}disconnectedCallback(){this.selectEl?.removeEventListener("change",this.onChange),this.optionObserver?.disconnect(),this.optionObserver=null}attributeChangedCallback(t){if(this.isConnected){if(t==="value"&&this.selectEl){this.selectEl.value=this.getAttribute("value")??"",this.syncFormValue();return}if(t==="error"||t==="description"||t==="label"){this.render(),this.syncOptions();return}this.render(),this.syncOptions()}}get value(){return this.selectEl?.value??this.getAttribute("value")??""}set value(t){this.setAttribute("value",t),this.selectEl&&(this.selectEl.value=t),this.syncFormValue()}onChange=()=>{let t=this.selectEl?.value??"";this.setAttribute("value",t),this.syncFormValue(),this.dispatchEvent(new CustomEvent("viking-change",{bubbles:!0,composed:!0,detail:{value:t}}))};syncFormValue(){N(this.internals,this.value)}observeOptions(){this.optionObserver?.disconnect(),this.optionObserver=new MutationObserver(()=>this.syncOptions()),this.optionObserver.observe(this,{childList:!0,subtree:!0,characterData:!0})}render(){let t=this.getAttribute("label")??"",i=this.getAttribute("name")??"",r=this.hasAttribute("disabled"),n=this.hasAttribute("required"),a=this.getAttribute("error")??"",o=this.getAttribute("description")??"",p=[o&&`${this.controlId}-desc`,a&&`${this.controlId}-error`].filter(Boolean).join(" ");this.shadow.innerHTML=`
      <div class="viking-field" part="field">
        ${t?`<label class="viking-field-label" part="label" for="${this.controlId}">${c(t)}</label>`:""}
        <select
          id="${this.controlId}"
...
`,se=e=>{document.documentElement.setAttribute("data-theme",e),document.documentElement.classList.toggle("dark",e==="dark"),localStorage.setItem("theme",e)},Ct=()=>{let e=localStorage.getItem("theme");return e==="light"||e==="dark"?e:window.matchMedia("(prefers-color-scheme: dark)").matches?"dark":"light"},A=class extends s{static tag="viking-theme-toggle-wc";shadow;button=null;sunIcon=null;moonIcon=null;constructor(){super(),this.shadow=this.attachShadow({mode:"open"}),d(this.shadow,oe)}connectedCallback(){this.render(),this.syncIcons(),this.button?.addEventListener("click",this.onClick)}disconnectedCallback(){this.button?.removeEventListener("click",this.onClick)}onClick=()=>{let t=Ct()==="light"?"dark":"light";se(t),this.syncIcons(),this.dispatchEvent(new CustomEvent("viking-theme-change",{bubbles:!0,composed:!0,detail:{theme:t}}))};syncIcons=()=>{let t=Ct()==="light";this.sunIcon?.classList.toggle("is-visible",t),this.moonIcon?.classList.toggle("is-visible",!t)};render(){let t=this.getAttribute("aria-label")??"Toggle light and dark theme";this.shadow.innerHTML=`
    ...
    `,this.button=this.shadow.querySelector("button"),this.sunIcon=this.shadow.querySelector(".theme-icon-sun"),this.moonIcon=this.shadow.querySelector(".theme-icon-moon")}},q=()=>{l(A.tag,A)};var tt=()=>{z(),D(),O(),$(),R(),W(),P(),E(),G(),q()};typeof globalThis<"u"&&typeof document<"u"&&(document.readyState==="loading"?document.addEventListener("DOMContentLoaded",tt):tt());return Ot(le);})();
