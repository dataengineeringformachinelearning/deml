...
**Additional Services for Reliability:**

- **Outbox Relay** (`python manage.py outbox_relay --once` or as daemon/cron): Publishes from the `OutboxEvent` table to Redpanda. Run as a separate Railway service or scheduled job to guarantee delivery.
- Idempotency and DLQ are handled inside the telemetry_worker.

**Note on Firebase Components**: Firebase Cloud Functions (`ingestEvent`, with event versioning) and Firestore rules (for the `deml` named database and Event Projections user data isolation) are deployed independently via a dedicated GitHub Actions workflow (`.github/workflows/firebase-backend-deploy.yml`) using the `FIREBASE_SERVICE_ACCOUNT_DEMLDOTCOM` service account. This is separate from Railway backend services and the marketing hosting deploy.
...
_(Once deployed, ensure the `DRAGONFLY_HOST` environment variable on the **Web Backend**, **Telemetry Worker**, and **ML Training Worker** points to this internal DNS, e.g., `deml-dragonfly.railway.internal`)_
